...
> => {
    const suspendedRetrySchedule =
        options.suspendedRetrySchedule ?? defaultRetrySchedule;
    ...
    const self: Workflow<Name, any, Success, Error> = {
        ...
        execute: Effect.fnUntraced(
            function* (fields: any, opts) {
                ...
                if (Option.isSome(parentInstance)) {
                    const result = yield* wrapActivityResult(
                        run,
                    ...
                    return yield* result.exit as Exit.Exit<
                        Success["Type"],
                        Error["Type"]
                    >;
                }
                ...
                while (true) {
                    ...
                    sleep ??= (yield* Schedule.driver(suspendedRetrySchedule))
                        ...
                        .pipe(
                            Effect.catchAll(() =>
                                Effect.dieMessage(
                                    `${options.name}.execute: suspendedRetrySchedule exhausted`,
                                ),
                            ),
                        );
                    yield* sleep;
        ...
        executionId: (payload) =>
            makeExecutionId(self.payloadSchema.make(payload)),
        withCompensation,
    };
        ...
        return Effect.uninterruptibleMask((restore) =>
            restore(effect).pipe(
                ...
                Effect.matchCauseEffect({
                    onSuccess: (value) =>
                        Effect.succeed(
                            new Complete({ exit: Exit.succeed(value) }),
                        ),
                    onFailure: (cause): Effect.Effect<Result<A, E>> =>
                        instance.suspended
                            ? Effect.succeed(
                                  new Suspended({ cause: instance.cause }),
                              )
        ...
        Effect.uninterruptibleMask((restore) =>
            Effect.tap(restore(effect), (value) =>
                Effect.contextWithEffect(
                    (context: Context.Context<WorkflowInstance>) =>
                        Effect.addFinalizer((exit) =>
                            Exit.isSuccess(exit) ||
                            Context.get(context, InstanceTag).suspended
                                ? Effect.void
