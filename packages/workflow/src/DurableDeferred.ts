...
>(self: DurableDeferred<Success, Error>) {
    ...
    const oexit = yield* Workflow.wrapActivityResult(
    ...
    return yield* Effect.flatten(
        Effect.orDie(Schema.decodeUnknown(self.exitSchema)(oexit.value)),
    );
});
                ...
                return Effect.onExit(
                    ...
                    Effect.fnUntraced(function* (exit) {
                        ...
                        const encodedExit = yield* Effect.orDie(
                            Schema.encode(self.exitSchema)(exit),
                        );
...
export const tokenFromPayload: {
    ...
    >(
        ...
        Effect.map(
            options.workflow.executionId(options.payload),
            (executionId) =>
