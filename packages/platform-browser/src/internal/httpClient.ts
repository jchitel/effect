            ...
            Effect.async<ClientResponseImpl, Error.RequestError>((resume) => {
                ...
                const onChange = () => {
                    if (!sent && xhr.readyState >= 2) {
                        ...
                        resume(
                            Effect.succeed(
                                new ClientResponseImpl(request, xhr),
                            ),
                        );
                    }
                };
    ...
    implements IncomingMessage.HttpIncomingMessage<E>
{
    ...
    get headers() {
        ...
        const result = parser(
            encoder.encode(this._rawHeaderString + "\r\n"),
            0,
        );
        this._rawHeaders =
            result._tag === "Headers" ? result.headers : undefined;
        const parsed =
            result._tag === "Headers"
        ...
        return Stream.async<Uint8Array, E>((emit) => {
            ...
            const onReadyStateChange = () => {
                if (this.source.readyState === 3) {
                    emit.single(
                        encoder.encode(this.source.responseText.slice(offset)),
                    );
    ...
    get arrayBuffer(): Effect.Effect<ArrayBuffer, E> {
        ...
        return (this._arrayBufferEffect = Effect.async<ArrayBuffer, E>(
            (resume) => {
                if (this.source.readyState === 4) {
