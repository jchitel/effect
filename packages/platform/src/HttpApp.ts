        ...
        >((fiber) =>
            Effect.flatMap(
                ...
                ([response, cause]) => {
                    ...
                    const handler = fiber.getFiberRef(
    ...
    const withMiddleware = unify(
        middleware === undefined
            ...
            : Effect.matchCauseEffect(
                  middleware(internalMiddleware.tracer(withErrorHandling)),
                  {
...
export const toWebHandlerRuntime = <R>(runtime: Runtime.Runtime<R>) => {
    const httpRuntime: Types.Mutable<Runtime.Runtime<R>> =
                ...
                request.signal?.addEventListener(
                    ...
                    () => {
                        fiber.unsafeInterruptAsFork(
                            ServerError.clientAbortFiberId,
                        );
                    },
                    { once: true },
                );
