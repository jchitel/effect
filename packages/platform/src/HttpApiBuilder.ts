    ...
    if (contentType.includes("application/json")) {
        ...
        return Effect.orDie(
            Option.match(multipartLimits, {
                ...
                onSome: (limits) =>
                    Multipart.withLimits(request.multipart, limits),
            }),
        );
    } else if (contentType.includes("x-www-form-urlencoded")) {
        return Effect.map(
            Effect.orDie(request.urlParamsBody),
            UrlParams.toRecord,
        );
...
export function normalizeUrlParams(
    ...
    for (const key in params) {
        ...
        out[key] =
            Array.isArray(value) || isSingleStringType(ast, key)
                ? value
                : [value];
    }
    return out;
...
const handlerToRoute = (
    ...
    const multipartLimits = endpoint.payloadSchema.pipe(
        Option.flatMapNullable(
            ({ ast }) =>
                HttpApiSchema.getMultipart(ast) ||
                HttpApiSchema.getMultipartStream(ast),
        ),
    );
    ...
    const decodeHeaders = Option.map(
        endpoint.headersSchema,
        Schema.decodeUnknown,
    );
    const encodeSuccess = Schema.encode(
        makeSuccessSchema(endpoint.successSchema),
    );
    return HttpRouter.makeRoute(
        ...
        applyMiddleware(
            ...
            Effect.gen(function* () {
                ...
                const routeContext = Context.unsafeGet(
                    context,
                ...
                if (decodeHeaders._tag === "Some") {
                    request.headers = yield* decodeHeaders.value(
                        httpRequest.headers,
                    );
                }
...
> => {
    switch (self._tag) {
        ...
        case "ApiKey": {
            const key =
                self.in === "header" ? self.key.toLowerCase() : self.key;
