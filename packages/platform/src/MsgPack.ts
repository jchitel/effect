                ...
                Channel.zipRight(
                    Channel.flatMap(
                        Effect.try({
                            try: () =>
                                Chunk.of(
                                    packr.pack(Chunk.toReadonlyArray(input)),
                                ),
                            catch: (cause) =>
                                new MsgPackError({ reason: "Pack", cause }),
                        }),
                        Channel.write,
                    ),
                    loop,
            ...
            const unpack = (value: Chunk.Chunk<Uint8Array>) =>
                Effect.try({
                    try: () =>
                        Chunk.flatMap(value, (buf) => {
                            ...
                            try {
                                return Chunk.unsafeFromArray(
                                    packr.unpackMultiple(buf).flat(),
                                );
                            } catch (error_) {
                                const error: any = error_;
                                if (error.incomplete) {
                                    incomplete = buf.subarray(
                                        error.lastPosition,
                                    );
                                    return Chunk.unsafeFromArray(
                                        error.values ?? [],
                        ...
                        new MsgPackError({ reason: "Unpack", cause }),
                });
 ...
 * @category schemas
 */
export interface schema<S extends Schema.Schema.Any>
