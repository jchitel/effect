        ...
        HttpApi.reflect(api as any, {
            ...
            onEndpoint(onEndpointOptions) {
                ...
                const decodeResponse =
                    HttpClientResponse.matchStatus(decodeMap);
                ...
                const encodePayloadBody = endpoint.payloadSchema.pipe(
                    Option.map((schema) => {
                        if (HttpMethod.hasBody(endpoint.method)) {
                            return Schema.encodeUnknown(
                                payloadSchemaBody(schema as any),
                            );
                        }
                    ...
                    function* (request: {
                        ...
                        if (encodeUrlParams._tag === "Some") {
                            httpRequest = HttpClientRequest.appendUrlParams(
                                httpRequest,
                                (yield* encodeUrlParams.value(
                                    request.urlParams,
                                )) as any,
                            );
                        }
                        const response = yield* httpClient.execute(httpRequest);
                        const value = yield* options.transformResponse ===
                        undefined
                            ? decodeResponse(response)
                            : options.transformResponse(
                                  decodeResponse(response),
                              );
                        return request?.withResponse === true
                            ? [value, response]
                            : value;
                    },
...
>(
    ...
    HttpApiMiddleware.HttpApiMiddleware.Without<
        | ApiR
        | HttpApiGroup.ClientContext<HttpApiGroup.WithName<Groups, GroupName>>
    >
> => {
    ...
    encode: (a, _, ast) => {
        ...
        return ParseResult.try({
            try: () => JSON.stringify(a),
                ...
                new ParseResult.Type(ast, a, "Could not encode as JSON"),
        });
    },
});
...
const schemaFromArrayBuffer = (
    ...
    if (ast._tag === "Union") {
        return Schema.Union(
            ...ast.types.map((ast) =>
                schemaFromArrayBuffer(
                    ast,
                    HttpApiSchema.getEncoding(ast, encoding),
                ),
            ),
        );
    }
...
const payloadSchemaBody = (
    ...
    const members =
        schema.ast._tag === "Union" ? schema.ast.types : [schema.ast];
