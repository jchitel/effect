            ...
            return Effect.gen(function* () {
                    ...
                    Effect.suspend(() => {
                        ...
                        switch (response[1]) {
                            ...
                            case 3: {
                                ...
                                const cause = WorkerError.decodeCause(
                                    response[2],
                                );
                                return Deferred.DeferredTypeId in mailbox
                                    ? Deferred.failCause(mailbox, cause)
                ...
                >(
                    ...
                    Effect.withFiberRuntime<{
                        ...
                        const context = fiber.getFiberRef(
                            ...
                            Effect.tap((mailbox) => {
                                ...
                                return wrappedEncode(request).pipe(
                                    ...
                                    Effect.catchAllCause((cause) =>
                                        Mailbox.isMailbox<O, E | WorkerError>(
                                            mailbox,
                                        )
                                            ? mailbox.failCause(cause)
                                            : Deferred.failCause(
                ...
                const executeRelease = (
                    ...
                    return Exit.isFailure(exit)
                        ? Effect.zipRight(
                              Effect.orDie(backing.send([id, 1])),
                              release,
                          )
                        : release;
                };
                ...
                if (initialMessage) {
                    yield* Effect.sync(initialMessage).pipe(
                        ...
                        Effect.mapError(
                            (cause) =>
                                new WorkerError({ reason: "spawn", cause }),
                        ),
                    );
                }
...
export const makePool = <I, O, E>(options: Worker.WorkerPool.Options<I>) =>
    Effect.gen(function* () {
        ...
        const pool: Worker.WorkerPool<I, O, E> = {
            ...
            execute: (message: I) =>
                Stream.unwrapScoped(
                    Effect.map(backing.get, (worker) =>
                        worker.execute(message),
                    ),
                ),
            executeEffect: (message: I) =>
                Effect.scoped(
                    Effect.flatMap(backing.get, (worker) =>
                        worker.executeEffect(message),
                    ),
                ),
        };
    ...
    Effect.gen(function* () {
        ...
        const execute = <Req extends I>(message: Req) => {
            const parseSuccess = Schema.decode(
                Schema.successSchema(message as any),
            );
            const parseFailure = Schema.decode(
                Schema.failureSchema(message as any),
            );
            return pipe(
        ...
        PlatformWorker.of({
            ...
            spawn<I, O>(id: number) {
                return Effect.gen(function* () {
                    ...
                    const buffer: Array<
                        [I, ReadonlyArray<unknown> | undefined]
                    > = [];
                    ...
                    const send = (
                        ...
                        Effect.try({
                            try: () => {
                                ...
                                new WorkerError({ reason: "send", cause }),
                        });
