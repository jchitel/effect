            ...
            Effect.flatMap(effect, (request) =>
                Effect.withFiberRuntime((fiber) => {
                    ...
                    return Effect.useSpan(
                        ...
                        (span) => {
                            span.attribute(
                                OtelSemConv.ATTR_HTTP_REQUEST_METHOD,
                                request.method,
                            );
                            span.attribute(
                                OtelSemConv.ATTR_SERVER_ADDRESS,
                                url.origin,
                            );
                            if (url.port !== "") {
                                span.attribute(
                                    OtelSemConv.ATTR_SERVER_PORT,
                                    +url.port,
                                );
                            }
                            span.attribute(
                                OtelSemConv.ATTR_URL_FULL,
                                url.toString(),
                            );
                            span.attribute(
                                OtelSemConv.ATTR_URL_PATH,
                                url.pathname,
                            );
                            span.attribute(
                                OtelSemConv.ATTR_URL_SCHEME,
                                url.protocol.slice(0, -1),
                            );
                            const query = url.search.slice(1);
                            if (query !== "") {
                                span.attribute(
                                    OtelSemConv.ATTR_URL_QUERY,
                                    query,
                                );
                            }
                            ...
                            for (const name in redactedHeaders) {
                                span.attribute(
                                    OtelSemConv.ATTR_HTTP_REQUEST_HEADER(name),
                                    String(redactedHeaders[name]),
                                );
                            }
                            request = fiber.getFiberRef(
                            ...
                            return Effect.uninterruptibleMask((restore) =>
                                restore(
                                    ...
                                    Effect.matchCauseEffect({
                                        onSuccess: (response) => {
                                            span.attribute(
                                                OtelSemConv.ATTR_HTTP_RESPONSE_STATUS_CODE,
                                                response.status,
                                            );
                                            ...
                                            for (const name in redactedHeaders) {
                                                span.attribute(
                                                    OtelSemConv.ATTR_HTTP_RESPONSE_HEADER(
                                                        name,
                                                    ),
                                                    String(
...
export const retryTransient: {
    ...
    <E extends E0, E0, R, B, R1 = never>(
        ...
        transformResponse(
            ...
            Effect.retry({
                while:
                    Schedule.ScheduleTypeId in options ||
