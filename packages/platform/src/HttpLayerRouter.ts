...
export interface HttpRouter {
    ...
    readonly addAll: <const Routes extends ReadonlyArray<Route<any, any>>>(
        ...
        | Request.From<
              "Requires",
              Exclude<Route.Context<Routes[number]>, Provided>
          >
    ...
    return HttpRouter.of({
        ...
        prefixed(this: HttpRouter, prefix: string) {
            return HttpRouter.of({
                ...
                addAll: (routes) =>
                    addAll(routes.map(prefixRoute(prefix))) as any,
...
function sliceRequestUrl(
    ...
    return request.modify({
        url:
            request.url.length <= prefexLen
                ? "/"
                : request.url.slice(prefexLen),
    });
}
...
export declare namespace Request {
     ...
     * @category Request types
     */
    export type Without<A> =
        A extends Request<infer _Kind, infer _> ? never : A;
}
        ...
        : new MiddlewareImpl(
              Effect.isEffect(middleware)
                  ...
                  : (Layer.succeedContext(
                        Context.unsafeMake(
                            new Map([[fnContextKey, middleware]]),
                        ),
                    ) as any),
          );
    ...
    constructor(
        ...
        this.layer = Layer.scopedContext(
            Effect.gen(this, function* () {
                ...
                return Context.unsafeMake<never>(
                    new Map([[contextKey, stack]]),
                );
            }),
...
> => {
    ...
    const RouterLayer = options?.routerConfig
        ? Layer.provide(
              layer,
              Layer.succeed(RouterConfig, options.routerConfig),
          )
        : layer;
