        ...
        onEndpoint({
            ...
            function processResponseMap(
                ...
                for (const [status, { ast, description }] of map) {
                    ...
                    ast.pipe(
                        Option.filter(
                            (ast) => !HttpApiSchema.getEmptyDecodeable(ast),
                        ),
                        Option.map((ast) => {
            ...
            middleware.forEach((middleware) => {
                ...
                for (const [name, security] of Object.entries(
                    middleware.security,
                )) {
                    processHttpApiSecurity(name, security);
            ...
            const method =
                endpoint.method.toLowerCase() as OpenAPISpecMethodName;
            if (!spec.paths[path]) {
                spec.paths[path] = {};
            }
