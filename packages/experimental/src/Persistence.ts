...
export interface ResultPersistenceStore {
    ...
    readonly getMany: <R, IE, E, IA, A>(
        ...
        Array<Option.Option<Exit.Exit<A, E>>>,
        PersistenceError,
        R
    >;
...
export declare namespace ResultPersistence {
    ...
    export interface Key<R, IE, E, IA, A>
        extends Schema.WithResult<A, IA, E, IE, R>,
     ...
     */
    export type KeyAny =
        | Key<any, any, any, any, any>
        | Key<any, never, never, any, any>;
        ...
        return ResultPersistence.of({
            ...
            make: (options) =>
                Effect.gen(function* () {
                    ...
                    const timeToLive =
                        options.timeToLive ?? (() => Duration.infinity);
    ...
    () => {
        ...
        const getStore = (storeId: string) => {
            ...
            if (store === undefined) {
                store = new Map<
                    string,
                    readonly [unknown, expires: number | null]
                >();
            ...
            make: (storeId) =>
                Effect.map(Effect.clock, (clock) => {
                    ...
                    return identity<BackingPersistenceStore>({
                        ...
                        getMany: (keys) =>
                            Effect.sync(() => keys.map(unsafeGet)),
                        set: (key, value, ttl) =>
                            Effect.sync(() =>
                                map.set(key, [
                                    value,
                                    unsafeTtlToExpires(clock, ttl),
                                ]),
                            ),
