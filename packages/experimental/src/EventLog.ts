                ...
                const writePayload = (
                    ...
                    Effect.gen(function* () {
                        const event = group.events[
                            tag
                        ] as any as Event.AnyWithProps;
                        const entry = new Entry(
                            {
                                id: makeEntryId({ msecs: timestamp }),
                                event: tag,
                                payload: yield* Schema.encode(
...
export class EventLog extends Context.Tag(
    ...
    EventLog,
    {
        ...
        >(options: {
            ...
            readonly event: Tag;
            readonly payload: Event.PayloadWithTag<
                EventGroup.Events<Groups>,
                Tag
            >;
        }) => Effect.Effect<
            Event.SuccessWithTag<EventGroup.Events<Groups>, Tag>,
            | Event.ErrorWithTag<EventGroup.Events<Groups>, Tag>
            | EventJournalError
        >;
                            ...
                            compact:
                                compactors.size > 0
                                    ? (remoteEntries) =>
                                          Effect.gen(function* () {
                                              let unprocessed =
                                                  remoteEntries as Array<RemoteEntry>;
                                              ...
                                              while (true) {
                                                  ...
                                                  for (
                                                      ...
                                                      if (
                                                          ...
                                                          uncompactedRemote.push(
                                                              remoteEntry,
                                                          );
                                                          continue;
                                                      }
                                                      if (
                                                          uncompacted.length > 0
                                                      ) {
                                                          brackets.push([
                                                              uncompacted,
                                                              uncompactedRemote,
    ...
    return EventLog.of({
        write: (options: {
            ...
            if (handler === undefined) {
                return Effect.die(
                    `Event handler not found for: "${options.event}"`,
                );
            }
            return Effect.gen(function* () {
