...
export interface Actor<M extends Machine.Any>
    extends Subscribable.Subscribable<Machine.State<M>> {
    ...
    Effect.gen(function* () {
        ...
        const sendIgnore = <R extends Machine.Public<M>>(request: R) =>
            Effect.flatMap(requestContext(request), (item) => {
                ...
                return Effect.useSpan(
                    ...
                    (span) =>
                        Queue.offer(requests, [request, deferred, span, true]),
                );
            });
        ...
        const run = Effect.gen(function* () {
            ...
            const fork = <A, E, R>(effect: Effect.Effect<A, E, R>) =>
                Effect.asVoid(
                    FiberSet.run(fiberSet, MachineDefect.wrap(effect)),
                );
            const forkWith: {
                (
...
> =>
    Effect.flatMap(
        ...
        ([input, previousState]) =>
            (boot as any)(self, input, { previousState }),
    );
