                ...
                Effect.acquireUseRelease(
                    ...
                    (entries, exit) =>
                        Effect.sync(() => {
                            ...
                            for (
                                let i = remote.missing.length - 1;
                                i >= 0;
                                i--
                            ) {
                                if (remote.missing[i].id === last.id) {
                                    remote.missing = remote.missing.slice(
                                        i + 1,
                                    );
                                    break;
                                }
            ...
            writeFromRemote: (options) =>
                Effect.gen(function* () {
                    ...
                    for (const [compacted, remoteEntries] of brackets) {
                        for (const originEntry of compacted) {
                            const conflicts: Array<Entry> = [];
                            yield* Effect.async<void, EventJournalError>(
                                (resume) => {
                                    const tx = db.transaction(
                                        "entries",
                                        "readonly",
                                    ...
                                    cursorRequest.onsuccess = () => {
                                        ...
                                        if (!cursor) return;
                                        const decodedEntry = decodeEntryIdb(
                                            cursor.value,
                                        );
                                        if (
                                            decodedEntry.event ===
                                                originEntry.event &&
                                            decodedEntry.primaryKey ===
                                                originEntry.primaryKey
                                        ) {
                        ...
                        yield* Effect.async<void, EventJournalError>(
                            (resume) => {
                                ...
                                for (const remoteEntry of remoteEntries) {
                                    entries.add(
                                        encodeEntryIdb(remoteEntry.entry),
                                    );
                                    remotes.put({
                                        remoteId: options.remoteId,
                                        entryId: remoteEntry.entry.id,
            ...
            withRemoteUncommited: (remoteId, f) =>
                Effect.async<Array<Entry>, EventJournalError>((resume) => {
                    ...
                    remoteEntryIdStore.get(remoteId).onsuccess = (event) => {
                        const startEntryId = (event.target as any).result
                            ?.entryId;
                        ...
                        entryCursor.onsuccess = () => {
                            ...
                            remotesStore.get([remoteId, entry.id]).onsuccess = (
                                event,
                            ) => {
                                if (!(event.target as any).result)
                                    entries.push(entry);
...
const idbReq = <T>(method: string, evaluate: () => IDBRequest<T>) =>
    Effect.async<T, EventJournalError>((resume) => {
        ...
        request.onerror = () =>
            resume(
                Effect.fail(
                    new EventJournalError({ method, cause: request.error }),
                ),
            );
    });
