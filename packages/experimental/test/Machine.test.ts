...
describe("Machine", () => {
    test("counter", () =>
        Effect.gen(function* (_) {
            ...
            assert.strictEqual(
                yield* _(booted.send(new IncrementBy({ number: 2 }))),
                4,
            );
            assert.strictEqual(yield* _(booted.send(new Decrement())), 3);
            assert.strictEqual(
                yield* _(booted.send(new FailBackground())),
                undefined,
            );
    ...
    test("forkWithState", () =>
        Effect.gen(function* (_) {
            ...
            assert.strictEqual(
                yield* _(
                    booted.send(
                        new DelayedIncrementBy({ number: 2, delay: 10 }),
                    ),
                ),
                undefined,
            );
...
describe("SerializableMachine", () => {
    test("counter", () =>
        Effect.gen(function* (_) {
            ...
            assert.deepStrictEqual(
                yield* _(actor.sendUnknown({ _tag: "Decrement" })),
                {
                    _tag: "Success",
                    value: 11,
                },
            ...
            const restored = yield* _(
                Machine.restore(counterSerializable, snapshot),
            );
