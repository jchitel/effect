...
export const make = (
    ...
    Effect.gen(function* () {
        ...
        class LibsqlConnectionImpl implements LibsqlConnection {
            ...
            run(sql: string, params: ReadonlyArray<Statement.Primitive> = []) {
                return Effect.map(
                    Effect.tryPromise({
                        try: () =>
                            this.sdk.execute({
            ...
            runRaw(
                ...
                return Effect.tryPromise({
                    try: () =>
                        this.sdk.execute({ sql, args: params as Array<any> }),
                    catch: (cause) =>
                        new SqlError({
                            cause,
                            message: "Failed to execute statement",
            ...
            get beginTransaction() {
                return Effect.map(
                    Effect.tryPromise({
                        try: () =>
                            (this.sdk as Libsql.Client).transaction("write"),
                        catch: (cause) =>
                            new SqlError({
                                cause,
                ...
                : yield* Effect.map(
                      Effect.acquireRelease(
                          Effect.sync(() =>
                              Libsql.createClient({
                                  ...options,
                                  authToken: Redacted.isRedacted(
                                      options.authToken,
                                  )
                                      ? Redacted.value(options.authToken)
                                      : options.authToken,
                                  encryptionKey: Redacted.isRedacted(
