...
const seededClient = Effect.gen(function* (_) {
    ...
    yield* _(
        Effect.forEach(
            Array.range(1, 100),
            (id) =>
                sql`INSERT INTO test ${sql.insert({ id, name: `name${id}` })}`,
        ),
    );
    ...
    (it) => {
        it.layer(Layer.empty)("ordered", (it) => {
            ...
            it.scoped("result length mismatch", () =>
                Effect.gen(function* (_) {
                    ...
                    const sql = yield* _(seededClient);
                    const Select = yield* _(
                        SqlResolver.ordered("Select", {
        ...
        it.layer(Layer.empty)("grouped", (it) => {
            it.scoped("find by name", () =>
                Effect.gen(function* (_) {
                    const sql = yield* _(seededClient);
                    const FindByName = yield* _(
                        SqlResolver.grouped("FindByName", {
                            Request: Schema.String,
                    ...
                    yield* _(
                        sql`INSERT INTO test ${sql.insert({ name: "name1" })}`,
                    );
                    assert.deepStrictEqual(
                        yield* _(
                            Effect.all(
        ...
        it.layer(Layer.empty)("findById", (it) => {
            it.scoped("find by id", () =>
                Effect.gen(function* (_) {
                    const sql = yield* _(seededClient);
                    const FindById = yield* _(
                        SqlResolver.findById("FindById", {
                            Id: Schema.Number,
                            Result: Schema.Struct({
                                id: Schema.Number,
                                name: Schema.String,
