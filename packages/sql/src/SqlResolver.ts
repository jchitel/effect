 ...
 * @category requests
 */
export interface SqlRequest<T extends string, A, E>
    extends Request.Request<A, E | ParseError> {
    readonly _tag: T;
        ...
        function makeExecute(
            ...
            return function (input: I) {
                return Effect.useSpan(
                    ...
                    (span) =>
                        Effect.withFiberRuntime<A, E | ParseError, RI>(
                            (fiber) => {
                                ...
                                let toProvide: Context.Context<R> | undefined =
                                    context;
    ...
    const resolver = RequestResolver.makeBatched(
        (
            ...
            return options.execute(inputs as any).pipe(
                ...
                Effect.flatMap(
                    Effect.forEach(
                        (result, i) => Request.succeed(requests[i], result),
                        {
                            discard: true,
                        },
                    ),
            ...
            return options.execute(inputs as any).pipe(
                ...
                Effect.tap(({ rawResults, results }) => {
                    ...
                    return Effect.forEach(
                        requests,
                        (request) => {
                            const key = options.RequestGroupKey(
                                request.input as I,
                            );
            ...
            return options.execute(inputs as any).pipe(
                ...
                Effect.tap((_) => {
                    ...
                    return Effect.forEach(
                        idMap,
                        ([, request]) =>
                            Request.succeed(request, Option.none()),
                        { discard: true },
                    );
                }),
                Effect.catchAllCause((cause) =>
