        ...
        return Effect.useSpan(
            ...
            (span) =>
                withStatement(this, span, (statement) => {
                    ...
                    span.attribute(
                        OtelSemConv.ATTR_DB_OPERATION_NAME,
                        operation,
                    );
...
class CustomImpl<T extends string, A, B, C>
    implements Statement.Custom<T, A, B, C>
{
...
class CompilerImpl implements Statement.Compiler {
    constructor(
        ...
        readonly parameterPlaceholder: (
            index: number,
            value: unknown,
        ) => string,
        readonly onIdentifier: (
            value: string,
            ...
            switch (segment._tag) {
                ...
                case "RecordInsertHelper": {
                    ...
                    if (this.onInsert) {
                        ...
                        > = new Array(segment.value.length);
                        let placeholders = "";
                        for (let i = 0; i < segment.value.length; i++) {
                            const row: Array<Statement.Primitive> = new Array(
                                keys.length,
                            );
                            values[i] = row;
                            placeholders += i === 0 ? "(" : ",(";
                        ...
                        const [s, b] = this.onInsert(
                            keys.map((_) =>
                                this.onIdentifier(_, withoutTransform),
                            ),
                            placeholders,
                            values,
                ...
                case "RecordUpdateHelperSingle": {
                    ...
                    if (this.onRecordUpdateSingle) {
                        ...
                        for (let i = 0, len = keys.length; i < len; i++) {
                            const column = this.onIdentifier(
                                keys[i],
                                withoutTransform,
                            );
                            if (i === 0) {
                ...
                case "RecordUpdateHelper": {
                    const keys = Object.keys(segment.value[0]);
                    const values: Array<ReadonlyArray<Statement.Primitive>> =
                        new Array(segment.value.length);
                    let placeholders = "";
                    for (let i = 0; i < segment.value.length; i++) {
                        const row: Array<Statement.Primitive> = new Array(
