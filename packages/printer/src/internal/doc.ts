        ...
        switch (self._tag) {
            ...
            case "Cat": {
                ...
                if (
                    ...
                    return InternalFlatten.flattened(
                        cat(left.value, right.value),
                    );
                }
                if (
                    InternalFlatten.isFlattened(left) &&
    ...
    switch (self._tag) {
        ...
        case "Column": {
            return Effect.succeed(
                column((position) =>
                    Effect.runSync(
                        alterAnnotationsSafe(self.react(position), f),
                    ),
                ),
            );
        }
        case "WithPageWidth": {
            return Effect.succeed(
                pageWidth((pageWidth) =>
                    Effect.runSync(
                        alterAnnotationsSafe(self.react(pageWidth), f),
                    ),
                ),
            );
        }
...
export const string = (str: string): Doc.Doc<never> => {
    return cats(
        str
            .split("\n")
            .map((s) =>
                s.length === 0 ? empty : s.length === 1 ? char(s) : text(s),
            ),
    );
};
