            ...
            switch (self._tag) {
                ...
                case "Cons": {
                    switch (self.document._tag) {
                        ...
                        case "Annotated": {
                            const undo = InternalPipeline.undoAnnotation(
                                self.pipeline,
                            );
                            const next = InternalPipeline.cons(
                                self.indent,
...
const fitsSmartLoop = <A>(
    ...
    while (w >= 0) {
        switch (stream._tag) {
            ...
            case "LineStream": {
                if (!minNestingLevel) {
                    minNestingLevel = Option.match(
                        getInitialIndentation(comparator()),
                        {
