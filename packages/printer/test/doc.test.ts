...
describe.concurrent("Doc", () => {
    describe.concurrent("constructors", () => {
        ...
        it("union", () => {
            const doc = Doc.union(
                Doc.string("A long string of words"),
                Doc.char("b"),
            );
            expect(Doc.render(doc, { style: "pretty" })).toBe(
        ...
        it("nesting", () => {
            const prefix = Doc.hsep([
                Doc.text("prefix"),
                Doc.nesting((l) =>
                    Doc.squareBracketed(Doc.text(`Nested: ${l}`)),
                ),
            ]);
            const doc = Doc.vsep([0, 4, 8].map((n) => Doc.indent(prefix, n)));
            expect(Doc.render(doc, { style: "pretty" })).toBe(
    ...
    describe.concurrent("refinements", () => {
        ...
        it("isFlatAlt", () => {
            expect(
                Doc.isFlatAlt(Doc.flatAlt(Doc.char("a"), Doc.char("b"))),
            ).toBe(true);
        ...
        it("isUnion", () => {
            expect(Doc.isUnion(Doc.union(Doc.char("a"), Doc.char("b")))).toBe(
                true,
            );
        ...
        it("isWithPageWidth", () => {
            expect(
                Doc.isWithPageWidth(Doc.pageWidth(() => Doc.char("a"))),
            ).toBe(true);
    ...
    describe.concurrent("reactive/conditional combinators", () => {
        it("width", () => {
            const annotate = <A>(self: Doc.Doc<A>): Doc.Doc<A> =>
                Doc.width(Doc.squareBracketed(self), (w) =>
                    Doc.text(` <- width: ${w}`),
                );
