        ...
        const streamRequest = (request: typeof ConverseRequest.Encoded) =>
            client.converseStream(request).pipe(
                ...
                Stream.pipeThroughChannel(
                    EventStreamEncoding.makeChannel(ConverseStreamResponse),
                ),
            );
                ...
                return streamRequest(request).pipe(
                    Stream.filterMap((response) => {
                        ...
                        if (Predicate.isNotUndefined(response.metadata)) {
                            ...
                            if (
                                Predicate.isNotUndefined(
                                    response.metadata.performanceConfig,
                                )
                            ) {
                        ...
                        if (
                            Predicate.isNotUndefined(response.contentBlockStart)
                        ) {
                            const index =
                                response.contentBlockStart.contentBlockIndex;
                            const toolUse =
                                response.contentBlockStart.start.toolUse;
                            toolCalls[index] = {
                        ...
                        if (
                            Predicate.isNotUndefined(response.contentBlockDelta)
                        ) {
                            ...
                            if ("reasoningContent" in delta) {
                                ...
                                if ("signature" in delta.reasoningContent) {
                                    if (
                                        Predicate.isNotUndefined(reasoningText)
                                    ) {
                                        parts.push(
                        ...
                        if (
                            Predicate.isNotUndefined(response.contentBlockStop)
                        ) {
                            const index =
                                response.contentBlockStop.contentBlockIndex;
                            const tool = toolCalls[index];
                            if (Predicate.isNotUndefined(tool)) {
                                try {
                                ...
                                new AiResponse.FinishPart(
                                    {
                                        ...
                                        reason: InternalUtilities.resolveFinishReason(
                                            response.messageStop.stopReason,
                                        ),
                                        providerMetadata: {
    ...
    return {
        converse: ({ modelId, ...payload }) =>
            HttpClientRequest.make("POST")(`/model/${modelId}/converse`).pipe(
                (request) =>
                    Effect.orDie(HttpClientRequest.bodyJson(request, payload)),
