...
): AiModel.AiModel<AiLanguageModel.AiLanguageModel, AmazonBedrockClient> =>
...
export const make = Effect.fnUntraced(function* (options: {
    ...
    const makeToolConfig = (options: {
        ...
        const toolConfig: Mutable<typeof ToolConfiguration.Encoded> = {
            tools,
        } as any;
        if (isStructured) {
    ...
    const makeRequest = Effect.fnUntraced(function* (
        method: string,
        { prompt, system, ...rest }: AiLanguageModel.AiLanguageModelOptions,
    ) {
                    ...
                    for (let k = 0; k < message.parts.length; k++) {
                        ...
                        switch (part._tag) {
                            ...
                            case "RedactedReasoningPart": {
                                const redactedContent = Encoding.encodeBase64(
                                    part.redactedText,
                                );
                                content.push({
                                ...
                                switch (part._tag) {
                                    case "FilePart": {
                                        if (
                                            ...
                                            return yield* new AiError({
                                                ...
                                                method,
                                                description:
                                                    "No media type specified for image",
                                            });
                                        }
                                        content.push({
                                            document: {
                                    ...
                                    case "FileUrlPart": {
                                        ...
                                        return yield* new AiError({
                                            ...
                                            method,
                                            description:
                                                "File URLs are not supported at this time",
                                        });
                                    }
                                    case "ImagePart": {
                                        if (
                                    ...
                                    case "ImageUrlPart": {
                                        ...
                                        return yield* new AiError({
                                            ...
                                            method,
                                            description:
                                                "Image URLs are not supported at this time",
                                        });
                                    }
                                    case "TextPart": {
                                        content.push({
            ...
            if ("redactedContent" in content) {
                parts.push(
                    new AiResponse.RedactedReasoningPart(
                        {
                            redactedText: Encoding.encodeBase64(
...
const annotateChatResponse = (
    ...
    addGenAIAnnotations(span, {
        response: {
            model: modelId,
            finishReasons: response.stopReason
                ? [response.stopReason]
                : undefined,
        },
        usage: {
