...
describe("AiResponse", () => {
    ...
    describe("toolCalls getter", () => {
        ...
        it("should return all tool calls from response", () => {
            ...
            const textPart = AiResponse.TextPart.make({ text: "Hello" });
            const response = new AiResponse.AiResponse({
                parts: [textPart, toolCall1, toolCall2],
            });
...
describe("WithToolCallResults", () => {
    it("should create tool call results response", () => {
        ...
        const results = new Map([
            [
                AiResponse.ToolCallId.make("1"),
                { name: "test-tool", result: "success" },
            ],
        ]);
    ...
    it("should get tool call result by name", () => {
        const results = new Map([
            ...
            [
                AiResponse.ToolCallId.make("2"),
                { name: "other-tool", result: "other-success" },
            ],
        ]);
        const encodedResults = new Map([
            [
                AiResponse.ToolCallId.make("1"),
                { name: "test-tool", result: "encoded-success" },
            ],
            [
                AiResponse.ToolCallId.make("2"),
                { name: "other-tool", result: "other-encoded-success" },
            ],
        ]);
...
describe("Parts", () => {
    ...
    describe("RedactedReasoningPart", () => {
        it("should create redacted reasoning part", () => {
            const redactedPart = AiResponse.RedactedReasoningPart.make({
                ...
                Effect.gen(function* () {
                    ...
                    assert.isTrue(
                        result.description.includes(
                            "Failed to parse parameters from JSON",
                        ),
                    );
                }),
            );
...
describe("Guards", () => {
    ...
    describe("hasToolCallResults", () => {
        it("should return true for WithToolCallResults instances", () => {
            const response = new AiResponse.WithToolCallResults<
                typeof TestTool
            >({
                results: new Map(),
...
describe("Combination", () => {
    describe("merge", () => {
        ...
        it("should merge text parts correctly", () => {
            ...
            assert.strictEqual(
                (merged.parts[0] as AiResponse.TextPart).text,
                "Hello World",
            );
        });
        ...
        it("should merge WithToolCallResults responses", () => {
            ...
            const encoded1 = new Map([
                [
                    AiResponse.ToolCallId.make("1"),
                    { name: "tool1", result: "encoded1" },
                ],
            ]);
            const encoded2 = new Map([
                [
                    AiResponse.ToolCallId.make("2"),
                    { name: "tool2", result: "encoded2" },
                ],
            ]);

            const response1 = new AiResponse.WithToolCallResults<
                typeof TestTool1 | typeof TestTool2
            >({
                results: results1,
                encodedResults: encoded1,
                parts: [],
            });
            const response2 = new AiResponse.WithToolCallResults<
                typeof TestTool1 | typeof TestTool2
            >({
                results: results2,
            ...
            const mergedWithResults = merged as AiResponse.WithToolCallResults<
                typeof TestTool1 | typeof TestTool2
            >;
        ...
        it("should merge WithStructuredOutput responses", () => {
            ...
            const mergedStructured =
                merged as AiResponse.WithStructuredOutput<any>;
            assert.strictEqual(
                mergedStructured.id,
                AiResponse.ToolCallId.make("1"),
            );
    ...
    describe("FinishReason", () => {
        it("should validate all finish reasons", () => {
            ...
            reasons.forEach((reason) => {
                assert.doesNotThrow(() =>
                    Schema.decodeSync(AiResponse.FinishReason)(reason),
                );
            });
        });
    });
    ...
    describe("ToolCallId", () => {
        it("should create and validate tool call IDs", () => {
            ...
            const decoded = Schema.decodeSync(AiResponse.ToolCallId)(
                "another-id",
            );
