...
describe("AiInput", () => {
    describe("make", () => {
        ...
        it("should create valid input from an iterable of messages", () => {
            ...
            assert.deepStrictEqual(
                AiInput.make(messages),
                new AiInput.AiInput({ messages }),
            );
        });
        it("should create valid input from an empty response", () => {
            assert.deepStrictEqual(
                AiInput.make(AiResponse.empty),
                AiInput.empty,
            );
        });
...
const mockToolCallsResponse = new AiResponse.WithToolCallResults({
    ...
    results: new Map([
        [AiInput.ToolCallId.make("1"), { name: "tool", result: {} } as never],
    ]),
    encodedResults: new Map([
        [AiInput.ToolCallId.make("1"), { name: "tool", result: {} }],
    ]),
});
