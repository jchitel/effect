    ...
    export interface Service {
        ...
        readonly generateText: <
            ...
            Options extends NoExcessProperties<
                AiLanguageModel.GenerateTextOptions<any>,
                Options
            >,
        >(
            options: Options &
                Omit<AiLanguageModel.GenerateTextOptions<Tools>, "system">,
        ) => Effect.Effect<
...
export const fromPrompt = Effect.fnUntraced(function* (options: {
    ...
    const provideContext = <A, E, R>(
        ...
        Effect.mapInputContext(effect, (input) =>
            Context.merge(context, input),
        );
    const provideContextStream = <A, E, R>(
        stream: Stream.Stream<A, E, R>,
    ): Stream.Stream<A, E, R> =>
        Stream.mapInputContext(stream, (input) =>
            Context.merge(context, input),
        );
    const history = yield* Ref.make<AiInput.AiInput>(
        AiInput.make(options.prompt),
    );
    ...
    return AiChat.of({
        ...
        generateText(options) {
            ...
            return Ref.get(history).pipe(
                    ...
                    return languageModel
                        ...
                        .pipe(
                            Effect.tap((response) => {
                                ...
                                return Ref.set(
                                    history,
                                    AiInput.concat(input, modelInput),
                                );
                            }),
                            provideContext,
                        );
                ...
                Effect.withSpan("AiChat.generateText", {
 ...
 * @category Constructors
 */
export const empty: Effect.Effect<
    AiChat.Service,
    never,
...
export const fromExport = (
    ...
    Effect.flatMap(decodeUnknown(data), (prompt) =>
        fromPrompt({ ...options, prompt }),
    );
...
export const fromJson = (
    ...
    Effect.flatMap(decodeJson(data), (prompt) =>
        fromPrompt({ ...options, prompt }),
    );
