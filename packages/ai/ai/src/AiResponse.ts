...
export class AiResponse extends Schema.Class<AiResponse>(
    ...
    get finishReason(): FinishReason {
        const finishPart = this.parts.find(
 ...
 * @category Type Ids
 */
export const StructuredResponseTypeId = Symbol.for(
    "@effect/ai/AiResponse/StructuredResponse",
);
 ...
 * @category Type Ids
 */
export const WithToolCallResultsTypeId = Symbol.for(
    "@effect/ai/AiResponse/WithToolCallResults",
);
...
export class WithToolCallResults<Tools extends AiTool.Any> extends AiResponse {
    ...
    ): Option.Option<AiTool.Success<Tools>> {
...
> = Schema.Union(ContentSourceAnnotation, FileAnnotation, UrlAnnotation);
 ...
 * @category Guards
 */
export const is = (u: unknown): u is AiResponse =>
...
const mergeParts = (
    ...
    const flushText = () => {
        if (accumulatedText.length > 0) {
            result.push(
                new TextPart({ text: accumulatedText }, constDisableValidation),
            );
            accumulatedText = "";
        }
    };
...
>(2, (self, toolCalls) =>
    Effect.forEach(toolCalls, (toolCall) =>
        ToolCallPart.fromJson(toolCall),
    ).pipe(
        Effect.map(
            (parts) =>
                new AiResponse(
