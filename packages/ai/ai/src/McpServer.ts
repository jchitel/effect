...
>() {
    ...
    static readonly make = Effect.gen(function* () {
        ...
        >();
        const notificationsMailbox =
            yield* Mailbox.make<RpcMessage.Request<any>>();
            ...
            run: (f) =>
                protocol.run((clientId, request_) => {
                    ...
                    switch (request._tag) {
                        case "Request": {
                            if (
                                ClientNotificationRpcs.requests.has(request.tag)
                            ) {
                                if (request.tag === "notifications/cancelled") {
                                    return f(clientId, {
                                        _tag: "Interrupt",
                                        requestId: String(
                                            (request.payload as any).requestId,
                                        ),
                                    });
                                }
...
export const registerToolkit: <Tools extends AiTool.Any>(
    ...
    | Exclude<AiTool.Context<Tools>, McpServerClient>
    ...
    for (const tool of built.tools) {
        const mcpTool = new Tool({
            ...
            annotations: new ToolAnnotations({
                ...
                destructiveHint: Context.get(
                    tool.annotations,
                    AiTool.Destructive,
                ),
                idempotentHint: Context.get(
                    tool.annotations,
                    AiTool.Idempotent,
                ),
...
export const registerResource: {
    ...
    >(options: {
        ...
        readonly completion?:
            | ValidateCompletions<
                  Completions,
                  keyof ResourceCompletions<Schemas>
              >
            | undefined;
        readonly content: (
            uri: string,
        ...
        | Exclude<
              | R
              | (Completions[keyof Completions] extends (
                    input: string,
                ) => infer Ret
        ...
        return Effect.gen(function* () {
            ...
            yield* registry.addResource(
                ...
                options.content.pipe(
                    ...
                    Effect.map((content) =>
                        resolveResourceContent(options.uri, content),
                    ),
    ...
    return Effect.fnUntraced(function* <E, R>(options: {
        ...
        for (const [param, handle] of Object.entries(
            ...
            const encodeArray = Schema.encodeUnknown(
        ...
        yield* registry.addResourceTemplate({
            ...
            handle: (uri, params) =>
                decode(params).pipe(
                    Effect.mapError(
                        (error) =>
                            new InvalidParams({ message: error.message }),
                    ),
                    Effect.flatMap((params) =>
                        options.content(uri, ...params).pipe(
                            Effect.map((content) =>
                                resolveResourceContent(uri, content),
                            ),
                            Effect.catchAllCause((cause) => {
                                const prettyError =
    ...
    for (const prop of propSignatures) {
        args.push({
            ...
            description: Option.getOrUndefined(
                AST.getDescriptionAnnotation(prop),
            ),
...
const compileUriTemplate = (
    ...
    if (schemas.length > 0) {
        ...
        for (let i = 0; i < schemas.length; i++) {
            ...
            const paramName = AST.getAnnotation(ParamAnnotation)(
                schema.ast,
            ).pipe(Option.getOrElse(() => `param${key}`));
        ...
        Effect.gen(function* () {
            ...
            return {
                ...
                "prompts/list": () =>
                    Effect.sync(
                        () =>
                            new ListPromptsResult({ prompts: server.prompts }),
                    ),
                "resources/list": () =>
                    Effect.sync(
                        () =>
                ...
                "tools/list": () =>
                    Effect.sync(
                        () => new ListToolsResult({ tools: server.tools }),
                    ),
