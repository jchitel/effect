 ...
 * @category Utility Types
 */
export type Tools<Toolkit> =
    Toolkit extends AiToolkit<infer Tool>
        ? string extends Tool["name"]
            ? never
            : Tool
        : never;
...
const Proto = {
    ...
    toContext(
        this: AiToolkit<any>,
        build: Effect.Effect<Record<string, (params: any) => any>>,
    ) {
        ...
        return Effect.gen(this, function* () {
            ...
            const schemasCache = new WeakMap<
                any,
                {
                    ...
                    readonly encodeSuccess: (
                        u: unknown,
                    ) => Effect.Effect<unknown, ParseError>;
                    readonly decodeFailure: (
                        u: unknown,
                    ) => Effect.Effect<AiTool.Failure<any>, ParseError>;
                    readonly decodeParameters: (
                        u: unknown,
                    ) => Effect.Effect<AiTool.Parameters<any>, ParseError>;
                }
            >();
            const getSchemas = (tool: AiTool.AnyWithProtocol) => {
                let schemas = schemasCache.get(tool);
                if (Predicate.isUndefined(schemas)) {
                    const handler = context.unsafeMap.get(
                        tool.key,
                    )! as AiTool.Handler<any>;
                    const encodeSuccess = Schema.encodeUnknown(
                        tool.successSchema,
                    ) as any;
                    const decodeFailure = Schema.decodeUnknown(
                        tool.failureSchema as any,
                    ) as any;
...
const makeProto = <Tools extends AiTool.Any>(
    tools: Record<string, Tools>,
): AiToolkit<Tools> => Object.assign(function () {}, Proto, { tools }) as any;
...
const resolveInput = <Tools extends ReadonlyArray<AiTool.Any>>(
    ...
    for (const tool of tools) {
        const value = (
            Schema.isSchema(tool) ? AiTool.fromTaggedRequest(tool as any) : tool
        ) as any;
