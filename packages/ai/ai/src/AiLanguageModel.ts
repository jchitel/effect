 ...
 * @category Models
 */
export type ToolChoice<Tool extends AiTool.Any> =
    | "auto"
    | "none"
    | "required"
    | {
...
const DefaultToolCallIdGenerator: ToolCallIdGenerator = {
    generateId: Effect.fnUntraced(function* () {
        ...
        for (let i = 0; i < 32; i++) {
            chars[i] =
                ALPHANUMS[
                    yield* Random.nextIntBetween(0, ALPHANUMS.length - 1)
                ];
        }
 ...
 * @category Context
 */
export class CurrentToolCallIdGenerator extends _Context.Tag(
    "@effect/ai/CurrentToolCallIdGenerator",
)<CurrentToolCallIdGenerator, ToolCallIdGenerator>() {}
...
export interface GenerateTextOptions<Tools extends AiTool.Any> {
    ...
    readonly toolkit?:
        ...
        | Effect.Effect<AiToolkit.ToHandler<Tools>, any, any>;
...
export type ExtractSuccess<Options> = Options extends {
    ...
    : Options extends {
      ...
      : Options extends {
              toolkit: Effect.Effect<
                  AiToolkit.ToHandler<infer _Tools>,
                  infer _E,
                  infer _R
              >;
          }
...
}) => Effect.Effect<AiLanguageModel.Service> = Effect.fnUntraced(
    function* (opts: {
        readonly generateText: (
            options: AiLanguageModelOptions,
        ) => Effect.Effect<AiResponse.AiResponse, AiError, Context>;
        ...
        const toolCallIdGenerator = yield* Effect.serviceOption(
            CurrentToolCallIdGenerator,
        ).pipe(Effect.map(Option.getOrElse(() => DefaultToolCallIdGenerator)));
            ...
            Effect.useSpan(
                ...
                Effect.fnUntraced(
                    function* (span) {
                        ...
                        const modelOptions: Mutable<AiLanguageModelOptions> = {
                            prompt,
                            system,
                            tools: [],
                            toolChoice: "none",
                        ...
                        if (Predicate.isUndefined(toolkit)) {
                            const response =
                                yield* opts.generateText(modelOptions);
        ...
        const streamText = Effect.fnUntraced(
            ...
            Effect.flatMap(
                Effect.fnUntraced(function* ([stream, options]) {
                    ...
                    return stream.pipe(
                        Stream.map((response) => {
                            finalResponse = AiResponse.merge(
                                finalResponse,
                                response,
                            );
                            return response;
...
const resolveParts = Effect.fnUntraced(function* <
    Tools extends AiTool.Any,
>(options: {
...
> =>
    Stream.unwrap(
        AiLanguageModel.pipe(Effect.map((_) => _.streamText(options))),
    );
