...
export class Config extends Context.Tag(
    "@effect/ai-google/GoogleAiLanguageModel/Config",
 ...
 * @category Context
 */
export class ProviderMetadata extends Context.Tag(
    InternalUtilities.ProviderMetadataKey,
)<ProviderMetadata, ProviderMetadata.Service>() {}
...
export declare namespace ProviderMetadata {
    ...
    export interface Service {
         ...
         */
        readonly citationSources?:
            | ReadonlyArray<typeof Generated.CitationSource.Encoded>
            | undefined;
        /**
         * Attribution information for sources that contributed to a grounded answer.
         */
        readonly groundingAttributions?:
            | ReadonlyArray<typeof Generated.GroundingAttribution.Encoded>
            | undefined;
        /**
         * Grounding metadata for the candidate.
         */
        readonly groundingMetadata?:
            | ReadonlyArray<typeof Generated.GroundingMetadata.Encoded>
            | undefined;
        /**
         * The URLs that were retrieved by the URL context retrieval tool.
         */
        readonly retrievedUrls?:
            | ReadonlyArray<typeof Generated.UrlMetadata.Encoded>
            | undefined;
        /**
        ...
        readonly safetyRatings?:
            | ReadonlyArray<typeof Generated.SafetyRating.Encoded>
            | undefined;
    }
}
...
export const model = (
    ...
    AiModel.make(layer({ model, config }));
...
export const make = Effect.fnUntraced(function* (options: {
    ...
    const makeRequest = Effect.fnUntraced(function* (
        method: string,
        {
            ...
            toolChoice,
            tools,
        }: AiLanguageModel.AiLanguageModelOptions,
    ) {
        const context = yield* Effect.context<never>();
        const perRequestConfig = context.unsafeMap.get(Config.key) as
            | Config.Service
            | undefined;
        const useStructured = tools.length === 1 && tools[0].structured;
        const responseMimeType = useStructured
            ? "application/json"
            : (options.config?.generationConfig?.responseMimeType ??
              perRequestConfig?.generationConfig?.responseMimeType);
        let toolConfig: typeof Generated.ToolConfig.Encoded | undefined =
            options.config?.toolConfig;
        if (
                    ...
                    ? [
                          {
                              functionDeclarations: tools.map((tool) => ({
                                  ...
                                  parameters:
                                      InternalUtilities.jsonSchemaToOpenApi(
                                          tool.parameters,
                                      ),
                              })),
                          },
    ...
    return yield* AiLanguageModel.make({
        generateText: Effect.fnUntraced(
            function* (_options) {
                ...
                const rawResponse = yield* client.client.GenerateContent(
                    request.model,
                    request,
                );
            ...
            case "UserMessage": {
                ...
                for (const part of message.parts) {
                    switch (part._tag) {
                        ...
                        case "FileUrlPart": {
                            parts.push({
                                fileData: { fileUri: part.url.toString() },
                            });
                            break;
                        }
                    }
...
const makeResponse = Effect.fnUntraced(function* (
    ...
    if (
        ...
        metadata.citationSources = Predicate.isUndefined(
            metadata.citationSources,
        )
            ? candidate.citationMetadata.citationSources
            : metadata.citationSources.concat(
                  candidate.citationMetadata.citationSources,
              );
    }
    if (Predicate.isNotUndefined(candidate.groundingMetadata)) {
        metadata.groundingMetadata = Predicate.isUndefined(
    ...
    if (Predicate.isNotUndefined(candidate.groundingAttributions)) {
        metadata.groundingAttributions = Predicate.isUndefined(
            metadata.groundingAttributions,
        )
            ? candidate.groundingAttributions
            : metadata.groundingAttributions.concat(
                  candidate.groundingAttributions,
              );
    }
    ...
    if (Predicate.isNotUndefined(candidate.content)) {
        ...
        for (const part of contentParts) {
            ...
            if (
                ...
                Predicate.isNotUndefined(part.functionCall)
            ) {
                hasToolCalls = true;
    ...
    if (Predicate.isNotUndefined(candidate.finishReason)) {
        ...
        const reason =
            candidate.finishReason === "STOP" && hasToolCalls
                ? "tool-calls"
...
const annotateStreamResponse = (
    ...
    const metadataPart = response.parts.find(
        (part) => part._tag === "MetadataPart",
    );
    const finishPart = response.parts.find(
        (part) => part._tag === "FinishPart",
    );
    addGenAIAnnotations(span, {
        response: {
            id: metadataPart?.id,
