...
export declare namespace GoogleAiClient {
    ...
    export interface Service {
        ...
        readonly streamRequest: (
            ...
            typeof Generated.GenerateContentResponse.Type,
        ...
        const client = Generated.make(httpClient, {
            transformClient: (client) =>
                GoogleAiConfig.getOrUndefined.pipe(
                    Effect.map((config) =>
                        config?.transformClient
                            ? config.transformClient(client)
                            : client,
                    ),
                ),
        });

        const decodeStreamResponse = Schema.decodeUnknown(
            Schema.ChunkFromSelf(Generated.GenerateContentResponse),
        );
        const streamRequest = (
            request: typeof Generated.GenerateContentRequest.Encoded,
        ) => {
            ...
            return Stream.unwrap(
                GoogleAiConfig.getOrUndefined.pipe(
                    Effect.map((config) => {
                        const client = Predicate.isNotUndefined(
                            config?.transformClient,
                        )
                    ...
                    Stream.filterMap((response) => {
                        ...
                        if (
                            ...
                            metadata.groundingMetadata = Predicate.isUndefined(
                                ...
                                ? [candidate.groundingMetadata]
                                : metadata.groundingMetadata.concat(
                                      candidate.groundingMetadata,
                                  );
                        }
                        if (
                        ...
                        if (Predicate.isNotUndefined(candidate.safetyRatings)) {
                            metadata.safetyRatings = Predicate.isUndefined(
                                metadata.safetyRatings,
                            )
                                ? candidate.safetyRatings
                                : metadata.safetyRatings.concat(
                                      candidate.safetyRatings,
                                  );
                        }
                        if (
                            ...
                            metadata.retrievedUrls = Predicate.isUndefined(
                                metadata.retrievedUrls,
                            )
                                ? candidate.urlContextMetadata.urlMetadata
