...
export const make = Effect.fnUntraced(function* (options: {
    ...
    const makeRequest = Effect.fnUntraced(function* (
        ...
        let tool_choice:
            | typeof Generated.ChatCompletionToolChoiceOption.Encoded
            | undefined = undefined;
        if (
    ...
    return yield* AiLanguageModel.make({
        generateText: Effect.fnUntraced(
            function* (options) {
                ...
                const rawResponse =
                    yield* client.client.createChatCompletion(request);
...
const makeMessages = Effect.fnUntraced(function* (
    ...
    type Messages = Array<
        typeof Generated.ChatCompletionRequestMessage.Encoded
    >;
    type UserPart =
        ...
        switch (message._tag) {
            case "AssistantMessage": {
                ...
                const toolCalls: Array<
                    typeof Generated.ChatCompletionMessageToolCall.Encoded
                > = [];
                    ...
                    switch (part._tag) {
                        ...
                        case "FileUrlPart": {
                            return yield* new AiError({
                                ...
                                method,
                                description:
                                    "OpenAi does not support file content parts with URL data",
                            });
                        }
                        case "TextPart": {
                        ...
                        case "ImageUrlPart": {
                            ...
                            content.push({
                                type: "image_url",
                                image_url: { url: part.url.toString() },
                            });
                        }
                    }
                }
...
const makeResponse = Effect.fnUntraced(function* (
    ...
    if (
        ...
        metadata.acceptedPredictionTokens =
            response.usage?.completion_tokens_details
    ...
    if (
        ...
        metadata.rejectedPredictionTokens =
            response.usage?.completion_tokens_details
...
const annotateChatResponse = (
    ...
    addGenAIAnnotations(span, {
        response: {
            id: response.id,
            model: response.model,
            finishReasons: response.choices.map(
...
const annotateStreamResponse = (
    ...
    addGenAIAnnotations(span, {
        ...
        openai: {
            response: {
                serviceTier: providerMetadata?.serviceTier as
                    | string
                    | undefined,
                systemFingerprint: providerMetadata?.systemFingerprint as
                    | string
                    | undefined,
            },
        },
