...
export const make = (options: {
    ...
    Effect.gen(function* () {
        ...
        const stream = (request: StreamCompletionRequest) =>
            Stream.suspend(() => {
                const toolCalls = {} as Record<
                    number,
                    RawToolCall & { isFinished: boolean }
                >;
                ...
                return streamRequest<RawCompletionChunk>(
                        ...
                        for (let i = 0; i < chunk.choices.length; i++) {
                            ...
                            if (Predicate.isNotNullable(choice.finish_reason)) {
                                ...
                                if (
                                    ...
                                    Predicate.isNotUndefined(toolCallIndex)
                                ) {
                                    finishToolCall(
                            ...
                            if (
                                ...
                                for (const delta of choice.delta.tool_calls) {
                                    ...
                                    if (Predicate.isUndefined(toolCallIndex)) {
                                        const toolCall =
                                            delta as unknown as RawToolCall;
