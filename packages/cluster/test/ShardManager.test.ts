...
const runner1 = RunnerWithMetadata({
    runner: Runner.make({
        address: RunnerAddress.make("1", 1),
        groups: ["default"],
        version: 1,
    }),
...
const runner2 = RunnerWithMetadata({
    runner: Runner.make({
        address: RunnerAddress.make("2", 2),
...
const runner3 = RunnerWithMetadata({
    runner: Runner.make({
        address: RunnerAddress.make("3", 3),
        groups: ["default", "custom"],
        version: 1,
    }),
    registeredAt: Number.MIN_SAFE_INTEGER,
...
describe("ShardManager", () => {
    describe("Rebalancing", () => {
        it("should rebalance unbalanced assignments", () => {
            ...
            const [assignments, unassignments] = decideAssignmentsForShards(
                state,
                "default",
            );
            expect(
                MutableHashMap.has(assignments, runner2.runner.address),
            ).toBe(true);
            expect(MutableHashMap.size(assignments)).toBe(1);
            expect(
                MutableHashMap.has(unassignments, runner1.runner.address),
            ).toBe(true);
        ...
        it("should not rebalance to runners with an older version", () => {
            const oldRunner2 = RunnerWithMetadata({
                runner: Runner.make({
                    address: runner2.runner.address,
                    groups: ["default"],
                    version: 0,
                }),
        ...
        it("should pick the runner with less shards", () => {
            ...
            expect(
                MutableHashMap.has(assignments, runner3.runner.address),
            ).toBe(true);
            expect(MutableHashMap.size(assignments)).toBe(1);
            expect(
                MutableHashMap.has(unassignments, runner1.runner.address),
        ...
        it("should not rebalance if there are no runners", () => {
            const state = makeDefaultState(
                ...
                new Map([[1, Option.some(runner1.runner.address)]]),
            );
            const [assignments, unassignments] = decideAssignmentsForShards(
                state,
                "default",
    ...
    describe.concurrent("Simulation", () => {
        ...
        const TestLive = ShardManagerLive.pipe(
            Layer.provideMerge(
                Layer.mergeAll(ShardStorage.layerNoop, RunnerHealthLive),
            ),
            Layer.provide([
        ...
        it.effect(
            ...
            () =>
                Effect.gen(function* () {
                    ...
                    const allRunnersAssigned = Array.every(
                        values,
                        Option.isSome,
                    );
                    expect(allRunnersAssigned).toBe(true);
                    let shardsPerRunner = getShardsPerRunner(
                        assignments,
                        "default",
                    );
                    expect(
                        shardsPerRunner.every((shards) => shards.length === 15),
                    ).toBe(true);
                    shardsPerRunner = getShardsPerRunner(assignments, "custom");
                    expect(
                        shardsPerRunner.every((shards) => shards.length === 30),
                    ).toBe(true);
                    ...
                    const assignments2 = yield* manager.getAssignments.pipe(
                        Effect.map(
                            Iterable.filter(
                                ([shardId, address]) =>
                                    shardId.group === "default" &&
                                    Option.isSome(address) &&
                                    address.value.port > 20,
                            ),
                        ),
                    );
                    const shardsPerRunner2 = getShardsPerRunner(
                        assignments2,
                        "default",
                    );
                    expect(
                        shardsPerRunner2.every(
                            (shards) => shards.length === 12,
                        ),
                    ).toBe(true);
        ...
        it.effect(
            "should succcessfully scale down",
            () =>
                Effect.gen(function* () {
                    const manager = yield* ShardManager.ShardManager;
                    ...
                    expect(
                        shardsPerRunner.every((shards) => shards.length === 12),
                    ).toBe(true);
                    shardsPerRunner = getShardsPerRunner(assignments, "custom");
                    expect(
                        shardsPerRunner.every((shards) => shards.length === 25),
                    ).toBe(true);
        ...
        it.effect(
            "should save state to storage when restarted",
            () =>
                Effect.gen(function* () {
                    const setup = Effect.gen(function* () {
                        ...
                        yield* Effect.iterate(
                            Array.empty<
                                [
                                    ShardId.ShardId,
                                    Option.Option<RunnerAddress.RunnerAddress>,
                                ]
                            >(),
                            {
                        ...
                        yield* Effect.iterate(
                            Array.empty<
                                [RunnerAddress.RunnerAddress, Runner.Runner]
                            >(),
                            {
                                while: Array.isEmptyArray,
                    ...
                    yield* setup;
                    yield* test;
                }).pipe(Effect.provide(ShardStorage.layerMemory)),
            10_000,
        );
    });
});
...
function getShardsPerRunner(
    assignments: Iterable<
        readonly [ShardId.ShardId, Option.Option<RunnerAddress.RunnerAddress>]
    >,
    group: string,
) {
    const shardsPerRunner = MutableHashMap.empty<
...
const simulate = Effect.fnUntraced(function* (
    ...
    const fiber = yield* Effect.forEach(events, handleEvent, {
...
const makeDefaultState = (
    runners: MutableHashMap.MutableHashMap<
        RunnerAddress.RunnerAddress,
        RunnerWithMetadata
    >,
