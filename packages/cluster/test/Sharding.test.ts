...
describe.concurrent("Sharding", () => {
    ...
    it.scoped("durable messages are retried when mailbox is full", () =>
        Effect.gen(function* () {
            const requestedIds =
                yield* Mailbox.make<Array<Snowflake.Snowflake>>();
            yield* Effect.gen(function* () {
                const state = yield* TestEntityState;
                const makeClient = yield* TestEntity.client;
                yield* TestClock.adjust(1);
                        ...
                        Layer.updateService(
                            ...
                            (storage) => ({
                                ...
                                unprocessedMessagesById(messageIds) {
                                    requestedIds.unsafeOffer(
                                        Array.fromIterable(messageIds),
                                    );
                                    return storage.unprocessedMessagesById(
                                        messageIds,
                                    );
                                },
                            }),
        ...
        Effect.gen(function* () {
            ...
            yield* Effect.gen(function* () {
                ...
                Effect.provide(
                    TestShardingWithoutStorage.pipe(
                        Layer.provideMerge(
                            Layer.service(MessageStorage.MemoryDriver),
                        ),
                        Layer.updateService(
                            MessageStorage.MessageStorage,
                            (storage) => ({
    ...
    it.scoped("client discard option", () =>
        Effect.gen(function* () {
            ...
            const makeClient = yield* TestEntity.client;
            const client = makeClient("1");
            const result = yield* client.GetUser(
                { id: 123 },
                { discard: true },
            );
            expect(result).toEqual(void 0);
    ...
    it.scoped("EntityNotManagedByRunner", () =>
        Effect.gen(function* () {
            ...
            const makeClient = yield* TestEntity.client;
            const client = makeClient("1");
            const error = yield* client.GetUser({ id: 123 }).pipe(Effect.flip);
