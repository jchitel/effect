...
export const make = Effect.fnUntraced(function* (options?: {
    ...
    return ShardStorage.makeEncoded({
        getAssignments:
            sql`SELECT shard_id, address FROM ${shardsTableSql} ORDER BY shard_id`.values.pipe(
        ...
        saveAssignments: (assignments) => {
            ...
            const values = assignments.map(
                ([shardId, address]) => sql`(${shardId}, ${address})`,
            );
            return remove.pipe(
                Effect.andThen(
                    sql`INSERT INTO ${shardsTableSql} (shard_id, address) VALUES ${sql.csv(values)}`
                        .unprepared,
                ),
                sql.withTransaction,
                PersistenceError.refail,
        ...
        getRunners:
            sql`SELECT address, runner FROM ${runnersTableSql}`.values.pipe(
                PersistenceError.refail,
                Effect.map(
                    Arr.map(
                        ([address, runner]) =>
                            [String(address), String(runner)] as const,
                    ),
                ),
                withTracerDisabled,
            ),
        ...
        saveRunners: (runners) => {
            ...
            const values = runners.map(
                ([address, runner]) => sql`(${address}, ${runner})`,
            );
            const insert =
                sql`INSERT INTO ${runnersTableSql} (address, runner) VALUES ${sql.csv(values)}`
                    .unprepared;
            return remove.pipe(
        ...
        acquire: Effect.fnUntraced(
            function* (address, shardIds) {
                const values = shardIds.map(
                    (shardId) => sql`(${shardId}, ${address}, ${sqlNow})`,
                );
        ...
        refresh: (address, shardIds) =>
            sql`UPDATE ${locksTableSql} SET acquired_at = ${sqlNow} WHERE address = ${address} AND ${sql.in(
                "shard_id",
                shardIds,
            )}`.pipe(
                Effect.andThen(
