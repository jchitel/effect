...
export const make = Effect.gen(function* () {
    ...
    const sendResumeParent = Effect.fnUntraced(function* (options: {
        ...
        const client = (yield* RcMap.get(clients, options.workflowName))(
            options.executionId,
        );
            ...
            return Effect.suspend(() => {
                if (entities.has(workflow.name)) {
                    return Effect.dieMessage(
                        `Workflow ${workflow.name} already registered`,
                    );
                }
                return sharding.registerEntity(
                    ...
                    Effect.gen(function* () {
                        ...
                        return {
                                ...
                                return execute(payload, executionId).pipe(
                                    Effect.ensuring(
                                        Effect.suspend(() => {
                                            ...
                                            return engine
                                                .deferredResult(InterruptSignal)
                                                .pipe(
                                                    Effect.flatMap(
                                                        (maybeResult) => {
                                                            if (
                            ...
                            activity: Effect.fnUntraced(function* (
                                ...
                                const contextMap = new Map(
                                    entry.runtime.context.unsafeMap,
                                );
                                contextMap.set(
        ...
        deferredResult: (deferred) =>
            WorkflowInstance.pipe(
                ...
                Effect.map(
                    Option.map((reply) =>
                        reply.exit._tag === "Success"
                            ? (reply.exit.value as any as Schema.ExitEncoded<
                                  unknown,
                                  unknown,
                                  unknown
                              >)
                    ...
                    client.run(
                        {
                            ...
                            wakeUp: DateTime.addDuration(
                                now,
                                options.clock.duration,
                            ),
                        },
                        { discard: true },
                    ),
...
const ExitUnknown = Schema.encodedSchema(
    Schema.Exit({
        success: Schema.Unknown,
