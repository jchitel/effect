                ...
                Duplicate: ({ lastReceivedReply, originalId }) => {
                    ...
                    if (
                        ...
                        return message.respond(
                            lastReceivedReply.value.withRequestId(
                                message.envelope.requestId,
                            ),
                        );
                    }
                    requestIdRewrites.set(
                        message.envelope.requestId,
                        originalId,
                    );
                    return afterPersist(
        ...
        sendLocal(options) {
            ...
            return Message.serialize(message).pipe(
                Effect.flatMap((encoded) =>
                    Message.deserializeLocal(message, encoded),
                ),
            ...
            return notifyWith(options.message, (message, duplicate) => {
                if (options.discard || message._tag === "OutgoingEnvelope") {
                    return Effect.catchTag(
                        options.notify(
                            Message.incomingLocalFromOutgoing(message),
                        ),
                        "EntityNotAssignedToRunner",
                        () => Effect.void,
                    );
...
> = make({
    send: ({ message }) =>
        Effect.fail(
            new EntityNotManagedByRunner({ address: message.envelope.address }),
        ),
    notify: () => Effect.void,
    ping: () => Effect.void,
});
                    ...
                    onSuccess: (request) =>
                        RcMap.get(clients, address).pipe(
                            ...
                            Effect.flatMap((reply) =>
                                Schema.decode(Reply.Reply(message.rpc))(
                                    reply,
                                ).pipe(
                                    Effect.locally(
                    ...
                    RcMap.get(clients, address).pipe(
                        ...
                        Effect.flatMap((mailbox) => {
                            const decode = Schema.decode(
                                Reply.Reply(message.rpc),
                            );
                            return mailbox.take.pipe(
                                Effect.flatMap((reply) =>
                                    Effect.orDie(decode(reply)),
                                ),
                                Effect.flatMap(message.respond),
                                Effect.forever,
                                Effect.catchTag("RpcClientError", Effect.die),
