 ...
 * @category models
 */
export class ShardStorage extends Context.Tag("@effect/cluster/ShardStorage")<
    ShardStorage,
    {
        /**
        ...
        readonly saveAssignments: (
            assignments: Iterable<
                readonly [ShardId, Option.Option<RunnerAddress>]
            >,
        ...
        readonly saveRunners: (
            runners: Iterable<readonly [RunnerAddress, Runner]>,
        ) => Effect.Effect<void, PersistenceError>;
...
export interface Encoded {
     ...
     */
    readonly getAssignments: Effect.Effect<
        Array<readonly [shardId: string, runnerAddress: string | null]>,
        PersistenceError
    >;
    ...
    readonly saveAssignments: (
        assignments: Array<
            readonly [shardId: string, RunnerAddress: string | null]
        >,
    ) => Effect.Effect<void, PersistenceError>;
     ...
     */
    readonly getRunners: Effect.Effect<
        Array<readonly [address: string, runner: string]>,
        PersistenceError
    >;
    ...
    ShardStorage.of({
        getAssignments: Effect.map(encoded.getAssignments, (assignments) => {
            ...
            for (const [shardId, runnerAddress] of assignments) {
                arr.push([
                    ...
                    runnerAddress === null
                        ? Option.none()
                        : Option.some(decodeRunnerAddress(runnerAddress)),
                ]);
            }
            return arr;
        }),
        saveAssignments: (assignments) => {
            ...
            for (const [shardId, runnerAddress] of assignments) {
                arr.push([
                    shardId.toString(),
                    Option.isNone(runnerAddress)
                        ? null
                        : encodeRunnerAddress(runnerAddress.value),
                ]);
            }
        ...
        getRunners: Effect.gen(function* () {
            ...
            for (let i = 0; i < runners.length; i++) {
                ...
                try {
                    results.push([
                        decodeRunnerAddress(address),
                        Runner.decodeSync(runner),
                    ]);
                } catch {
                    //
                }
        ...
        saveRunners: (runners) =>
            Effect.suspend(() =>
                encoded.saveRunners(
                    Array.from(runners, ([address, runner]) => [
        ...
        refresh: (address, shardIds) => {
            const arr = Array.from(shardIds, (id) => id.toString());
            return encoded
                .refresh(encodeRunnerAddress(address), arr)
                .pipe(Effect.map((shards) => shards.map(ShardId.fromString)));
        },
...
export const makeMemory = Effect.gen(function* () {
    ...
    function saveAssignments(
        value: Iterable<readonly [ShardId, Option.Option<RunnerAddress>]>,
    ) {
        return Effect.sync(() => {
 ...
 * @category layers
 */
export const layerMemory: Layer.Layer<ShardStorage> = Layer.effect(
    ShardStorage,
    makeMemory,
);
