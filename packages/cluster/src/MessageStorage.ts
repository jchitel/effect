...
export class MessageStorage extends Context.Tag(
    ...
    MessageStorage,
    {
        ...
        readonly requestIdForPrimaryKey: (options: {
            ...
            Option.Option<Snowflake.Snowflake>,
            PersistenceError
        >;
    ...
    Effect.fnUntraced(function* (encoded: Encoded) {
        ...
        const storage: MessageStorage["Type"] = yield* make({
            ...
            repliesForUnfiltered: (ids) =>
                encoded.repliesForUnfiltered(Array.from(ids, String)),
        ...
        effect: Effect.gen(function* () {
            ...
            const unprocessedWith = (
                ...
                const messages: Array<{
                    readonly envelope: Envelope.Envelope.Encoded;
                    readonly lastSentReply: Option.Option<
                    ...
                    Effect.sync(() => {
                        ...
                        if (existing) {
                            return SaveResultEncoded.Duplicate({
                                originalId: Snowflake.Snowflake(
                ...
                requestIdForPrimaryKey: (primaryKey) =>
                    Effect.sync(() => {
                        ...
                        return Option.fromNullable(
                            entry?.envelope.requestId,
                        ).pipe(Option.map(Snowflake.Snowflake));
                    }),
                repliesFor: (requestIds) =>
                    Effect.sync(() => repliesFor(requestIds)),
                repliesForUnfiltered: (requestIds) =>
                    Effect.sync(() =>
                        requestIds.flatMap(
                            (id) => requests.get(String(id))?.replies ?? [],
                        ),
                    ),
                unprocessedMessages: (shardIds) =>
                    Effect.sync(() => {
