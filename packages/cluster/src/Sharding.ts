...
const make = Effect.gen(function* () {
    ...
    if (Option.isSome(config.runnerAddress)) {
        ...
        yield* Effect.gen(function* () {
            while (true) {
                ...
                const acquired = yield* shardStorage.acquire(
                    selfAddress,
                    unacquiredShards,
                );
                ...
                Effect.forEach(
                    ...
                    (shardId) =>
                        Effect.forEach(
                            ...
                            Effect.andThen(
                                shardStorage.release(selfAddress, shardId),
                            ),
                            Effect.annotateLogs({
                                runner: selfAddress,
                            }),
        ...
        Effect.fnUntraced(function* (name, run, options) {
            ...
            if (MutableHashMap.has(map, address)) {
                return yield* Effect.dieMessage(
                    `Singleton '${name}' is already registered`,
                );
            }
        ...
        Effect.gen(function* () {
            for (const [shardId, map] of singletons) {
                for (const [address, run] of map) {
                    const running = FiberMap.unsafeHas(
                        singletonFibers,
                        address,
                    );
                    const shouldBeRunning = MutableHashSet.has(
                        acquiredShards,
                        shardId,
                    );
    ...
    let storageAlreadyProcessed = (_message: Message.IncomingRequest<any>) =>
        true;
    ...
    if (storageEnabled && Option.isSome(config.runnerAddress)) {
        ...
        yield* Effect.gen(function* () {
            ...
            storageAlreadyProcessed = (
                message: Message.IncomingRequest<any>,
            ) => {
                if (!sentRequestIds.has(message.envelope.requestId)) {
                    return false;
                }
                const state = entityManagers.get(
                    message.envelope.address.entityType,
                );
            ...
            while (true) {
                ...
                const messages =
                    yield* storage.unprocessedMessages(acquiredShards);
                ...
                const send = Effect.catchAllCause(
                    Effect.suspend(() => {
                        ...
                        const isProcessing =
                            state.manager.isProcessingFor(message);
        ...
        const resumeEntityFromStorageImpl = Effect.fnUntraced(
            function* (address: EntityAddress) {
                ...
                while (!done) {
                    ...
                    const messages =
                        yield* storage.unprocessedMessagesById(messageIds);
                    ...
                    > = Effect.catchTags(
                        Effect.suspend(() => {
                            ...
                            const interrupt =
                                ...
                                resumptionState.interrupts.get(
                                    message.envelope.requestId,
                                );
                            return interrupt
                                ? Effect.flatMap(
            ...
            Effect.catchAllCause((cause) =>
                Effect.logError("Could not resume unprocessed messages", cause),
            ),
            (effect, address) =>
            ...
            return message._tag === "IncomingRequest" ||
                ...
                : (runners.sendLocal({
                      ...
                      simulateRemoteSerialization:
                          config.simulateRemoteSerialization,
                  }) as any);
        });

    const notifyLocal = <
        M extends Message.Outgoing<any> | Message.Incoming<any>,
    >(
        message: M,
        discard: boolean,
    ) =>
        Effect.suspend(
            ...
            > => {
                ...
                if (!isEntityOnLocalShards(address)) {
                    ...
                    return Effect.fail(
                        new EntityNotManagedByRunner({ address }),
                    );
                }
                ...
                const notify = storageEnabled
                    ...
                    : () =>
                          Effect.dieMessage(
                              "Sharding.notifyLocal: storage is disabled",
                          );
    ...
    const isTransientError = Predicate.or(
        RunnerUnavailable.is,
        EntityNotAssignedToRunner.is,
    );
    ...
    > {
        return Effect.catchIf(
            Effect.suspend(() => {
                ...
                const runnerIsLocal =
                    Option.isSome(maybeRunner) &&
                    isLocalRunner(maybeRunner.value);
                if (isPersisted) {
    ...
    const startShardManagerTimeout = FiberHandle.run(
        ...
        Effect.flatMap(
            Effect.sleep(config.shardManagerUnavailableTimeout),
            () => {
    ...
    yield* Effect.gen(function* () {
        ...
        if (Option.isSome(config.runnerAddress)) {
            const machineId = yield* shardManager.register(
                config.runnerAddress.value,
                config.shardGroups,
            );
            ...
            const client = yield* RpcClient.makeNoSerialization(
                entity.protocol,
                {
                    ...
                    onFromClient(
                        ...
                        | PersistenceError
                    > {
                        const address = Context.unsafeGet(
                            options.context,
                            ClientAddressTag,
                        );
                        switch (options.message._tag) {
                            case "Request": {
                                const fiber = Option.getOrThrow(
                                    Fiber.getCurrentFiber(),
                                );
                                const id = Snowflake.Snowflake(
                                    options.message.id,
                                );
                                const rpc = entity.protocol.requests.get(
                                    options.message.tag,
                                )!;
                                let respond: (
                                ...
                                return sendOutgoing(
                                    new Message.OutgoingRequest({
                                        ...
                                        rpc,
                                        context:
                                            fiber.currentContext as Context.Context<any>,
                                        respond,
                                    }),
                                    options.discard,
                                );
                            ...
                            case "Ack": {
                                const requestId = Snowflake.Snowflake(
    ...
    const registerEntity: Sharding["Type"]["registerEntity"] =
        Effect.fnUntraced(function* (entity, build, options) {
            ...
            yield* Scope.addFinalizer(
                scope,
                Effect.sync(() => entityManagers.delete(entity.type)),
            );
        ...
        Effect.forEach(
            ...
            (state) =>
                Effect.catchAllCause(Scope.close(state.scope, exit), (cause) =>
                    Effect.annotateLogs(
                        Effect.logError("Error closing entity manager", cause),
                        {
        ...
        yield* Scope.addFinalizer(
            ...
            Effect.gen(function* () {
                yield* Effect.logDebug(
                    "Unregistering runner from shard manager",
                    selfAddress,
                );
                yield* shardManager
                    .unregister(selfAddress)
                    .pipe(
                        Effect.catchAllCause((cause) =>
                            Effect.logError(
                                "Error calling unregister with shard manager",
                                cause,
                            ),
                        ),
                    );
