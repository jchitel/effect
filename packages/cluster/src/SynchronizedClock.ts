 ...
 */
export const make: (
    getRemoteTime: Effect.Effect<number, never, never>,
) => Effect.Effect<Clock.Clock, never, Scope> = Effect.fnUntraced(
    function* (getRemoteTime) {
        ...
        yield* getRemoteTime.pipe(
            ...
            Effect.map(([duration, shardManagerTime]) => {
                ...
                shardManagerTime =
                    shardManagerTime + Duration.toMillis(halfTrip) + 1;
 ...
 * @category Layers
 */
export const layer: Layer.Layer<never, never, ShardManagerClient> =
    Layer.unwrapScoped(
        Effect.gen(function* () {
