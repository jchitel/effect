 ...
 * @category models
 */
export class ShardManager extends Context.Tag("@effect/cluster/ShardManager")<
    ShardManager,
    {
        /**
         ...
         */
        readonly shardingEvents: Effect.Effect<
            Queue.Dequeue<ShardingEvent>,
            never,
            Scope
        >;
        /**
         ...
         */
        readonly notifyUnhealthyRunner: (
 ...
 * @category Config
 */
export class Config extends Context.Tag("@effect/cluster/ShardManager/Config")<
    Config,
    {
        /**
...
export const configConfig: Config_.Config<Config["Type"]> = Config_.all({
    ...
    runnerHealthCheckInterval: Config_.duration(
        "runnerHealthCheckInterval",
    ).pipe(
 ...
 * @category Config
 */
export const configFromEnv: Effect.Effect<Config["Type"], ConfigError> =
    configConfig.pipe(
...
export const layerConfig = (
    config?: Partial<Config["Type"]> | undefined,
): Layer.Layer<Config> =>
...
export const layerConfigFromEnv = (
    ...
    Layer.effect(
        Config,
        config
            ? Effect.map(configFromEnv, (env) => ({ ...env, ...config }))
            : configFromEnv,
    );
 ...
 * @category Client
 */
export class ShardManagerClient extends Context.Tag(
    ...
    ShardManagerClient,
    {
         ...
         */
        readonly shardingEvents: Effect.Effect<
            Mailbox.ReadonlyMailbox<ShardingEvent>,
            never,
            Scope
        >;
        /**
...
export class Rpcs extends RpcGroup.make(
    ...
    Rpc.make("GetAssignments", {
        success: Schema.Array(
            Schema.Tuple(ShardId, Schema.Option(RunnerAddress)),
        ),
    }),
...
> = Effect.gen(function* () {
    ...
    return ShardManagerClient.of({
        ...
        notifyUnhealthyRunner: (address) =>
            Effect.orDie(client.NotifyUnhealthyRunner({ address })),
...
export const make = Effect.gen(function* () {
    ...
    const state = yield* Effect.orDie(
        State.fromStorage(shardingConfig.shardsPerGroup),
    );
    const scope = yield* Effect.scope;
    const events = yield* PubSub.unbounded<ShardingEvent>();

    function updateRunnerMetrics() {
        ClusterMetrics.runners.unsafeUpdate(
            MutableHashMap.size(state.allRunners),
            [],
        );
    }
    ...
    function updateShardMetrics() {
        ...
        for (const [address, shardCount] of stats.perRunner) {
            ClusterMetrics.assignedShards.unsafeUpdate(shardCount, [
    ...
    const persistRunners = Effect.unsafeMakeSemaphore(1).withPermits(1)(
        withRetry(
            Effect.suspend(() =>
                storage.saveRunners(
                    Iterable.map(state.allRunners, ([address, runner]) => [
                        address,
                        runner.runner,
                    ]),
                ),
    ...
    const persistAssignments = Effect.unsafeMakeSemaphore(1).withPermits(1)(
        withRetry(
            Effect.suspend(() => storage.saveAssignments(state.assignments)),
        ),
    );

    const notifyUnhealthyRunner = Effect.fnUntraced(function* (
        address: RunnerAddress,
    ) {
        ...
        if (!(yield* runnerHealthApi.isAlive(address))) {
            yield* Effect.logWarning(
                `Runner at address '${address.toString()}' is not alive`,
            );
    ...
    function updateShardsState(
        ...
        return Effect.suspend(() => {
            if (
                ...
                !MutableHashMap.has(state.allRunners, address.value)
            ) {
                return Effect.fail(
                    new RunnerNotRegistered({ address: address.value }),
                );
            }
    ...
    const register = Effect.fnUntraced(function* (runner: Runner) {
        ...
        const current = MutableHashMap.get(
            state.allRunners,
            runner.address,
        ).pipe(Option.filter((r) => r.runner.version === runner.version));
        ...
        yield* PubSub.publish(
            events,
            ShardingEvent.RunnerRegistered({ address: runner.address }),
        );
    ...
    const unregister = Effect.fnUntraced(function* (address: RunnerAddress) {
        ...
        if (unassignments.length > 0) {
            yield* PubSub.publish(
                events,
                ShardingEvent.RunnerUnregistered({ address }),
            );
        }
    ...
    const runRebalance = Effect.gen(function* () {
        ...
        const assignments = MutableHashMap.empty<
            RunnerAddress,
            MutableHashSet.MutableHashSet<ShardId>
        >();
        ...
        for (const group of state.shards.keys()) {
            const [groupAssignments, groupUnassignments, groupChanges] =
                decideAssignmentsForShards(state, group);
            for (const [address, shards] of groupAssignments) {
                addAllNested(
                    assignments,
                    address,
                    Array.from(shards, (id) => makeShardId(group, id)),
                );
            }
        ...
        Effect.withSpan("ShardManager.rebalance", { captureStackTrace: false }),
    );

    const checkRunnerHealth: Effect.Effect<void> = Effect.suspend(() =>
        Effect.forEach(
            MutableHashMap.keys(state.allRunners),
    ...
    yield* Effect.addFinalizer(() =>
        persistAssignments.pipe(
            Effect.catchAllCause((cause) =>
                Effect.logWarning(
                    "Failed to persist assignments on shutdown",
                    cause,
                ),
            ),
            Effect.zipRight(
                persistRunners.pipe(
                    Effect.catchAllCause((cause) =>
                        Effect.logWarning(
                            "Failed to persist runners on shutdown",
                            cause,
                        ),
                    ),
                ),
...
export const layerServerHandlers = Rpcs.toLayer(
    Effect.gen(function* () {
        ...
        return {
            ...
            GetAssignments: () =>
                Effect.map(shardManager.getAssignments, (assignments) =>
