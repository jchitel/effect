...
/** @internal */
export class State {
    static fromStorage = Effect.fnUntraced(function* (shardsPerGroup: number) {
        ...
        if (deadRunners.length > 0) {
            yield* Effect.logWarning(
                "Ignoring runners that are no longer considered alive:",
                deadRunners,
            );
        }
        ...
        for (const [shard, address] of storedAssignments) {
            if (
                ...
                MutableHashMap.has(aliveRunners, address.value)
            ) {
        ...
        const runnerState = new Map<
            string,
            MutableHashMap.MutableHashMap<RunnerAddress, RunnerWithMetadata>
        >();
        ...
        const shardState = new Map<
            string,
            Map<number, Option.Option<RunnerAddress>>
        >();
    ...
    constructor(
        readonly allRunners: MutableHashMap.MutableHashMap<
            RunnerAddress,
            RunnerWithMetadata
        >,
        readonly runners: Map<
            string,
            MutableHashMap.MutableHashMap<RunnerAddress, RunnerWithMetadata>
        >,
        ...
        this.perRunner = new Map<
            string,
            MutableHashMap.MutableHashMap<RunnerAddress, Set<number>>
        >();
        ...
        for (const [group, groupMap] of this.shards) {
            ...
            for (const [id, address_] of groupMap) {
                const address = Option.filter(address_, (addr) =>
                    MutableHashMap.has(this.allRunners, addr),
                );
                MutableHashMap.set(
                    this.assignments,
                    new ShardId({ group, id }),
                    address,
                );
                if (Option.isSome(address)) {
                    Option.getOrUndefined(
                        MutableHashMap.get(perRunnerMap, address.value),
                    )?.add(id);
                }
            }
        }
    ...
    readonly assignments: MutableHashMap.MutableHashMap<
        ShardId,
        Option.Option<RunnerAddress>
    >;
    readonly perRunner: Map<
        string,
        MutableHashMap.MutableHashMap<RunnerAddress, Set<number>>
    >;
    ...
    addGroup(group: string): void {
        this.runners.set(
            group,
            MutableHashMap.empty<RunnerAddress, RunnerWithMetadata>(),
        );
    ...
    addAssignments(
        ...
        for (const shardId of shards) {
            const currentAddress = Option.flatten(
                MutableHashMap.get(this.assignments, shardId),
            );
            ...
            if (Option.isSome(currentAddress)) {
                Option.getOrUndefined(
                    MutableHashMap.get(perRunner, currentAddress.value),
                )?.delete(shardId.id);
            }
            if (Option.isSome(address)) {
                Option.getOrUndefined(
                    MutableHashMap.get(perRunner, address.value),
                )?.add(shardId.id);
            }
        }
    }
    ...
    allRunnersHaveVersion(version: Option.Option<number>): boolean {
        return version.pipe(
            Option.map((max) =>
                Arr.every(this.runnerVersions, (version) => version === max),
            ),
    ...
    averageShardsPerRunner(group: string): number {
        const runnerCount = MutableHashMap.size(
            this.runners.get(group) ?? MutableHashMap.empty(),
        );
...
/** @internal */
export const RunnerWithMetadata = (
    runner: RunnerWithMetadata,
): RunnerWithMetadata => runner;

/** @internal */
export function decideAssignmentsForShards(
    state: State,
    group: string,
): readonly [
    ...
    if (state.allRunnersHaveVersion(maxVersion)) {
        ...
        MutableHashMap.forEach(shardsPerRunner, (shards) => {
            const extraShards = Math.max(
                0,
                shards.size - averageShardsPerRunner,
            );
...
function pickNewRunners(
    ...
    for (const shardId of shardsToRebalance) {
        ...
        for (const [address, shards] of shardsPerRunner) {
            ...
            if (
                candidate === undefined ||
                shards.size < candidateShards!.size
            ) {
                candidate = address;
        ...
        const oldRunner = Option.getOrUndefined(
            shardsGroup.get(shardId) ?? Option.none(),
        );
        ...
        const oldShards =
            oldRunner &&
            Option.getOrUndefined(
                MutableHashMap.get(shardsPerRunner, oldRunner),
            );
