...
>(
                    ...
                    Effect.fnUntraced(function* (scope) {
                        ...
                        const server = yield* RpcServer.makeNoSerialization(
                            entity.protocol,
                            {
                                ...
                                disableFatalDefects:
                                    options.disableFatalDefects,
                                onFromServer(response): Effect.Effect<void> {
                                    switch (response._tag) {
                                        case "Exit": {
                                            const request = activeRequests.get(
                                                response.requestId,
                                            );
                                            ...
                                            return Effect.orDie(
                                                retryRespond(
                                                    ...
                                                    Effect.suspend(() => {
                                                        ...
                                                        request.lastSentChunk =
                                                            Option.some(reply);
                                                        return request.message.respond(
                                                            reply,
                                                        );
                                                    }),
                                                ),
                ...
                yield* Scope.addFinalizer(
                    ...
                    Effect.withFiberRuntime((fiber) => {
                        ...
                        return state
                            ...
                            .pipe(
                                Effect.andThen(
                                    Effect.interruptible(endLatch.await),
                                ),
    ...
    const gauge = ClusterMetrics.entities.pipe(
                ...
                > => {
                    switch (message._tag) {
                        case "IncomingRequestLocal": {
                            ...
                            let entry = server.activeRequests.get(
                            ...
                            if (
                                !storageEnabled &&
                                Context.get(rpc.annotations, Persisted)
                            ) {
                        ...
                        case "IncomingEnvelope": {
                            ...
                            if (!entry) {
                                ...
                                message.envelope.replyId !==
                                    entry.lastSentChunk.value.id
                            ) {
                                return Effect.void;
                            }
                            return server.write(
    ...
    const interruptShard = (shardId: ShardId) =>
        Effect.suspend(function loop(): Effect.Effect<void> {
            ...
            return Effect.flatMap(
                Effect.forEach(
                    toInterrupt,
                    (state) => entities.removeIgnore(state.address),
                    {
    ...
    return identity<EntityManager>({
        ...
        isProcessingFor(message, options) {
            ...
            const request = state.activeRequests.get(
                message.envelope.requestId,
            );
                            ...
                            message.respond(
                                new Reply.ReplyWithContext({
                                    reply: new Reply.WithExit({
                                        ...
                                        exit: Exit.die(
                                            new MalformedMessage({ cause }),
                                        ),
                                    }),
                                    rpc: entity.protocol.requests.get(
                                        message.envelope.tag,
                                    )!,
                                    context,
                                }),
...
const makeMessageSchema = <Type extends string, Rpcs extends Rpc.Any>(
    entity: Entity<Type, Rpcs>,
): Schema.Schema<
    | {
