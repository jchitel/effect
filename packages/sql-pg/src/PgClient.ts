    ...
    Effect.gen(function* () {
        ...
        const opts: PartialWithUndefined<PostgresOptions> = {
            ...
            max_lifetime: options.connectionTTL
                ? Math.round(
                      Duration.toMillis(
                          Duration.decode(options.connectionTTL),
                      ) / 1000,
                  )
                : undefined,
            idle_timeout: options.idleTimeout
                ? Math.round(
                      Duration.toMillis(Duration.decode(options.idleTimeout)) /
                          1000,
                  )
                : undefined,
            connect_timeout: options.connectTimeout
                ? Math.round(
                      Duration.toMillis(
                          Duration.decode(options.connectTimeout),
                      ) / 1000,
                  )
                : undefined,
        ...
        class ConnectionImpl implements Connection {
            ...
            private run(
                query:
                    ...
                    | postgres.PendingValuesQuery<any>,
            ) {
            ...
            executeStream(
                ...
                return Stream.mapChunks(
                    Stream.fromAsyncIterable(
                        this.pg
                            .unsafe(sql, params as any)
                            .cursor(16) as AsyncIterable<Array<any>>,
                        (cause) =>
                            new SqlError({
                                cause,
                                message: "Failed to execute statement",
        ...
        return Object.assign(
            yield* Client.make({
                ...
                spanAttributes: [
                    ...(options.spanAttributes
                        ...
                        : []),
                    [
                        OtelSemConv.ATTR_DB_SYSTEM_NAME,
                        OtelSemConv.DB_SYSTEM_NAME_VALUE_POSTGRESQL,
                    ],
                    [
                        OtelSemConv.ATTR_DB_NAMESPACE,
                        opts.database ?? options.username ?? "postgres",
                    ],
            ...
            }),
            {
                ...
                config: {
                    ...
                    password: client.options.pass
                        ? Redacted.make(client.options.pass)
                        : undefined,
