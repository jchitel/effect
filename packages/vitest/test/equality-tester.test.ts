...
describe("toMatchObject", () => {
    ...
    it("option", () => {
        expect(Option.some({ a: 1, b: 2 })).toMatchObject(
            Option.some({ a: 1 }),
        );
        ...
        expect({
            ...
            y: Option.none(),
        }).toMatchObject({ x: Option.some({ a: 1 }) });
    ...
    it("either", () => {
        expect(Either.right({ a: 1, b: 2 })).toMatchObject(
            Either.right({ a: 1 }),
        );
        expect(Either.left({ a: 1, b: 2 })).toMatchObject(
            Either.left({ a: 1 }),
        );
...
describe.each(["toStrictEqual", "toEqual"] as const)("%s", (matcher) => {
    ...
    it("exit", () => {
        ...
        expect(
            Exit.failCause(
                Cause.sequential(Cause.fail("f1"), Cause.fail("f2")),
            ),
        ).not[matcher](
            Exit.failCause(
                Cause.sequential(Cause.fail("f1"), Cause.fail("f3")),
            ),
        );
    });
