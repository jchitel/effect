...
export class Literal implements Annotated {
    ...
    toString() {
        return Option.getOrElse(getExpected(this), () =>
            util_.formatUnknown(this.literal),
        );
    }
    /**
     * @since 3.10.0
 ...
 * @since 3.10.0
 */
export const isLiteral: (ast: AST) => ast is Literal =
    createASTGuard("Literal");
...
export class UniqueSymbol implements Annotated {
    ...
    toString() {
        return Option.getOrElse(getExpected(this), () =>
            util_.formatUnknown(this.symbol),
        );
    }
    /**
     * @since 3.10.0
...
const templateLiteralSpanUnionTypeToString = (
    ...
    switch (type._tag) {
        ...
        case "Union":
            return type.types
                .map(templateLiteralSpanUnionTypeToString)
                .join(" | ");
    }
};
...
export class TemplateLiteralSpan {
    ...
    constructor(
        ...
        if (isTemplateLiteralSpanType(type)) {
            ...
            throw new Error(
                errors_.getSchemaUnsupportedLiteralSpanErrorMessage(type),
            );
        }
    }
    /**
...
export class TypeLiteral implements Annotated {
        ...
        for (let i = 0; i < indexSignatures.length; i++) {
            ...
            if (isStringKeyword(encodedParameter)) {
                if (parameters.string) {
                    throw new Error(
                        errors_.getASTDuplicateIndexSignatureErrorMessage(
                            "string",
                        ),
                    );
                }
    ...
    toJSON(): object {
        return {
            _tag: this._tag,
            propertySignatures: this.propertySignatures.map((ps) =>
                ps.toJSON(),
            ),
...
export const flatten = (candidates: ReadonlyArray<AST>): Array<AST> =>
    Arr.flatMap(candidates, (ast) =>
        isUnion(ast) ? flatten(ast.types) : [ast],
    );
 ...
 * @since 3.10.0
 */
export const isSuspend: (ast: AST) => ast is Suspend =
    createASTGuard("Suspend");
...
const createTransformationGuard =
    ...
    ): ast is Extract<TransformationKind, { _tag: T }> =>
        ast._tag === tag;
    ...
    switch (ast._tag) {
        ...
        case "Union":
            return new PropertySignature(
                ...
                Union.make(
                    ast.types.map(
                        (ast) => getPropertyKeyIndexedAccess(ast, name).type,
                    ),
                ),
                false,
                true,
        ...
        switch (key._tag) {
            ...
            case "Literal":
                if (
                    ...
                    throw new Error(
                        errors_.getASTUnsupportedLiteralErrorMessage(
                            key.literal,
                        ),
                    );
                }
            ...
            default:
                throw new Error(
                    errors_.getASTUnsupportedKeySchemaErrorMessage(key),
                );
        }
    };
    go(key);
        ...
        case "Transformation": {
            switch (ast.transformation._tag) {
                ...
                case "TypeLiteralTransformation": {
                    ...
                    for (const k of keys) {
                        const t =
                            ast.transformation.propertySignatureTransformations.find(
                                (t) => t.to === k,
                            );
                        if (t) {
                            ts.push(t);
...
export const partial = (ast: AST, options?: { readonly exact: true }): AST => {
    ...
    switch (ast._tag) {
        case "TupleType":
            return new TupleType(
                ast.elements.map(
                    (e) =>
                        new OptionalType(
                            exact ? e.type : orUndefined(e.type),
                            true,
                        ),
                ),
                Arr.match(ast.rest, {
        ...
        case "Union":
            return Union.make(
                ast.types.map((member) => partial(member, options)),
            );
        case "Suspend":
...
export const pickAnnotations =
    ...
    (annotated: Annotated): Annotations | undefined => {
        ...
        for (const id of annotationIds) {
            if (
                Object.prototype.hasOwnProperty.call(annotated.annotations, id)
            ) {
                if (out === undefined) {
                    out = {};
                }
...
const createJSONIdentifierAnnotation = (
    annotated: Annotated,
...
const encodedAST_ = (ast: AST, isBound: boolean): AST => {
    switch (ast._tag) {
        ...
        case "TupleType": {
            ...
            const restASTs = getRestASTs(ast.rest);
            const rest = changeMap(restASTs, (ast) =>
                encodedAST_(ast, isBound),
            );
            return elements === ast.elements && rest === restASTs
                ? ast
        ...
        case "Union": {
            const types = changeMap(ast.types, (ast) =>
                encodedAST_(ast, isBound),
            );
            return types === ast.types
                ? ast
        ...
        case "Transformation": {
            const identifier = createJSONIdentifierAnnotation(ast);
            return encodedAST_(
                identifier ? annotations(ast.from, identifier) : ast.from,
                isBound,
            );
        }
    }
            ...
            return ast.propertySignatures
                ...
                .concat(
                    ast.indexSignatures.map((is) =>
                        getEncodedParameter(is.parameter),
                    ),
                );
        case "Suspend":
            return _keyof(ast.f());
...
function getBrands(ast: Annotated): string {
    return Option.match(getBrandAnnotation(ast), {
        ...
        onSome: (brands) =>
            brands
                .map((brand) => ` & Brand<${util_.formatUnknown(brand)}>`)
                .join(""),
    });
}
