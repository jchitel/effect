...
const ConsProto: Omit<Cons<unknown>, "head" | "tail" | typeof nonEmpty> = {
    ...
    [Equal.symbol](this: Cons<unknown>, that: unknown): boolean {
        return (
            isList(that) && this._tag === that._tag && _equivalence(this, that)
        );
    },
...
export declare namespace List {
     ...
     * @since 2.0.0
     */
    export type Infer<S extends List<any>> =
        S extends List<infer A> ? A : never;
...
export const splitAt: {
    (
        n: number,
    ): <A>(self: List<A>) => [beforeIndex: List<A>, fromIndex: List<A>];
