...
const getJsonSchemaAnnotations = (
    ast: AST.AST,
...
const removeDefaultJsonSchemaAnnotations = (
    ...
    if (
        jsonSchemaAnnotations["title"] ===
        ast.annotations[AST.TitleAnnotationId]
    ) {
        delete jsonSchemaAnnotations["title"];
    }
    if (
        jsonSchemaAnnotations["description"] ===
        ast.annotations[AST.DescriptionAnnotationId]
    ) {
...
const getASTJsonSchemaAnnotations = (ast: AST.AST): JsonSchemaAnnotations => {
    ...
    switch (ast._tag) {
        case "StringKeyword":
            return removeDefaultJsonSchemaAnnotations(
                jsonSchemaAnnotations,
                AST.stringKeyword,
            );
        case "NumberKeyword":
...
const addEnumType = (jsonSchema: JsonSchema7): JsonSchema7 => {
    if ("enum" in jsonSchema && !("type" in jsonSchema)) {
        const type: "string" | "number" | "boolean" | undefined =
            jsonSchema.enum.every(Predicate.isString)
                ? "string"
...
const isNeverJSONSchema = (
    jsonSchema: JsonSchema7,
): jsonSchema is JsonSchema7Never =>
    "$id" in jsonSchema && jsonSchema.$id === "/schemas/never";

const isAnyJSONSchema = (
    jsonSchema: JsonSchema7,
): jsonSchema is JsonSchema7Any =>
    "$id" in jsonSchema && jsonSchema.$id === "/schemas/any";

const isUnknownJSONSchema = (
    jsonSchema: JsonSchema7,
): jsonSchema is JsonSchema7Unknown =>
    "$id" in jsonSchema && jsonSchema.$id === "/schemas/unknown";

const isVoidJSONSchema = (
    jsonSchema: JsonSchema7,
): jsonSchema is JsonSchema7Void =>
    ...
    switch (ast._tag) {
        ...
        case "TupleType": {
            ...
            if (len > 0) {
                output.minItems =
                    len -
                    ast.elements.filter((element) => element.isOptional).length;
            ...
            if (restLength > 0) {
                ...
                if (restLength > 1) {
                    throw new Error(
                        errors_.getJSONSchemaUnsupportedPostRestElementsErrorMessage(
                            path,
                        ),
                    );
                }
        ...
        case "TypeLiteral": {
                ...
                switch (parameter._tag) {
                    ...
                    case "TemplateLiteral": {
                        ...
                        propertyNames = {
                            type: "string",
                            pattern:
                                AST.getTemplateLiteralRegExp(parameter).source,
                        };
                        break;
                    }
                    case "Refinement": {
            ...
            for (let i = 0; i < ast.propertySignatures.length; i++) {
                ...
                if (Predicate.isString(name)) {
                    ...
                    throw new Error(
                        errors_.getJSONSchemaUnsupportedKeyErrorMessage(
                            name,
                            path,
                        ),
                    );
        ...
        case "Enums": {
            const anyOf = ast.enums.map((e) =>
                addEnumType({ title: e[0], enum: [e[1]] }),
            );
        ...
        case "Refinement": {
            ...
            if (AST.getTransformationFrom(ast) === undefined) {
                throw new Error(
                    errors_.getJSONSchemaMissingAnnotationErrorMessage(
                        path,
                        ast,
                    ),
                );
        ...
        case "Suspend": {
            const identifier = Option.orElse(AST.getJSONIdentifier(ast), () =>
        ...
        case "Transformation": {
            ...
            if (AST.isTypeLiteralTransformation(ast.transformation)) {
                ...
                if (
                    ...
                    Option.isNone(AST.getIdentifierAnnotation(next))
                ) {
                ...
                if (
                    Option.isSome(title) &&
                    Option.isNone(AST.getTitleAnnotation(next))
                ) {
                    next = AST.annotations(next, {
                ...
                if (
                    ...
                    Option.isNone(AST.getDescriptionAnnotation(next))
                ) {
                    next = AST.annotations(next, {
