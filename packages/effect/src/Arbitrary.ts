                                ...
                                : makeNumberConstraints({
                                      ...
                                      noDefaultInfinity:
                                          ...
                                          meta.type === "number"
                                              ...
                                              : undefined,
                                      min:
                                          meta.exclusiveMinimum ?? meta.minimum,
                                      minExcluded:
                                          "exclusiveMinimum" in meta
                                              ? true
                                              : undefined,
                                      max:
                                          meta.exclusiveMaximum ?? meta.maximum,
                                      maxExcluded:
                                          "exclusiveMaximum" in meta
                                              ? true
                                              : undefined,
                                  });
                        return {
...
function mergeNumberConstraints(
    ...
    return makeNumberConstraints({
        ...
        minExcluded: getOr(
            c1.constraints.minExcluded,
            c2.constraints.minExcluded,
        ),
        max: getMin(c1.constraints.max, c2.constraints.max),
        maxExcluded: getOr(
            c1.constraints.maxExcluded,
            c2.constraints.maxExcluded,
        ),
    ...
    (description, ctx, lazyArb) => {
            ...
            switch (description._tag) {
                ...
                case "Enums":
                    if (description.enums.length === 0) {
                        throw new Error(
                            errors_.getArbitraryEmptyEnumErrorMessage(
                                description.path,
                            ),
                        );
                    }
        ...
        if (description._tag === "Declaration") {
            return applyFilters(
                filters,
                annotation(
                    ...description.typeParameters.map((p) => go(p, ctx)),
                    ctx,
                ),
            );
        }
        ...
        switch (description._tag) {
            ...
            case "TemplateLiteral": {
                return (fc) => {
                    ...
                    const getTemplateLiteralArb = (
                        description: TemplateLiteral,
                    ) => {
                        ...
                        description.spans.forEach((span) => {
                            components.push(
                                getTemplateLiteralSpanTypeArb(span.description),
                            );
                            if (span.literal !== "") {
            ...
            case "TupleType": {
                ...
                return (fc) => {
                    ...
                    if (Arr.isNonEmptyReadonlyArray(rest)) {
                        const constraints =
                            buildArrayConstraints(description) ??
                            constArrayConstraints;
            ...
            */
            ...
            case "Ref": {
                ...
                throw new Error(
                    `BUG: Ref ${JSON.stringify(description.id)} not found`,
                );
            }
        }
    },
