 ...
 * @category models
 */
export interface Micro<out A, out E = never, out R = never>
    extends Effect<A, E, R> {
 ...
 * @since 3.4.3
 */
export interface MicroUnify<A extends { [Unify.typeSymbol]?: any }>
...
class MicroFiberImpl<in out A = any, in out E = any>
    implements MicroFiber<A, E>
{
    readonly [MicroFiberTypeId]: MicroFiber.Variance<A, E>;
    ...
    getCont<S extends successCont | failureCont>(
        ...
        | (Primitive &
              Record<S, (value: any, fiber: MicroFiberImpl) => Primitive>)
        | undefined {
        while (true) {
...
>(options: {
    ...
    readonly eval?: (
        this: Primitive & {
            readonly [args]: Single extends true
                ? Parameters<Fn>[0]
                : Parameters<Fn>;
        },
        fiber: MicroFiberImpl,
            ...
            const onExit = (exit: MicroExit<any, any>) => {
                ...
                resume(
                    fibers.size === 0
                        ? exit
                        : flatMap(
                              uninterruptible(fiberInterruptAll(fibers)),
                              () => exit,
                          ),
                );
            };
...
export const filterOrFail: {
    ...
    <A, E, R, B extends A, E2>(
        ...
        flatMap(self, (a) =>
            refinement(a) ? succeed(a) : fail(orFailWith(a)),
        ),
);
...
export const scheduleWithMaxDelay: {
    ...
    (self: MicroSchedule, max: number): MicroSchedule =>
        (attempt, elapsed) =>
            Option.map(self(attempt, elapsed), (duration) =>
...
export const catchIf: {
    <E, EB extends E, A2, E2, R2>(
        ...
        self: Micro<A, E, R>,
    ) => Micro<A2 | A, E2 | Exclude<E, EB>, R2 | R>;
    <E, A2, E2, R2>(
    ...
    <A, E, R, A2, E2, R2>(
        ...
        catchCauseIf(
            self,
            (f): f is MicroCause.Fail<E> =>
                causeIsFail(f) && predicate(f.error),
            (fail) => f(fail.error),
        ),
);
...
export const orElseSucceed: {
    <B>(
        f: LazyArg<B>,
    ): <A, E, R>(self: Micro<A, E, R>) => Micro<A | B, never, R>;
        ...
        repeatExit(self, {
            ...
            while: (exit) =>
                ...
                (options?.while === undefined ||
                    options.while(exit.cause.error)),
        }),
);
    ...
    state:
        | {
              ...
              readonly finalizers: Set<
                  (exit: MicroExit<any, any>) => Micro<void>
              >;
          }
        | {
...
export const acquireUseRelease = <Resource, E, R, A, E2, R2, E3, R3>(
    ...
    uninterruptibleMask((restore) =>
        flatMap(acquire, (a) =>
            flatMap(exit(restore(use(a))), (exit) =>
                andThen(release(a, exit), exit),
            ),
        ),
    );
...
const unsafeFork = <FA, FE, A, E, R>(
    ...
    const child = new MicroFiberImpl<A, E>(
        parent.context,
    ...
    <A, E, R>(
        ...
        uninterruptibleMask((restore) =>
            flatMap(scope.fork, (scope) =>
                tap(
                    restore(
                        forkDaemon(onExit(self, (exit) => scope.close(exit))),
                    ),
...
export interface YieldableError extends Pipeable, Inspectable, Readonly<Error> {
    readonly [Effectable.EffectTypeId]: Effect.VarianceStruct<
        never,
        this,
        never
    >;
    readonly [Effectable.StreamTypeId]: Stream.VarianceStruct<
        never,
        this,
        never
    >;
