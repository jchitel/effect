 ...
 * @since 2.0.0
 */
export const isLeft: <R, L>(self: Either<R, L>) => self is Left<L, R> =
    either.isLeft;
 ...
 * @since 2.0.0
 */
export const isRight: <R, L>(self: Either<R, L>) => self is Right<L, R> =
    either.isRight;
...
} = dual(2, <R, L, B>(self: Either<R, L>, onLeft: (left: L) => B): R | B =>
 ...
 * @since 2.0.0
 */
export const getOrNull: <R, L>(self: Either<R, L>) => R | null =
    getOrElse(constNull);
 ...
 * @since 2.0.0
 */
export const getOrUndefined: <R, L>(self: Either<R, L>) => R | undefined =
...
export const orElse: {
    <L, R2, L2>(
        that: (left: L) => Either<R2, L2>,
    ): <R>(self: Either<R, L>) => Either<R | R2, L2>;
    <R, L, R2, L2>(
        self: Either<R, L>,
        that: (left: L) => Either<R2, L2>,
    ): Either<R | R2, L2>;
...
export const flatMap: {
    <R, R2, L2>(
        f: (right: R) => Either<R2, L2>,
    ): <L>(self: Either<R, L>) => Either<R2, L | L2>;
    <R, L, R2, L2>(
        self: Either<R, L>,
        f: (right: R) => Either<R2, L2>,
    ): Either<R2, L | L2>;
    ...
    <R1, L1, R2, L2>(
        self: Either<R1, L1>,
        f: (right: R1) => Either<R2, L2>,
    ): Either<R2, L1 | L2> => (isLeft(self) ? left(self.left) : f(self.right)),
);
...
export const andThen: {
    ...
    <R2, L2>(
        f: Either<R2, L2>,
    ): <L, R1>(self: Either<R1, L>) => Either<R2, L | L2>;
    <R, R2>(f: (right: R) => R2): <L>(self: Either<R, L>) => Either<R2, L>;
    ...
    <R, L, R2, L2>(
        self: Either<R, L>,
        f: (right: R) => Either<R2, L2> | Either<R2, L2>,
    ): Either<R2, L | L2> =>
        flatMap(self, (a) => {
...
export const zipWith: {
    ...
    <R, L, R2, L2, B>(
        ...
        that: Either<R2, L2>,
        f: (right: R, right2: R2) => B,
    ): Either<B, L | L2> => flatMap(self, (r) => map(that, (r2) => f(r, r2))),
);
...
export const ap: {
    <R, L2>(
        that: Either<R, L2>,
    ): <R2, L>(self: Either<(right: R) => R2, L>) => Either<R2, L | L2>;
    <R, R2, L, L2>(
        self: Either<(right: R) => R2, L>,
        that: Either<R, L2>,
    ): Either<R2, L | L2>;
} = dual(
    2,
    <R, R2, L, L2>(
        self: Either<(right: R) => R2, L>,
        that: Either<R, L2>,
    ): Either<R2, L | L2> => zipWith(self, that, (f, a) => f(a)),
);
...
export const all: <
    const I extends
        | Iterable<Either<any, any>>
        | Record<string, Either<any, any>>,
>(
    input: I,
) => [I] extends [ReadonlyArray<Either<any, any>>]
    ? Either<
          {
              -readonly [K in keyof I]: [I[K]] extends [Either<infer R, any>]
                  ? R
                  : never;
          },
          I[number] extends never
              ? never
              : [I[number]] extends [Either<any, infer L>]
                ? L
                : never
      >
    : [I] extends [Iterable<Either<infer R, infer L>>]
      ...
      : Either<
            ...
            I[keyof I] extends never
                ? never
                : [I[keyof I]] extends [Either<any, infer L>]
                  ? L
                  : never
        > = (
 ...
 * @category mapping
 */
export const flip = <R, L>(self: Either<R, L>): Either<L, R> =>
...
export const bindTo: {
    <N extends string>(
        name: N,
    ): <R, L>(self: Either<R, L>) => Either<{ [K in N]: R }, L>;
