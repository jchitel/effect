    ...
    switch (ast._tag) {
        ...
        case "Transformation": {
            ...
            const from = isDecoding
                ? goMemo(ast.from, true)
                : goMemo(ast.to, false);
            const to = isDecoding
                ? goMemo(ast.to, true)
                : goMemo(ast.from, false);
            return (i, options) =>
                handleForbidden(
                    flatMap(
                        mapError(
                        ...
                        (a) =>
                            flatMap(
                                mapError(
                                    transform(
                                        a,
                                        options ?? AST.defaultParseOption,
                                        ast,
                                        i,
                                    ),
                                    (e) =>
            ...
            return (input: unknown, options) => {
                ...
                for (let i = len; i <= requiredLen - 1; i++) {
                    ...
                    if (allErrors) {
                        ...
                        return Either.left(
                            new Composite(ast, input, e, output),
                        );
                    }
                }
        ...
        case "TypeLiteral": {
            ...
            return (input: unknown, options) => {
                ...
                let queue:
                    | Array<
                          (state: State) => Effect.Effect<void, ParseIssue, any>
                      >
                    | undefined = undefined;
            ...
            return (input, options) => {
                ...
                let queue:
                    | Array<
                          (
                              state: State,
                          ) => Effect.Effect<unknown, ParseIssue, any>
                      >
                    | undefined = undefined;
                ...
                const computeResult = (es: State["es"]) =>
                    Arr.isNonEmptyArray(es)
                        ? es.length === 1 && es[0][1]._tag === "Type"
                            ? Either.left(es[0][1])
                            : Either.left(
                                  new Composite(ast, input, sortByIndex(es)),
                              )
        ...
        case "Suspend": {
            const get = util_.memoizeThunk(() =>
                goMemo(AST.annotations(ast.f(), ast.annotations), isDecoding),
            );
...
export const getFinalTransformation = (
    ...
    switch (transformation._tag) {
        ...
        case "TypeLiteralTransformation":
            return (input) => {
                let out: Effect.Effect<any, ParseIssue, any> =
                    Either.right(input);
