                    ...
                    if (end > data.instant) {
                        return [
                            ...
                            makeData(
                                ...
                                pipe(
                                    data.sleeps,
                                    Chunk.prepend([end, deferred] as const),
                                ),
                            ),
                        ] as const;
                    }
                ...
                pipe(
                    ref.modify(this.clockState, (data) => {
                        ...
                        const sorted = pipe(
                            data.sleeps,
                            Chunk.sort<
                                readonly [number, Deferred.Deferred<void>]
                            >(
                                pipe(
                                    number.Order,
                        ...
                        if (Chunk.isNonEmpty(sorted)) {
                            const [instant, deferred] =
                                Chunk.headNonEmpty(sorted);
                            if (instant <= end) {
                                return [
                                    Option.some([end, deferred] as const),
                                    makeData(
                                        instant,
                                        Chunk.tailNonEmpty(sorted),
                                    ),
                                ] as const;
                            }
                        }
    ...
    layer.scoped(
        ...
        core.gen(function* ($) {
            ...
            const warningState = yield* $(
                circular.makeSynchronized(WarningData.start),
            );
            const suspendedWarningState = yield* $(
                circular.makeSynchronized(SuspendedWarningData.start),
            );
            ...
            yield* $(
                fiberRuntime.addFinalizer(() =>
                    core.zipRight(
                        ...
                        testClock.suspendedWarningDone(),
                    ),
                ),
            );
            return testClock;
