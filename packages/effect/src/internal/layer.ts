                    ...
                    core.flatMap((observers) =>
                        pipe(
                            ...
                            core.flatMap((deferred) =>
                                pipe(
                                    ref.make<Scope.Scope.Finalizer>(
                                        () => core.void,
                                    ),
...
>(2, (self, scope) =>
    core.flatMap(makeMemoMap, (memoMap) =>
...
> => {
    ...
    switch (op._op_layer) {
        ...
        case "Fresh": {
            return core.sync(
                () => (_: Layer.MemoMap) =>
                    pipe(op.layer, buildWithScope(scope)),
            );
        }
        case "FromEffect": {
            return inMemoMap
...
>(2, (self, onFailure) =>
    match(self, { onFailure, onSuccess: succeedContext }),
);
...
export const launch = <RIn, E, ROut>(
    ...
    fiberRuntime.scopedEffect(
        core.zipRight(
            fiberRuntime.scopeWith((scope) =>
                pipe(self, buildWithScope(scope)),
            ),
            core.never,
        ),
    );
    ...
    (
        ...
        suspend(() => {
            ...
            provideTo.first = Object.create(proto, {
                ...
                second: {
                    value: Array.isArray(that)
                        ? mergeAll(...(that as any))
                        : that,
                },
                zipK: {
...
export const unwrapEffect = <A, E1, R1, E, R>(
    ...
    return flatMap(fromEffect(tag, self), (context) =>
            ...
            core.map(
                options?.onEnd
                    ? core.tap(
                          ...
                          (span) =>
                              fiberRuntime.addFinalizer((exit) =>
        ...
        core.withFiberRuntime((fiber) => {
            ...
            return fiberRuntime.ensuring(
                ...
                core.withFiberRuntime((fiber) => {
                    ...
                    fiber.currentRuntimeFlags = runtimeFlags.patch(
                        rollbackFlags,
    ...
    <A, E, R, ROut>(
        ...
        if (Array.isArray(source)) {
            ...
            return provideSomeLayer(
                self,
                source as Layer.Layer<ROut, any, any>,
            );
