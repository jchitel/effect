    ...
    implements Pool<A, E>
{
    ...
    readonly allocate: Effect<PoolItem<A, E>> = core.acquireUseRelease(
        ...
        (scope, exit) =>
            exit._tag === "Failure" ? scope.close(exit) : core.void,
    );
                    ...
                    core
                        .suspend(() => {
                            ...
                            if (this.isShuttingDown) {
                                ...
                                const self = this;
                                return core.flatMap(
                                    this.resizeSemaphore.withPermitsIfAvailable(
                                        1,
                                    )(
                                        circular.forkIn(
                                            core.interruptible(this.resize),
                                            this.scope,
                                        ),
                                    ),
                            ...
                            return core.succeed(
                                Iterable.unsafeHead(this.available),
                            );
                        })
                        .pipe(
                            fiberRuntime.ensuring(
                                core.sync(() => this.waiters--),
                            ),
                            core.tap((item) => {
                                    ...
                                    core.zipRight(
                                        core.suspend(() => {
                                            ...
                                            if (this.invalidated.has(item)) {
                                                return this.invalidatePoolItem(
                                                    item,
                                                );
                                            }
                                            this.available.add(item);
    ...
    invalidate(item: A): Effect<void> {
        return core.suspend(() => {
            ...
            for (const poolItem of this.items) {
                if (
                    poolItem.exit._tag === "Success" &&
                    poolItem.exit.value === item
                ) {
                    poolItem.disableReclaim = true;
                    return core.uninterruptible(
                        this.invalidatePoolItem(poolItem),
                    );
                }
            }
            return core.void;
