...
> {
    ...
    run(): ChannelState.ChannelState<unknown, Env> {
        let result: ChannelState.ChannelState<unknown, Env> | undefined =
            undefined;
                    ...
                    if (this._currentChannel === undefined) {
                        ...
                        switch (this._currentChannel._tag) {
                            ...
                            case ChannelOpCodes.OP_CONCAT_ALL: {
                                const executor: ErasedExecutor<Env> =
                                    new ChannelExecutor(
                            ...
                            case ChannelOpCodes.OP_FAIL: {
                                result = this.doneHalt(
                                    this._currentChannel.error(),
                                );
                                break;
                            }
                            ...
                            case ChannelOpCodes.OP_PIPE_TO: {
                                ...
                                const leftExec: ErasedExecutor<Env> =
                                    new ChannelExecutor(
                            ...
                            case ChannelOpCodes.OP_SUCCEED: {
                                result = this.doneSucceed(
                                    this._currentChannel.evaluate(),
                                );
                                break;
                            }
    ...
    > {
        ...
        while (this._doneStack.length !== 0) {
            ...
            builder.push(
                cont as Continuation.ContinuationFinalizer<
                    Env,
                    unknown,
                    unknown
                >,
    ...
    close(
        ...
        let closeSelf: Effect.Effect<unknown, never, Env> | undefined =
            undefined;
                ...
                ),
                {
                    ...
                    onSuccess: (out) =>
                        Effect.sync(() => {
                            ...
                            this._currentChannel = core.write(
                                out,
                            ) as core.Primitive;
                        }),
                },
    ...
    runSubexecutor(): ChannelState.ChannelState<unknown, Env> | undefined {
        const subexecutor = this
            ._activeSubexecutor as Subexecutor.Primitive<Env>;
                ...
                onFailure: (cause) => {
                    ...
                    return state === undefined
                        ...
                        : (ChannelState.effectOrUndefinedIgnored(
                              state,
                          ) as Effect.Effect<void, never, Env>);
                },
                onSuccess: (doneValue) => {
                    this.finishWithDoneValue(
                        childExecutor,
            ...
            (emitted) => {
                if (this._closeLastSubstream !== undefined) {
                    ...
                    return pipe(
                        this._executeCloseLastSubstream(closeLastSubstream),
                        Effect.map(() => {
                            const childExecutor: ErasedExecutor<Env> =
                                new ChannelExecutor(
                            ...
                            const [emitSeparator, updatedChildExecutors] =
                                this.applyUpstreamPullStrategy(
                                    ...
                                    subexecutor.activeChildExecutors,
                                    subexecutor.onPull(
                                        upstreamPullRequest.Pulled(emitted),
                                    ),
                                );
...
const runFinalizers = <Env>(
    ...
    return pipe(
        ...
        Effect.flatMap((exit) =>
            Effect.suspend(() => exit as Exit.Exit<unknown>),
        ),
    );
};
                        ...
                        Effect.flatMap((fiber) =>
                            scope
                                ...
                                .pipe(
                                    Effect.zipRight(
                                        restore(
                                            Deferred.await(channelDeferred),
                                        ),
                                    ),
                                ),
                        ),
