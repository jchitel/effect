        ...
        SingleProducerAsyncInput.SingleProducerAsyncInput<Err, Elem, Done>
{
    ...
    get close(): Effect.Effect<unknown> {
        return Effect.fiberIdWith((fiberId) =>
            this.error(Cause.interrupt(fiberId)),
        );
    }
                            ...
                            if (notifyConsumer !== undefined) {
                                return [
                                    ...
                                    (notifyConsumers.length === 0
                                        ? stateEmpty(deferred)
                                        : stateEmit(notifyConsumers)) as State<
                                        Err,
                                        Elem,
                                        Done
                                    >,
    ...
    get take(): Effect.Effect<Exit.Exit<Elem, Either.Either<Done, Err>>> {
        return this.takeWith(
            ...
            (elem) =>
                Exit.succeed(elem) as Exit.Exit<Elem, Either.Either<Done, Err>>,
                ...
                Effect.flatten(
                    Ref.modify(this.ref, (state) => {
                        switch (state._tag) {
                            ...
                            case OP_STATE_ERROR: {
                                return [
                                    Effect.succeed(onError(state.cause)),
                                    state,
                                ];
                            }
                            case OP_STATE_DONE: {
                                return [
                                    Effect.succeed(onDone(state.done)),
                                    state,
                                ];
                            }
                        }
