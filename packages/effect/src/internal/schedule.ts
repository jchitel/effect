...
/** @internal */
class ScheduleDriverImpl<Out, In, R>
    implements Schedule.ScheduleDriver<Out, In, R>
{
    ...
    get reset(): Effect.Effect<void> {
        return ref
            ...
            .pipe(
                core.zipLeft(
                    ref.set(this.iterationMeta, defaultIterationMetadata),
                ),
            );
    }
                    ...
                    core.flatMap((now) =>
                        pipe(
                            ...
                            core.flatMap(([state, out, decision]) => {
                                ...
                                const millis =
                                    Intervals.start(decision.intervals) - now;
                                if (millis <= 0) {
                                    return setState.pipe(
                                        core.zipRight(
                                            updateInfo(
...
>(2, (self, f) =>
    modifyDelayEffect(self, (out, duration) =>
        core.map(f(out), (delay) =>
            Duration.sum(duration, Duration.decode(delay)),
        ),
    ),
);
...
export const collectAllOutputs = <Out, In, R>(
    ...
    reduce(self, Chunk.empty<Out>(), (outs, out) =>
        pipe(outs, Chunk.append(out)),
    );
                            ...
                            : ScheduleDecision.isDone(rDecision)
                              ...
                              : ([
                                    ...
                                    ScheduleDecision.continue(
                                        pipe(
                                            lDecision.intervals,
                                            Intervals.max(rDecision.intervals),
                                        ),
                                    ),
                                ] as const),
                ),
            ...
            if (now < previous[0]) {
                return core.succeed([
                    ...
                    ScheduleDecision.continueWith(
                        Interval.make(previous[1], previous[2]),
                    ),
                ]);
            }
...
export const driver = <Out, In, R>(
    ...
    pipe(
        ref.make<readonly [Option.Option<Out>, any]>([
            Option.none(),
            self.initial,
        ]),
        ...
        core.succeed(
            state
                ? ([
                      ...
                      duration,
                      ScheduleDecision.continueWith(
                          Interval.after(now + durationMillis),
                      ),
                  ] as const)
            ...
            switch (option._tag) {
                case "None": {
                    return [
                        ...
                        n,
                        ScheduleDecision.continueWith(
                            Interval.after(now + intervalMillis),
                        ),
                    ];
                }
                case "Some": {
        ...
        core.flatMap(self.step(now, input, state), ([state, out, decision]) => {
            ...
            return core.map(f(out, delay), (durationInput) => {
                ...
                return [
                    state,
                    out,
                    ScheduleDecision.continueWith(newInterval),
                ] as const;
            });
        }),
    ),
...
export const passthrough = <Out, In, R>(
    ...
    makeWithState(self.initial, (now, input, state) =>
        pipe(
            self.step(now, input, state),
            core.map(
                ([state, _, decision]) => [state, input, decision] as const,
            ),
        ),
    );
...
export const recurUpTo = (
    ...
    const duration = Duration.decode(durationInput);
    return whileOutput(elapsed, (elapsed) =>
        Duration.lessThan(elapsed, duration),
    );
};
                ...
                case "Some": {
                    return core.succeed([
                        ...
                        ScheduleDecision.continueWith(
                            Interval.after(
                                now +
                                    (millis - ((now - option.value) % millis)),
                            ),
                        ),
                    ]);
                }
...
export const repeat_combined = dual<
    ...
    },
    {
        <
            ...
            E,
            R,
            O extends Types.NoExcessProperties<Effect.Repeat.Options<A>, O>,
        >(
            self: Effect.Effect<A, E, R>,
            options: O,
        ): Effect.Repeat.Return<R, E, A, O>;
...
>(2, (self, policy) =>
    retryOrElse_Effect(self, policy, (e, _) => core.fail(e)),
);
...
export const retry_combined: {
    ...
    (
        ...
        return scheduleDefectRefail(
            retry_Effect(self, fromRetryOptions(options)),
        );
    },
);
