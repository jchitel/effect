            ...
            pipe(
                core.fromEffect(
                    Effect.uninterruptible(
                        Effect.tap(acquire, (a) =>
                            Ref.set(ref, (exit) => release(a, exit)),
                        ),
                    ),
                ),
                core.flatMap(use),
        ...
        > = core.readWith({
            onInput: (out) =>
                Option.match(pf(out), {
                    onNone: () => collector,
                    onSome: (out2) =>
                        core.flatMap(core.write(out2), () => collector),
                }),
            onFailure: core.fail,
...
>(
    ...
    > => {
        ...
        > = core.readWith({
            onInput: (inElem) =>
                core.flatMap(core.write(f(inElem)), () => reader),
            onFailure: core.fail,
            onDone: core.succeedNow,
        });
        return core.pipeTo(reader, self);
...
>(
    ...
    > => {
        ...
        > = core.readWith({
            onInput: (outElem) =>
                core.flatMap(core.write(f(outElem)), () => reader),
            onFailure: core.fail,
            onDone: core.succeedNow,
        });
    ...
    > =>
        unwrapScopedWith((scope) =>
            Effect.gen(function* () {
                ...
                const lastDone = yield* Ref.make<Option.Option<OutDone>>(
                    Option.none(),
                );
                const errorSignal = yield* Deferred.make<void>();
                const withPermits = (yield* Effect.makeSemaphore(concurrencyN))
                    .withPermits;
                const pull = yield* toPullIn(
                    core.pipeTo(queueReader, channels),
                    scope,
                );
                ...
                function evaluatePull(
                    ...
                    return pull.pipe(
                        ...
                        Effect.repeat({
                            until: (_): _ is Option.Some<OutDone> =>
                                Option.isSome(_),
                        }),
                        Effect.flatMap((outDone) =>
                            Ref.update(
                                ...
                                Option.match({
                                    ...
                                    onSome: (lastDone) =>
                                        Option.some(f(lastDone, outDone.value)),
                                }),
                            ),
                        ),
                        Effect.catchAllCause((cause) =>
                            Cause.isInterrupted(cause)
                                ...
                                : Queue.offer(
                                      ...
                                      Effect.zipRight(
                                          Deferred.succeed(errorSignal, void 0),
                                      ),
                                      Effect.asVoid,
                                  ),
                        ),
                        ...
                        onSuccess: Either.match({
                            onLeft: (outDone) =>
                                Effect.raceWith(
                                    ...
                                    Effect.interruptible(
                                        withPermits(concurrencyN)(Effect.void),
                                    ),
                                    {
    ...
    > => {
        function merge(scope: Scope.Scope) {
            return Effect.gen(function* () {
                ...
                >();
                const queueReader = fromInput(input);
                const pullL = yield* toPullIn(
                    core.pipeTo(queueReader, self),
                    scope,
                );
                ...
                function handleSide<Err, Done, Err2, Done2>(
                    ...
                    >,
                    fiber: Fiber.Fiber<
                        Either.Either<OutElem | OutElem1, Done2>,
                        Err2
                    >,
                    pull: Effect.Effect<
                        Either.Either<OutElem | OutElem1, Done>,
                    ...
                    > => {
                        ...
                        > {
                            ...
                            return Effect.map(
                                ...
                                Exit.match({
                                    onFailure: (cause) =>
                                        core.fromEffect(
                                            op.f(Exit.failCause(cause)),
                                        ),
                                    onSuccess: Either.match({
                                        onLeft: (done) =>
                                            core.fromEffect(
                                                op.f(Exit.succeed(done)),
                                            ),
                                        onRight: (elem) =>
                                            zipRight(
                                                core.write(elem),
                        ...
                        return Exit.match(exit, {
                            onFailure: (cause) =>
                                onDecision(done(Exit.failCause(cause))),
                            onSuccess: Either.match({
                                onLeft: (z) =>
                ...
                > {
                    switch (state._tag) {
                        case MergeStateOpCodes.OP_BOTH_RUNNING: {
                            const leftJoin = Effect.interruptible(
                                Fiber.join(state.left),
                            );
                            const rightJoin = Effect.interruptible(
                            ...
                            return unwrap(
                                Effect.map(
                                    ...
                                    Exit.match({
                                        onFailure: (cause) =>
                                            core.fromEffect(
                                                state.f(Exit.failCause(cause)),
                                            ),
                                        onSuccess: Either.match({
                                            onLeft: (done) =>
                                                core.fromEffect(
                                                    state.f(Exit.succeed(done)),
                                                ),
                                            onRight: (elem) =>
                                                core.flatMap(
                                                    core.write(elem),
                ...
                return core
                    .fromEffect(
                        Effect.withFiberRuntime<
                            ...
                            Env | Env1
                        >((parent) => {
                            const inherit = Effect.withFiberRuntime<
                                void,
                                never,
                                never
                            >((state) => {
                                (state as any).transferChildren(
                                    (parent as any).scope(),
                                );
                                return Effect.void;
                            });
...
>(
    ...
    > =>
        core.suspend(() => {
            ...
            > = core.readWith({
                onInput: (outElem) =>
                    core.flatMap(core.write(outElem), () => reader),
                onFailure: (outErr) => {
            ...
            return core.pipeTo(
                core.pipeTo(core.pipeTo(self, reader), that),
                writer,
            );
        }),
);
            ...
            Chunk.map(chunk, (str) => {
                if (str.length !== 0) {
                    ...
                    if (midCRLF) {
                        ...
                        stringBuilder =
                            stringBuilder + str.substring(from, str.length);
                    }
                }
            });
...
>(
    ...
    <OutElem, InElem, OutErr, InErr, OutDone, InDone, Env>(
        ...
        Effect.zip(
            Effect.sync(
                () => new executor.ChannelExecutor(self, void 0, identity),
            ),
            Effect.runtime<Env>(),
        ).pipe(
            ...
            Effect.map(([executor]) =>
                Effect.suspend(() =>
                    interpretToPull(
                        executor.run() as ChannelState.ChannelState<
                            OutErr,
                            Env
                        >,
                        executor,
...
const toQueueInternal = <Err, Done, Elem>(
    ...
    return core.readWithCause({
        ...
        onDone: (done) =>
            core.fromEffect(
                Queue.offer(queue, Either.left(Exit.succeed(done))),
            ),
    });
};
    ...
    if (dataFirst) {
        ...
        return acquireUseRelease(
            ...
            ([span, context]) =>
                core.provideContext(
                    self,
                    Context.add(context, tracer.spanTag, span),
                ),
