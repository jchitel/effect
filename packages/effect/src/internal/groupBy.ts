                ...
                Effect.gen(function* () {
                    const decider =
                        yield* Deferred.make<
                            (
                                key: K,
                                value: V,
                            ) => Effect.Effect<Predicate<number>>
                        >();
                    const output = yield* Effect.acquireRelease(
                        Queue.bounded<
                            Exit.Exit<
                                readonly [
                                    K,
                                    Queue.Dequeue<Take.Take<V, E | E2>>,
                                ],
                                Option.Option<E | E2>
                            >
                    ...
                    const add = yield* pipe(
                        ...
                        stream.distributedWithDynamicCallback(
                            options?.bufferSize ?? 16,
                            ([key, value]) =>
                                Effect.flatMap(Deferred.await(decider), (f) =>
                                    f(key, value),
                                ),
    ...
    <A, E, R, K>(
        ...
        const loop = (
            ...
            core.readWithCause({
                ...
                onFailure: (cause) =>
                    core.fromEffect(
                        Queue.offer(outerQueue, take.failCause(cause)),
                    ),
                onDone: () =>
                    core.fromEffect(
                        pipe(
                ...
                Effect.gen(function* () {
                    ...
                    const queue =
                        yield* Queue.unbounded<
                            Take.Take<
                                readonly [K, Queue.Queue<Take.Take<A, E>>],
                                E
                            >
                        >();
