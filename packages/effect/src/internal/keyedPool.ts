...
class KeyedPoolImpl<in K, in out A, out E = never>
...
const makeImpl = <K, A, E, R>(
    ...
    pipe(
        fiberRuntime.all([
            ...
            core.fiberId,
            core.sync(() =>
                MutableRef.make(HashMap.empty<K, MapValue<A, E>>()),
            ),
        ...
        core.map(([context, fiberId, map, scope]) => {
            const getOrCreatePool = (key: K): Effect.Effect<Pool.Pool<A, E>> =>
                core.suspend(() => {
                    let value: MapValue<A, E> | undefined =
                    ...
                    if (value === undefined) {
                        return core.uninterruptibleMask((restore) => {
                            const deferred =
                                core.deferredUnsafeMake<Pool.Pool<A, E>>(
                                    fiberId,
                                );
                            value = new Pending(deferred);
                            let previous: MapValue<A, E> | undefined =
                                undefined;
                            ...
                            switch (previous._tag) {
                                ...
                                case "Pending": {
                                    return restore(
                                        core.deferredAwait(previous.deferred),
                                    );
                                }
                            }
                        });
            ...
            const activePools: Effect.Effect<Array<Pool.Pool<A, E>>> =
                core.suspend(() =>
