...
export const fromFlat = (
    ...
    make({
        load: (config) =>
            core.flatMap(
                fromFlatLoop(flat, Arr.empty(), config, false),
                (chunk) =>
...
export const fromEnv = (
    ...
    const load = <A>(
        ...
        const valueOpt =
            pathString in current
                ? Option.some(current[pathString]!)
                : Option.none();
        return pipe(
            valueOpt,
            core.mapError(() =>
    ...
    return fromFlat(
        makeFlat({ load, enumerateChildren, patch: pathPatch.empty }),
    );
};
                    ...
                    core.flatMap(
                        core.forEachSequential((a) =>
                            pipe(
                                op.mapOrFail(a),
                                core.mapError(
                                    configError.prefixed(
                                        appendConfigPath(prefix, op.original),
                                    ),
                                ),
                            ),
                        ),
                        ...
                        pipe(
                            ...
                            core.flatMap((right) => {
                                if (
                                    ...
                                    return core.fail(
                                        configError.And(left.left, right.left),
                                    );
                                }
                                if (
                                    Either.isLeft(left) &&
