...
export const unsafeMakeZoned = (
    ...
    if (
        self.epochMillis < minEpochMillis ||
        self.epochMillis > maxEpochMillis
    ) {
        throw new IllegalArgumentException(
            `Epoch millis out of range: ${self.epochMillis}`,
        );
    }
    let zone: DateTime.TimeZone;
    if (options?.timeZone === undefined) {
        const offset =
            new Date(self.epochMillis).getTimezoneOffset() * -60 * 1000;
...
export const setZoneNamed: {
    ...
    (
        ...
        Option.map(zoneMakeNamed(zoneId), (zone) =>
            setZone(self, zone, options),
        ),
);
...
const calculateNamedOffset = (
    ...
    const offset =
        zone.format
            .formatToParts(adjustedMillis)
            .find((_) => _.type === "timeZoneName")?.value ?? "";
    if (offset === "GMT") {
        return 0;
    }
