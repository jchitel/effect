...
/** @internal */
export class StreamImpl<out A, out E = never, out R = never>
                ...
                ([
                    ...
                    > = core.readWithCause({
                        ...
                        onFailure: (cause) =>
                            core.fromEffect(
                                ...
                                >(handoff, HandoffSignal.halt(cause)),
                            ),
                        onDone: () =>
                            core.fromEffect(
                                Handoff.offer<
                    ...
                    const scheduledAggregator = (
                        sinkFiber: Fiber.RuntimeFiber<
                            ...
                            E | E2
                        >,
                        scheduleFiber: Fiber.RuntimeFiber<
                            C,
                            Option.Option<never>
                        >,
                        scope: Scope.Scope,
                    ...
                    > = Queue.take(output).pipe(
                        ...
                        Effect.match({
                            ...
                            onSuccess: (chunk) =>
                                core
                                    .write(chunk)
                                    .pipe(core.flatMap(() => loop)),
                        }),
                        channel.unwrap,
                    );
                    return fromChannel(loop).pipe(
                        ensuring(value ?? Effect.void),
                    );
                }),
            ),
        ),
    ...
    Effect.acquireRelease(
        ...
        Effect.map((queue) => {
            const loop: Channel.Channel<
                ...
                unknown,
                E
            > = core.flatMap(Queue.take(queue), (item) =>
                Exit.isExit(item)
                    ? Exit.isSuccess(item)
                        ? core.void
                        : core.failCause(item.cause)
                    : channel.zipRight(
                          core.write(Chunk.unsafeFromArray(item)),
                          loop,
                      ),
            );
            return loop;
    ...
    <A, E, R, A2, E2, R2>(
        ...
        suspend(() => {
            ...
            return new StreamImpl(
                pipe(
                    toChannel(self),
                    channel.pipeToOrFail(buffering(Chunk.empty<A>())),
                ),
            );
        }),
);
    ...
    > =>
        Effect.flatMap(pubsubFromOptions(maximumLag), (pubsub) =>
            pipe(
                ...
                >,
                Effect.tap(() =>
                    Effect.forkScoped(runIntoPubSubScoped(self, pubsub)),
                ),
            ),
        ),
);
        ...
        > =>
            core.readWithCause({
                onInput: (input: Chunk.Chunk<A>) => {
                    ...
                    return core.flatMap(core.write(newChunk), () =>
                        writer(newLast),
                    );
                },
                onFailure: core.failCause,
                    ...
                    core.readWithCause({
                        onInput: (input) =>
                            core.flatMap(
                                core.fromEffect(
                                    Handoff.offer<
                                        Exit.Exit<Elem, Option.Option<Err>>
                                    >(handoff, Exit.succeed(input)),
                                ),
            ...
            channel.unwrapScopedWith((scope) =>
                Effect.all([
                    ...
                    Effect.map(([left, right, latchL, latchR]) => {
                        const pullLeft = Handoff.offer<void>(
                            latchL,
                            void 0,
                        ).pipe(
                            Effect.zipRight(
                                Handoff.take(left).pipe(
                                    Effect.flatMap(identity),
                                ),
                            ),
                        );
                        const pullRight = Handoff.offer<void>(
                            latchR,
                            void 0,
                        ).pipe(
                            Effect.zipRight(
                                Handoff.take(right).pipe(
                                    Effect.flatMap(identity),
                                ),
                            ),
                        );
...
export const concatAll = <A, E, R>(
    ...
    suspend(() =>
        pipe(
            streams,
            Chunk.reduce(empty as Stream.Stream<A, E, R>, (x, y) =>
                concat(y)(x),
            ),
        ),
    );
...
>(
    ...
    <A, E, R>(
        ...
        unwrapScopedWith((scope) =>
            Effect.gen(function* () {
                const handoff =
                    yield* Handoff.make<HandoffSignal.HandoffSignal<A, E>>();
                ...
                > {
                    return Clock.sleep(duration).pipe(
                        ...
                        Effect.forkIn(scope),
                        Effect.map((fiber) =>
                            consumer(DebounceState.previous(fiber)),
                        ),
                    );
                }
                            ...
                            onSome: (elem) =>
                                core
                                    .fromEffect(
                                        Handoff.offer(
                                            handoff,
                                            HandoffSignal.emit(Chunk.of(elem)),
                                        ),
                                    )
                ...
                return scopedWith((scope) =>
                    ...
                    crossRight(
                        new StreamImpl(consumer(DebounceState.notStarted)),
                    ),
                );
            }),
        ),
            ...
            Effect.flatMap((deferred) =>
                pipe(
                    ...
                    distributedWithDynamic({
                        ...
                        decide: (a) =>
                            Effect.flatMap(Deferred.await(deferred), (f) =>
                                f(a),
                            ),
                    }),
                    Effect.flatMap((next) =>
...
>(
    ...
    > =>
        pipe(
            Effect.acquireRelease(
                Ref.make<
                    Map<number, Queue.Queue<Exit.Exit<A, Option.Option<E>>>>
                >(new Map()),
                (ref, _) =>
                    pipe(
                        Ref.get(ref),
                ...
                Effect.gen(function* () {
                    ...
                    const newQueue = yield* Ref.make<
                        Effect.Effect<
                            [
                                number,
                                Queue.Queue<Exit.Exit<A, Option.Option<E>>>,
                            ]
                        >
                    >(
                        pipe(
                            Queue.bounded<Exit.Exit<A, Option.Option<E>>>(
                                maximumLag,
                            ),
                            Effect.flatMap((queue) => {
...
>(
    ...
    <A, E, R>(
        ...
        return new StreamImpl(
            pipe(toChannel(self), channel.pipeToOrFail(loop(n))),
        );
    },
);
...
>(
    ...
    <A, E, R>(
        ...
        return suspend(() => {
            ...
            return new StreamImpl(
                pipe(toChannel(self), channel.pipeToOrFail(reader)),
            );
        });
    },
);
...
>(
    ...
    <A, E, R, E2, R2>(
        ...
        return new StreamImpl(
            pipe(toChannel(self), channel.pipeToOrFail(loop)),
        );
    },
);
                    ...
                    return pipe(
                        ...
                        Option.match({
                            ...
                            onSome: Effect.map((a2) =>
                                core.flatMap(core.write(Chunk.of(a2)), () =>
                                    loop(iterator),
                                ),
                            ),
                        }),
            ...
            return new StreamImpl(
                pipe(
                    ...
                    channel.pipeToOrFail(
                        loop(Chunk.empty<A>()[Symbol.iterator]()),
                    ),
                ),
            );
        }),
                    ...
                    channel.concatMap(toChannel(self), (as) =>
                        pipe(
                            ...
                            Chunk.reduce(
                                ...
                                >,
                                (left, right) =>
                                    pipe(left, channel.zipRight(right)),
                            ),
                        ),
                    ),
                ),
...
export const fromChunkPubSub: {
    ...
    return options?.shutdown
        ? ensuring(stream, PubSub.shutdown(pubsub))
        : stream;
};
...
export const fromTPubSub = <A>(
    ...
    return unwrapScoped(
        Effect.map(TPubSub.subscribeScoped(pubsub), (queue) =>
            fromTQueue(queue),
        ),
    );
};
...
export const fromReadableStreamByob: {
    ...
    const onError = args.length === 1 ? args[0].onError : args[1];
    const allocSize =
        (args.length === 1 ? args[0].bufferSize : args[2]) ?? 4096;
    const releaseLockOnEnd =
        args.length === 1 ? args[0].releaseLockOnEnd === true : false;
    return unwrapScoped(
        Effect.map(
            ...
            (reader) =>
                catchAll(
                    forever(
                        readChunkStreamByobReader(reader, onError, allocSize),
                    ),
...
>(
    ...
    <A, E, R, K>(
        ...
        > =>
            core.readWithCause({
                onInput: (input: Chunk.Chunk<A>) => {
                    const [updatedState, output] = groupAdjacentByChunk(
                        state,
                        input,
                    );
...
>(
    ...
    <A, E, R, S, A2>(
        ...
        return new StreamImpl(
            pipe(toChannel(self), core.pipeTo(accumulator(s))),
        );
    },
);
...
>(
    ...
    <A, E, R, A2, E2, R2>(
        ...
        pipe(
            self,
            mapEffectSequential((a) =>
                pipe(f(a), Effect.map(Chunk.fromIterable)),
            ),
...
export const onEnd: {
    ...
    <A, E, R, _, E2, R2>(
        ...
        concat(self, drain(fromEffect(effect))),
);
...
export const paginateChunkEffect = <S, A, E, R>(
    ...
    f: (
        s: S,
    ) => Effect.Effect<readonly [Chunk.Chunk<A>, Option.Option<S>], E, R>,
): Stream.Stream<A, E, R> => {
    const loop = (
        s: S,
    ): Channel.Channel<
    ...
    > =>
        channel.unwrap(
            Effect.map(f(s), ([chunk, option]) =>
                Option.match(option, {
                    onNone: () =>
                        channel.zipRight(core.write(chunk), core.void),
                    onSome: (s) =>
        ...
        return pipe(
            ...
            Effect.flatMap((deferred) =>
                pipe(
                    Handoff.make<Signal>(),
                    Effect.map((handoff) => {
                        const consumer = sink_.foldSink(
                            sink_.collectLeftover(sink),
                            {
                                onFailure: (error) =>
                                    sink_.zipRight(
                                        sink_.fromEffect(
...
export const partition: {
    ...
    > =>
        partitionEither(
            self,
            (a) =>
                Effect.succeed(predicate(a) ? Either.right(a) : Either.left(a)),
            options,
        ),
);
        ...
        pipe(
            ...
            Effect.flatMap(([queue1, queue2]) =>
                Effect.succeed([
                    filterMap(
                        flattenExitOption(
                            fromQueue(queue1, { shutdown: true }),
                        ),
                        (_) =>
                            Either.match(_, {
                                onLeft: Option.some,
                    ...
                    filterMap(
                        flattenExitOption(
                            fromQueue(queue2, { shutdown: true }),
                        ),
                        (_) =>
                            Either.match(_, {
                                onLeft: Option.none,
        ...
        new StreamImpl(
            channel.unwrapScopedWith((scope) =>
                Layer.buildWithScope(layer, scope).pipe(
                    Effect.map((env) =>
                        pipe(toChannel(self), core.provideContext(env)),
                    ),
                ),
            ),
        ),
        ...
        contextWithStream((env: Context.Context<R2 | Exclude<R, I>>) =>
            flatMap(stream, (service) =>
                pipe(
                    self,
                    provideContext(
                        Context.add(env, tag, service) as Context.Context<
                            R | R2
                        >,
                    ),
                ),
                ...
                while (index < chunk.length) {
                    while (index < chunk.length && result === undefined) {
                        result = rechunker.write(
                            pipe(chunk, Chunk.unsafeGet(index)),
                        );
                        index = index + 1;
                    }
                    if (result !== undefined) {
...
export const repeatEffectChunkOption = <A, E, R>(
    ...
    unfoldChunkEffect(effect, (effect) =>
        pipe(
            Effect.map(effect, (chunk) =>
                Option.some([chunk, effect] as const),
            ),
            Effect.catchAll(
                Option.match({
                    ...
                    Effect.match(driver.next(void 0), {
                        ...
                        onSuccess: (output) =>
                            core.flatMap(process, () =>
                                channel.zipRight(
                                    core.write(
                                        Chunk.of(options.onSchedule(output)),
                                    ),
                                    loop,
                                ),
                            ),
            ...
            return concat(
                ...
                unfoldEffect(a, (s) =>
                    Effect.matchEffect(driver.next(s as A0), {
                        ...
                        onSuccess: () =>
                            Effect.map(
                                provideLastIterationInfo(effect),
                                (nextA) => Option.some([nextA, nextA] as const),
                            ),
                    }),
                ),
            );
            ...
            > = suspend(() => {
                ...
                if (Option.isSome(lastError)) {
                    ...
                    nextStream = scheduleDefectRefail(
                        retry(
                            nextStream,
                            internalExecutionPlan.scheduleFromStep(
                                step,
                                false,
                            )!,
                        ),
                    ...
                    const schedule = internalExecutionPlan.scheduleFromStep(
                        step,
                        true,
                    );
                    nextStream = schedule
...
>(
    ...
    <A, E, R, S, E2, R2>(
        ...
        run(self, sink_.foldEffect(s, cont, f)),
);
        ...
        > = core.readWithCause({
            ...
            onFailure: (cause) =>
                core.fromEffect(
                    Queue.offer(
                        queue,
                        Exit.failCause(Cause.map(cause, Option.some)),
                    ),
                ),
            onDone: () =>
...
>(
    ...
    <A, E, R>(
        ...
        > = core.readWithCause({
            onInput: (input: Chunk.Chunk<A>) =>
                core.flatMap(
                    core.write(InternalTake.chunk(input)),
                    () => writer,
                ),
                    ...
                    switch (option._tag) {
                        ...
                        case "Some": {
                            return pipe(
                                f(option.value, a),
                                Effect.map(
                                    (b) => [Option.some<A | A2>(b), b] as const,
                                ),
                            );
                        }
                    }
                ...
                Effect.matchEffect(driver.next(next.value as A0), {
                    ...
                    onSuccess: () =>
                        Effect.succeed(
                            pipe(
                                core.write(
                                    Chunk.of(options.onElement(next.value)),
                                ),
                                core.flatMap(() => loop(driver, iterator)),
                            ),
                        ),
            ...
            pipe(
                ...
                core.flatMap((driver) =>
                    pipe(
                        toChannel(self),
                        core.pipeTo(
                            loop(driver, Chunk.empty<A>()[Symbol.iterator]()),
                        ),
                    ),
                ),
            ),
        ...
        return new StreamImpl(
            core.suspend(() => {
                ...
                const emitOnStreamEnd = (
                    ...
                    const lastEmitIndex =
                        queueSize - ((queueSize - chunkSize) % stepSize);
                    if (lastEmitIndex === queueSize) {
                        return channelEnd;
                    }
                    const leftovers =
                        queueSize - (lastEmitIndex - chunkSize + stepSize);
                    const lastItems = pipe(
                        queue.toChunk(),
                        Chunk.takeRight(leftovers),
                    );
        ...
        > => {
            ...
            if (Chunk.isEmpty(chunk) || Chunk.isEmpty(remaining)) {
                return loop(
                    pipe(
                        chunk,
                        Chunk.appendAll(pipe(remaining, Chunk.drop(1))),
                    ),
                );
            }
            return pipe(
                ...
                core.flatMap(() =>
                    split(Chunk.empty(), pipe(remaining, Chunk.drop(1))),
                ),
            );
        };
        const loop = (
        ...
        > =>
            core.readWith({
                ...
                onDone: () => {
                    ...
                    if (
                        Option.isNone(
                            pipe(leftovers, Chunk.findFirst(predicate)),
                        )
                    ) {
                        return channel.zipRight(
                            core.write(Chunk.of(leftovers)),
                            core.void,
                        );
                    }
                    return channel.zipRight(
                            ...
                            ([carry, delimiterCursor], a) => {
                                ...
                                if (
                                    ...
                                    Equal.equals(
                                        ...
                                        pipe(
                                            delimiter,
                                            Chunk.unsafeGet(delimiterCursor),
                                        ),
                                    )
                                ) {
                                    if (
                                ...
                                return [
                                    ...
                                    Equal.equals(
                                        a,
                                        pipe(delimiter, Chunk.unsafeGet(0)),
                                    )
                                        ? 1
                                        : 0,
                                ] as const;
...
>(
    ...
    <A, E, R>(
        ...
        if (!Number.isInteger(n)) {
            return die(
                new Cause.IllegalArgumentException(`${n} must be an integer`),
            );
        }
        const loop = (
            n: number,
...
export const tapBoth: {
    ...
    <A, E, R, X1, E2, R2, X2, E3, R3>(
        ...
        options: {
            readonly onFailure: (
                e: Types.NoInfer<E>,
            ) => Effect.Effect<X1, E2, R2>;
            readonly onSuccess: (
                a: Types.NoInfer<A>,
            ) => Effect.Effect<X2, E3, R3>;
        },
    ): Stream.Stream<A, E | E2 | E3, R | R2 | R3>;
} = dual(
    2,
...
export const tapErrorCause: {
    ...
    <A, E, R, X, E2, R2>(
        ...
        > = core.readWithCause({
            ...
            onFailure: (cause) =>
                core.fromEffect(
                    Effect.zipRight(f(cause), Effect.failCause(cause)),
                ),
            onDone: core.succeedNow,
        });
            ...
            flatMap(([queue, deferred]) => {
                ...
                > = core.readWithCause({
                    onInput: (chunk: Chunk.Chunk<A>) =>
                        pipe(
                            core.fromEffect(
                                Queue.offer(queue, InternalTake.chunk(chunk)),
                            ),
                    ...
                    onDone: () =>
                        pipe(
                            core.fromEffect(
                                Queue.offer(queue, InternalTake.end),
                            ),
                            core.foldCauseChannel({
                                onFailure: () => core.void,
                                onSuccess: () => core.void,
...
const throttleEnforceEffect = <A, E, R, E2, R2>(
                    ...
                    Effect.map(([weight, currentTimeMillis]) => {
                        ...
                        if (weight <= available) {
                            return pipe(
                                core.write(input),
                                core.flatMap(() =>
                                    loop(available - weight, currentTimeMillis),
                                ),
                            );
                        }
    ...
    return new StreamImpl(
        pipe(toChannel(self), channel.pipeToOrFail(throttled)),
    );
};
            ...
            onInput: (input: Chunk.Chunk<A>) =>
                pipe(
                    ...
                    Effect.map(([weight, currentTimeMillis]) => {
                        ...
                        const waitCycles =
                            remaining >= 0 ? 0 : -remaining / units;
                        const delay = Duration.millis(
                            Math.max(
                                0,
                                waitCycles * Duration.toMillis(duration),
                            ),
                        );
...
>(
    ...
    <A, E, R>(
        ...
        Effect.tap(
            ...
            (queue) =>
                Effect.forkScoped(runIntoQueueElementsScoped(self, queue)),
        ),
);
                ...
                core.flatMap(([leftover, z]) =>
                    pipe(
                        ...
                        core.flatMap(([done, newLeftovers]) => {
                            const nextChannel =
                                done && Chunk.isEmpty(newLeftovers)
                                    ? core.void
                                    : transducer;
                            return pipe(
            ...
            [Symbol.asyncIterator]() {
                ...
                return {
                    next() {
                        ...
                        return new Promise<IteratorResult<A>>(
                            (resolve, reject) => {
                                currentResolve = resolve;
                    ...
                    return() {
                        ...
                        return Effect.runPromise(
                            Effect.as(Fiber.interrupt(fiber), {
                                done: true,
                                value: void 0,
                            }),
                        );
...
export const unfoldChunkEffect = <S, A, E, R>(
    ...
    f: (
        s: S,
    ) => Effect.Effect<Option.Option<readonly [Chunk.Chunk<A>, S]>, E, R>,
): Stream.Stream<A, E, R> =>
    suspend(() => {
        const loop = (
            s: S,
                    ...
                    return pipe(
                        ...
                        Effect.matchEffect({
                            ...
                            onSuccess: ([leftOption, rightOption]) => {
                                if (
                                    Option.isSome(leftOption) &&
                                    Option.isSome(rightOption)
                                ) {
                                    if (
                    ...
                    return Effect.matchEffect(pullLeft, {
                        ...
                        onSuccess: (leftChunk) =>
                            Chunk.isEmpty(leftChunk)
                                ...
                                : Effect.succeed(
                                      Exit.succeed(
                                          merge(leftChunk, state.rightChunk),
                                      ),
                                  ),
                    });
                }
                    ...
                    return Effect.matchEffect(pullRight, {
                        ...
                        onSuccess: (rightChunk) =>
                            Chunk.isEmpty(rightChunk)
                                ...
                                : Effect.succeed(
                                      Exit.succeed(
                                          merge(state.leftChunk, rightChunk),
                                      ),
                                  ),
                    });
                }
            ...
            while (loop) {
                ...
                if (compare === 0) {
                    ...
                    if (hasNext(leftChunk, leftIndex)) {
                        ...
                        state = ZipAllState.PullLeft(
                            Chunk.unsafeFromArray(rightBuilder),
                        );
                        loop = false;
                    }
                } else {
                    ...
                    if (hasNext(rightChunk, rightIndex)) {
                        ...
                        while (hasNext(leftChunk, leftIndex)) {
                            ...
                            leftTuple = pipe(
                                leftChunk,
                                Chunk.unsafeGet(leftIndex),
                            );
                            leftBuilder.push(leftTuple);
                        }
                        state = ZipAllState.PullRight(
                            Chunk.unsafeFromArray(leftBuilder),
                        );
                        loop = false;
                    }
                }
        ...
        const zip = (
            ...
            switch (either._tag) {
                case "Left": {
                    ...
                    return [
                        output,
                        ZipAllState.PullRight(either.left),
                    ] as const;
                }
                case "Right": {
                    ...
                    return [
                        output,
                        ZipAllState.PullLeft(either.right),
                    ] as const;
                }
            }
        };
    ...
    <A, E, R, A2, E2, R2, A3>(
        ...
        const pull = (
            ...
            Exit.Exit<
                readonly [
                    Chunk.Chunk<A3>,
                    ZipChunksState.ZipChunksState<A, A2>,
                ],
                Option.Option<E | E2>
            >,
            never,
        ...
        const zip = (
            ...
            switch (either._tag) {
                case "Left": {
                    ...
                    return [
                        output,
                        ZipChunksState.PullRight(either.left),
                    ] as const;
                }
                case "Right": {
                    if (Chunk.isEmpty(either.right)) {
                    ...
                    return [
                        output,
                        ZipChunksState.PullLeft(either.right),
                    ] as const;
                }
            }
        };
            ...
            onDone: () =>
                Option.match(last, {
                    ...
                    onSome: (value) =>
                        channel.zipRight(
                            core.write(
                                Chunk.of<readonly [A, Option.Option<A>]>([
                                    value,
                                    Option.none(),
                                ]),
                            ),
...
export const fromEventListener = <A = unknown>(
    ...
    asyncPush<A>(
        ...
        {
            bufferSize:
                typeof options === "object" ? options.bufferSize : undefined,
        },
    );
