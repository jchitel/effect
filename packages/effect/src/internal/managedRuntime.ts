...
export const make = <R, ER>(
    ...
    const runtimeEffect = core.withFiberRuntime<Runtime.Runtime<R>, ER>(
        (fiber) => {
            if (!buildFiber) {
    ...
    const self: ManagedRuntimeImpl<R, ER> = Object.assign(
        Object.create(ManagedRuntimeProto),
        {
            ...
            dispose(): Promise<void> {
                return internalRuntime.unsafeRunPromiseEffect(
                    self.disposeEffect,
                );
            },
            disposeEffect: core.suspend(() => {
                (self as Mutable<ManagedRuntimeImpl<R, ER>>).runtimeEffect =
                    core.die("ManagedRuntime disposed");
            ...
            runFork<A, E>(
                ...
                return self.cachedRuntime === undefined
                    ? internalRuntime.unsafeForkEffect(
                          provide(self, effect),
                          options,
                      )
                    : internalRuntime.unsafeFork(self.cachedRuntime)(
                          effect,
                          options,
                      );
            },
            runSyncExit<A, E>(effect: Effect.Effect<A, E, R>): Exit<A, E | ER> {
                return self.cachedRuntime === undefined
                    ...
                    : internalRuntime.unsafeRunSyncExit(self.cachedRuntime)(
                          effect,
                      );
            },
            ...
            runCallback<A, E>(
                ...
                return self.cachedRuntime === undefined
                    ? internalRuntime.unsafeRunCallback(
                          internalRuntime.defaultRuntime,
                      )(provide(self, effect), options)
