...
export const catchTag: {
    ...
    >(
        ...
        if (args.length === 2) {
            ...
            predicate = (e) => {
                const tag = Predicate.hasProperty(e, "_tag")
                    ? e["_tag"]
                    : undefined;
                if (!tag) return false;
        ...
        return core.catchIf(
            self,
            predicate as Predicate.Refinement<
                E,
                Extract<E, { _tag: K[number] }>
            >,
            f,
        ) as any;
    },
        ...
        core.suspend(() => {
            ...
            while ((next = iterator.next()) && !next.done) {
                ...
                dropping = core.flatMap(dropping, (d) =>
                    core.map(
                        d ? predicate(a, index) : core.succeed(false),
                        (b) => {
                            if (!b) {
                                builder.push(a);
                            }
...
export const eventually = <A, E, R>(
    ...
    core.orElse(self, () =>
        core.flatMap(core.yieldNow(), () => eventually(self)),
    );
    ...
    <A, E, R, E2>(
        ...
        filterOrElse(
            ...
            (
                ...
                never,
                E2 | Cause.NoSuchElementException,
                never
            > =>
...
export const every: {
    ...
    <A, E, R>(
        ...
        core.suspend(() =>
            forAllLoop(elements[Symbol.iterator](), 0, predicate),
        ),
);
...
export const iterate: {
    ...
    core.suspend<A, E, R>(() => {
        if (options.while(initial)) {
            return core.flatMap(options.body(initial), (z2) =>
                iterate(z2, options),
            );
        }
    ...
    <A, E, R, I, S, E1, R1>(
        ...
        core.contextWithEffect((env: Context.Context<R1 | Exclude<R, I>>) =>
            core.flatMap(effect, (service) =>
                core.provideContext(
                    self,
                    pipe(env, Context.add(tag, service)) as Context.Context<
                        R | R1
                    >,
                ),
            ),
...
export const unsafeMakeSpan = <XA, XE>(
    ...
    if (disablePropagation) {
        ...
        const linksFromEnv = FiberRefs.get(
            fiberRefs,
            core.currentTracerSpanLinks,
        );
...
export const endSpan = <A, E>(
    ...
    core.sync(() => {
        ...
        if (core.exitIsFailure(exit) && internalCause.spanToTrace.has(span)) {
            ...
            span.attribute(
                "code.stacktrace",
                internalCause.spanToTrace.get(span)!(),
            );
        }
        span.end(
            timingEnabled ? clock.unsafeCurrentTimeNanos() : bigint0,
    ...
    function (this: any) {
        ...
        return core.suspend(() => {
            ...
            return withSpan(
                core.suspend(() =>
                    internalCall(() =>
                        options.body.apply(this, arguments as any),
                    ),
                ),
                opts.name,
                {
