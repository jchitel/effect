    ...
    match(self, {
        ...
        onSequential: (leftSet, rightSet) =>
            HashSet.flatMap(leftSet, (leftCause) =>
                HashSet.map(rightSet, (rightCause) =>
                    sequential(leftCause, rightCause),
                ),
            ),
        onParallel: (leftSet, rightSet) =>
            HashSet.flatMap(leftSet, (leftCause) =>
                HashSet.map(rightSet, (rightCause) =>
                    parallel(leftCause, rightCause),
                ),
            ),
    });
        ...
        switch (cause._tag) {
            ...
            case OpCodes.OP_FAIL: {
                _parallel = HashSet.add(
                    _parallel,
                    Chunk.make(cause._tag, cause.error),
                );
                if (stack.length === 0) {
                    return [_parallel, _sequential];
                }
            ...
            case OpCodes.OP_DIE: {
                _parallel = HashSet.add(
                    _parallel,
                    Chunk.make(cause._tag, cause.defect),
                );
                if (stack.length === 0) {
                    return [_parallel, _sequential];
                }
    ...
    <C, E, Z>(
        ...
        while (input.length > 0) {
            ...
            switch (cause._tag) {
                ...
                case OpCodes.OP_FAIL: {
                    output.push(
                        Either.right(reducer.failCase(context, cause.error)),
                    );
                    break;
                }
                case OpCodes.OP_DIE: {
                    output.push(
                        Either.right(reducer.dieCase(context, cause.defect)),
                    );
                    break;
                }
                case OpCodes.OP_INTERRUPT: {
                ...
                case "Left": {
                    switch (either.left._tag) {
                        case OP_SEQUENTIAL_CASE: {
                            ...
                            const value = reducer.sequentialCase(
                                context,
                                left,
                                right,
                            );
                        ...
                        case OP_PARALLEL_CASE: {
                            const left = accumulator.pop()!;
                            const right = accumulator.pop()!;
                            const value = reducer.parallelCase(
                                context,
                                left,
                                right,
                            );
