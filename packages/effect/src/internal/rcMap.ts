    ...
    core.withFiberRuntime<RcMap.RcMap<K, A, E>, never, R | Scope.Scope>(
        (fiber) => {
            const context = fiber.getFiberRef(
                core.currentContext,
            ) as Context.Context<R | Scope.Scope>;
...
export const get: {
    <K>(
        key: K,
    ): <A, E>(self: RcMap.RcMap<K, A, E>) => Effect<A, E, Scope.Scope>;
    <K, A, E>(self: RcMap.RcMap<K, A, E>, key: K): Effect<A, E, Scope.Scope>;
} = dual(
    2,
    <K, A, E>(
        self_: RcMap.RcMap<K, A, E>,
        key: K,
...
const getImpl = core.fnUntraced(function* <K, A, E>(
    ...
    if (o._tag === "Some") {
        ...
        entry = yield* self.semaphore.withPermits(1)(
            acquire(self, key, restore),
        );
    }
...
const release = <K, A, E>(
    ...
    coreEffect.clockWith((clock) => {
        ...
        entry.expiresAt =
            ...
            Duration.toMillis(self.idleTimeToLive);
        ...
        return core
            .interruptibleMask(function loop(restore): Effect<void> {
                ...
                return core.flatMap(
                    clock.sleep(Duration.millis(remaining)),
                    () => loop(restore),
                );
            })
            .pipe(
