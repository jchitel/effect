...
/** @internal */
export interface AndThen<in out Input, in out Output, in out Output2>
...
/** @internal */
export interface AddService<in out Env, in out T, in out I>
    extends Differ.Context.Patch<Env, Env | I> {
...
/** @internal */
export interface RemoveService<in out Env, in out I>
...
/** @internal */
export interface UpdateService<in out Env, in out T, in out I>
    ...
    for (const [tag, newService] of newValue.unsafeMap.entries()) {
        if (missingServices.has(tag)) {
            ...
            if (!Equal.equals(old, newService)) {
                patch = combine(makeUpdateService(tag, () => newService))(
                    patch,
                );
            }
        } else {
...
>(
    ...
    <Input, Output>(
        ...
        let patches: Chunk.Chunk<Differ.Context.Patch<unknown, unknown>> =
            Chunk.of(self as Differ.Context.Patch<unknown, unknown>);
        const updatedContext: Map<string, unknown> = new Map(context.unsafeMap);
        while (Chunk.isNonEmpty(patches)) {
            const head: Instruction = Chunk.headNonEmpty(
                patches,
            ) as Instruction;
            ...
            switch (head._tag) {
                ...
                case "AndThen": {
                    patches = Chunk.prepend(
                        Chunk.prepend(tail, head.second),
                        head.first,
                    );
                    break;
                }
