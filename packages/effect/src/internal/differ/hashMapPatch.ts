...
export const diff = <Key, Value, Patch>(options: {
    ...
    const [removed, patch] = HashMap.reduce(
        ...
        ([map, patch], newValue: Value, key: Key) => {
            ...
            switch (option._tag) {
                case "Some": {
                    const valuePatch = options.differ.diff(
                        option.value,
                        newValue,
                    );
                    ...
                    return [
                        ...
                        combine<Key, Value, Patch>(makeUpdate(key, valuePatch))(
                            patch,
                        ),
                    ] as const;
                }
                case "None": {
                    return [
                        map,
                        combine<Key, Value, Patch>(makeAdd(key, newValue))(
                            patch,
                        ),
                    ] as const;
                }
            ...
            switch (head._tag) {
                ...
                case "AndThen": {
                    patches = Chunk.prepend(head.first)(
                        Chunk.prepend(head.second)(tail),
                    );
                    break;
                }
                case "Add": {
