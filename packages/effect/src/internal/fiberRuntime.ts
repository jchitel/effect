...
const drainQueueWhileRunningTable = {
    ...
    [FiberMessage.OP_YIELD_NOW]: (
        ...
        cur: Effect.Effect<any, any, any>,
        _message: FiberMessage.FiberMessage & {
            _tag: FiberMessage.OP_YIELD_NOW;
        },
    ) => {
    ...
    core.forEachSequentialDiscard(
        ...
        (requestsByRequestResolver) =>
            forEachConcurrentDiscard(
                RequestBlock_.sequentialCollectionToChunk(
                    requestsByRequestResolver,
                ),
    ...
    implements Fiber.RuntimeFiber<A, E>
{
    ...
    interruptAsFork(fiberId: FiberId.FiberId): Effect.Effect<void> {
        return core.sync(() =>
            this.tell(
                FiberMessage.interruptSignal(internalCause.interrupt(fiberId)),
            ),
        );
    }
    ...
    refreshRefCache() {
        ...
        this.currentSpan = this.currentContext.unsafeMap.get(
            tracer.spanTag.key,
        );
    }
    ...
    sendInterruptSignalToAllChildren(): boolean {
        ...
        for (const child of this._children) {
            child.tell(
                FiberMessage.interruptSignal(
                    internalCause.interrupt(this.id()),
                ),
            );
            told = true;
        }
                ...
                case OpCodes.OP_ON_SUCCESS_AND_FAILURE: {
                    if (
                        ...
                        return core.exitFailCause(
                            internalCause.stripFailures(cause),
                        );
                    }
                }
                case "OnStep": {
            ...
            return core.exitSucceed(
                core.blocked(
                    ...
                    core.withFiberRuntime<unknown, unknown>((newFiber) => {
                        ...
                        newFiber.currentRuntimeFlags = runtimeFlags_.patch(
                            patchFlags,
    ...
    [OpCodes.OP_UPDATE_RUNTIME_FLAGS](
        ...
        const newRuntimeFlags = runtimeFlags_.patch(
            oldRuntimeFlags,
            updateFlags,
        );
        ...
        if (
            runtimeFlags_.interruptible(newRuntimeFlags) &&
            this.isInterrupted()
        ) {
            ...
            if (op.effect_instruction_i1) {
                ...
                const revertFlags = runtimeFlags_.diff(
                    newRuntimeFlags,
                    oldRuntimeFlags,
                );
    ...
    [OpCodes.OP_ON_SUCCESS](
        op: core.Primitive & { _op: OpCodes.OP_ON_SUCCESS },
    ) {
        this.pushStack(op);
    ...
    [OpCodes.OP_ON_FAILURE](
        op: core.Primitive & { _op: OpCodes.OP_ON_FAILURE },
    ) {
        this.pushStack(op);
        return op.effect_instruction_i0;
    }
    ...
    runLoop(
        ...
        while (true) {
            ...
            if (this._queue.length > 0) {
                cur = this.drainQueueWhileRunning(
                    this.currentRuntimeFlags,
                    cur,
                );
            }
            ...
            try {
                ...
                cur = this.currentTracer.context(() => {
                    if (
                        ...
                        if (level._tag === "Some") {
                            const effectVersion = (cur as core.Primitive)[
                                core.EffectTypeId
                            ]._V;
                            this.log(
                    ...
                    // @ts-expect-error
                    return this[(cur as core.Primitive)._op](
                        cur as core.Primitive,
                    );
                }, this);
                ...
                if (cur === YieldedOp) {
                    ...
                    yieldedOpChannel.currentOp = null;
                    return op._op === OpCodes.OP_SUCCESS ||
...
export const currentMinimumLogLevel: FiberRef.FiberRef<LogLevel.LogLevel> =
    globalValue("effect/FiberRef/currentMinimumLogLevel", () =>
        core.fiberRefUnsafeMake<LogLevel.LogLevel>(
        ...
        return core.flatMap(scope, (scope) =>
            core.scopeAddFinalizerExit(scope, (exit) =>
                core.withFiberRuntime((runtimeFinalizer) => {
                    ...
                    const patchFlags = runtimeFlags_.diff(
                        preFlags,
                        acquireFlags,
                    );
                    const inverseRefs = FiberRefsPatch.diff(
                        acquireRefs,
                        preRefs,
                    );
                ...
                () =>
                    finalizersMaskInternal(
                        ...
                        isRequestBatchingEnabled
                            ...
                            : core.forEachSequential(self, (a, i) =>
                                  restore(f(a, i)),
                              ),
                    ),
                () =>
                            ...
                            if (todos.length > 0) {
                                ...
                                fiber.addObserver((wrapped) => {
                                    ...
                                    if (wrapped._op === "Failure") {
                                        ...
                                        exit =
                                            wrapped.effect_instruction_i0 as any;
                                    }
                                ...
                                return core.async<any, any>((cb) => {
                                    ...
                                    const next = () => {
                                        runFiber(
                                            toPop.pop()!,
                                            true,
                                        ).addObserver(check(index, true));
                                        index++;
                                    };
                                    processingFiber.addObserver(
                                        check(index, false),
                                    );
                                    index++;
        ...
        concurrency.matchSimple(
            ...
            () =>
                RA.fromIterable(elements).reduce(
                    (acc, a, i) =>
                        core.zipWith(acc, a, (acc, a) => f(acc, a, i)),
                ...
                core.flatMap(Ref.make(zero), (acc) =>
                    core.flatMap(
                        forEach(
                            elements,
                            (effect, i) =>
                                core.flatMap(effect, (a) =>
                                    Ref.update(acc, (b) => f(b, a, i)),
                                ),
                            options,
                        ),
                        () => Ref.get(acc),
...
> = <A, E, R>(
    ...
    const inheritAll = (res: readonly [A, Fiber.Fiber<A, E>]) =>
        pipe(internalFiber.inheritAll(res[1]), core.as(res[0]));
    return pipe(
                    ...
                    core.uninterruptibleMask<A, E, R>((restore) =>
                        pipe(
                            ...
                            core.flatMap((head) =>
                                pipe(
                                    effects,
                                    core.forEachSequential((effect) =>
                                        fork(core.interruptible(effect)),
                                    ),
                                    core.map((fibers) =>
                                        Chunk.unsafeFromArray(fibers),
                                    ),
                                    core.map(
                                        (tail) =>
                                            pipe(
            ...
            Option.match(Context.getOption(context, scopeTag), {
                ...
                onSome: (scope) => {
                    ...
                    return core.flatMap(
                        core.scopeFork(
                            scope,
                            ExecutionStrategy.parallelN(parallelism),
                        ),
                        (inner) => scopeExtend(self, inner),
                    );
                },
...
>(
    ...
    (self, that, options) =>
        validateWith(self, that, (a, b) => [a, b], options),
);
...
export const withRuntimeFlagsScoped = (
    ...
    return pipe(
        ...
        core.flatMap((runtimeFlags) => {
            const updatedRuntimeFlags = runtimeFlags_.patch(
                runtimeFlags,
                update,
            );
            ...
            return pipe(
                ...
                core.zipRight(
                    addFinalizer(() =>
                        core.updateRuntimeFlags(revertRuntimeFlags),
                    ),
                ),
                core.asVoid,
            );
...
const ScopeImplProto: Omit<ScopeImpl, "strategy" | "state"> = {
    ...
    fork(this: ScopeImpl, strategy) {
        return core.sync(() => {
            ...
            const key = {};
            const fin = (exit: Exit.Exit<unknown, unknown>) =>
                newScope.close(exit);
    ...
    close(this: ScopeImpl, exit) {
        return core.suspend(() => {
            ...
            return executionStrategy.isSequential(this.strategy)
                ? pipe(
                      core.forEachSequential(finalizers, (fin) =>
                          core.exit(fin(exit)),
                      ),
    ...
    [T] extends [ReadonlyArray<infer U>]
        ? number extends T["length"]
            ? Array<
              ...
              >
            : {
                  -readonly [K in keyof T]: T[K] extends Fiber.Fiber<
                      infer A,
                      infer E
                  >
                      ? Exit.Exit<A, E>
...
export const fiberAll = <A, E>(
    ...
    const _fiberAll = {
        ...
        await: core.exit(
            forEachParUnbounded(
                fibers,
                (fiber) => core.flatten(fiber.await),
                false,
            ),
        ),
        children: core.map(
        ...
        poll: core.map(
            ...
            RA.reduceRight(
                Option.some<Exit.Exit<Array<A>, E>>(
                    core.exitSucceed(new Array()),
                ),
                (optionB, optionA) => {
                    switch (optionA._tag) {
                        case "None": {
            ...
            core.matchCauseEffect(restore(self), {
                onFailure: (cause1) =>
                    core.matchCauseEffect(finalizer, {
                        onFailure: (cause2) =>
                            core.failCause(
                                internalCause.sequential(cause1, cause2),
                            ),
...
export const makeSpanScoped = (
    ...
    return core.uninterruptible(
        core.withFiberRuntime((fiber) => {
            ...
            const timingEnabled = fiber.getFiberRef(
