...
export interface Constant
    extends Op<
        OpCodes.OP_CONSTANT,
        {
            ...
            parse(
                text: string,
            ): Either.Either<unknown, ConfigError.ConfigError>;
        }
    > {}
        ...
        Either.try({
            try: () => new URL(text),
                ...
                configError.InvalidData(
                    [],
                    `Expected an URL value but received ${text}`,
                ),
        }),
    );
    return name === undefined ? config : nested(config, name);
    ...
    const config = mapOrFail(string(), (value) => {
        ...
        return level === undefined
            ? Either.left(
                  configError.InvalidData(
                      [],
                      `Expected a log level but received ${value}`,
                  ),
              )
            : Either.right(level);
    });
...
export const duration = (name?: string): Config.Config<Duration.Duration> => {
    const config = mapOrFail(string(), (value) => {
        ...
        return Either.fromOption(duration, () =>
            configError.InvalidData(
                [],
                `Expected a duration but received ${value}`,
            ),
        );
    });
    return name === undefined ? config : nested(config, name);
                ...
                .pipe(
                    Either.mapLeft((brandErrors) =>
                        configError.InvalidData(
                            [],
                            brandErrors
                                .map((brandError) => brandError.message)
                                .join("\n"),
                        ),
                    ),
                ),
