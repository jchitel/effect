...
/** @internal */
export interface Blocked<out E = any, out A = any>
    extends Op<
        "Blocked",
        {
...
export interface OnStep
    extends Op<"OnStep", { readonly effect_instruction_i0: Primitive }> {}
...
export const isEffect = (
    u: unknown,
): u is Effect.Effect<unknown, unknown, unknown> =>
...
export const die = (defect: unknown): Effect.Effect<never> =>
    isObject(defect) && !(internalCause.spanSymbol in defect)
        ? withFiberRuntime((fiber) =>
              failCause(
                  internalCause.die(
                      capture(defect, currentSpanFromFiber(fiber)),
                  ),
              ),
          )
...
export const fail = <E>(error: E): Effect.Effect<never, E> =>
    isObject(error) && !(internalCause.spanSymbol in error)
        ? withFiberRuntime((fiber) =>
              failCause(
                  internalCause.fail(
                      capture(error, currentSpanFromFiber(fiber)),
                  ),
              ),
          )
...
export const matchCauseEffect: {
    ...
    <A, E, R, A2, E2, R2, A3, E3, R3>(
        ...
        const effect = new EffectPrimitive(
            OpCodes.OP_ON_SUCCESS_AND_FAILURE,
        ) as any;
        effect.effect_instruction_i0 = self;
...
export const intoDeferred: {
    ...
    <A, E, R>(
        ...
        uninterruptibleMask((restore) =>
            flatMap(exit(restore(self)), (exit) =>
        ...
        matchCauseEffect(self, {
            onFailure: (cause) => {
                ...
                if (defects.length > 0) {
                    return failCause(
                        Option.getOrThrow(
                            internalCause.keepDefectsAndElectFailures(cause),
                        ),
                    );
                }
                return that();
...
export const fiberRefModifySome = <A, B>(
    ...
    fiberRefModify(self, (v) =>
        Option.getOrElse(f(v), () => [def, v] as const),
    );
...
/** @internal */
export class RequestResolverImpl<in A, out R>
                ...
                Option.match({
                    onNone: () => {
                        const interrupts = Arr.fromIterable(
                            ...
                            Arr.fromIterable(FiberId.ids(fiberId)).map(
                                (id) => `#${id}`,
                            ),
                        );
...
export const isInterruptedException = (
    u: unknown,
): u is Cause.InterruptedException =>
    ...
    return pipe(
        ...
        Arr.reduce(
            pipe(
                ...
                exitMap<A, Chunk.Chunk<A>>(Chunk.of),
            ),
            (accumulator, current) =>
                pipe(
                    accumulator,
