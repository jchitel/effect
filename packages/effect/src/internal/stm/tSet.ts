...
>(2, (self, other) =>
    core.flatMap(toHashSet(other), (values) =>
        removeIf(self, (value) => HashSet.has(values, value), {
            discard: true,
        }),
    ),
);
...
export const fromIterable = <A>(iterable: Iterable<A>): STM.STM<TSet.TSet<A>> =>
    core.map(
        tMap.fromIterable(
            Array.from(iterable).map((a): [A, void] => [a, void 0]),
        ),
...
>(2, (self, other) =>
    core.flatMap(toHashSet(other), (values) =>
        pipe(
            self,
            retainIf((value) => pipe(values, HashSet.has(value)), {
                discard: true,
            }),
        ),
    ),
