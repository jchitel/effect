...
class TMapImpl<in out K, in out V> implements TMap.TMap<K, V> {
    ...
    constructor(
        readonly tBuckets: TRef.TRef<
            TArray.TArray<Chunk.Chunk<readonly [K, V]>>
        >,
...
>(2, (self, f) =>
    reduceSTM(self, void 0 as void, (_, value, key) =>
        stm.asVoid(f(key, value)),
    ),
);
    ...
    const resize = (
        ...
        while (index < capacity) {
            ...
            while ((next = iterator.next()) && !next.done) {
                ...
                newBuckets[newIndex] = Chunk.prepend(
                    newBuckets[newIndex],
                    next.value,
                );
            }
...
>(
    ...
    <K, V, A>(
        ...
        pipe(
            core.effect<never, Option.Option<RA.NonEmptyArray<A>>>(
                (journal) => {
                    const buckets = tRef.unsafeGet(self.tBuckets, journal);
                    const capacity = buckets.chunk.length;
            ...
            while ((next = iterator.next()) && !next.done) {
                ...
                if (
                    !Chunk.some(newBucket, (entry) =>
                        Equal.equals(entry[0], newPair[0]),
                    )
                ) {
                ...
                core.effect<never, void>((journal) => {
                    ...
                    while ((next = iterator.next()) && !next.done) {
                        ...
                        if (
                            ...
                            newBuckets[index] = Chunk.prepend(
                                newBucket,
                                next.value,
                            );
                            newSize = newSize + 1;
...
>(3, (self, key, f) =>
    core.flatMap(get(self, key), (option) =>
        Option.match(f(option), {
            ...
            onSome: (value) =>
                stm.as(set(self, key, value), Option.some(value)),
        }),
    ),
);
