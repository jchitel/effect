...
class TPubSubImpl<in out A> implements TPubSub.TPubSub<A> {
    ...
    offer(value: A): STM.STM<boolean> {
        return core.withSTMRuntime((runtime) => {
            ...
            const currentPubSubSize = tRef.unsafeGet(
                this.pubsubSize,
                runtime.journal,
            );
            ...
            switch (this.strategy._tag) {
                ...
                case OpCodes.OP_SLIDING_STRATEGY: {
                    if (this.requestedCapacity > 0) {
                        let currentPublisherHead: TRef.TRef<
                            Node<A> | undefined
                        > = tRef.unsafeGet(this.publisherHead, runtime.journal);
                        let loop = true;
                        while (loop) {
                            const node = tRef.unsafeGet(
    ...
    shutdown: STM.STM<void> = core.effect<never, void>((journal) => {
        ...
        if (currentPublisherTail !== undefined) {
            ...
            const currentSubscribers = tRef.unsafeGet(
                this.subscribers,
                journal,
            );
...
class TPubSubSubscriptionImpl<in out A> implements TQueue.TDequeue<A> {
    ...
    isShutdown: STM.STM<boolean> = core.effect<never, boolean>((journal) => {
        const currentSubscriberHead = tRef.unsafeGet(
            this.subscriberHead,
            journal,
        );
            ...
            pipe(
                ...
                stm.tap(([subscriberHead, _, currentSubscribers]) =>
                    pipe(
                        ...
                        tRef.set(
                            pipe(
                                currentSubscribers as any,
                                HashSet.add(subscriberHead),
                            ),
                        ),
                    ),
                ),
...
/** @internal */
export const capacity = <A>(self: TPubSub.TPubSub<A>): number =>
    self.capacity();
...
export const publishAll = dual<
    <A>(
        iterable: Iterable<A>,
    ) => (self: TPubSub.TPubSub<A>) => STM.STM<boolean>,
...
export const subscribeScoped = <A>(
    ...
    Effect.acquireRelease(subscribe(self), (dequeue) =>
