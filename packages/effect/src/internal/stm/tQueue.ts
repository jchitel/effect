    ...
    offerAll(iterable: Iterable<A>): STM.STM<boolean> {
        return core.withSTMRuntime((runtime) => {
            ...
            switch (this.strategy._tag) {
                ...
                case OpCodes.OP_DROPPING_STRATEGY: {
                    const forQueue = as.slice(
                        0,
                        this.requestedCapacity - queue.length,
                    );
                    tRef.unsafeSet(
                        this.ref,
                        [...queue, ...forQueue],
                ...
                case OpCodes.OP_SLIDING_STRATEGY: {
                    ...
                    const toDrop =
                        queue.length + forQueue.length - this.requestedCapacity;
...
export const offerAll = dual<
    <A>(
        iterable: Iterable<A>,
    ) => (self: TQueue.TEnqueue<A>) => STM.STM<boolean>,
