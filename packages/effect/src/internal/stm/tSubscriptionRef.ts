...
class TDequeueMerge<A> implements TQueue.TDequeue<A> {
    ...
    takeAll: STM.STM<Array<A>> = STM.gen(this, function* () {
        return [
            ...(yield* this.first.takeAll),
            ...(yield* this.second.takeAll),
        ];
    });
    ...
    takeUpTo(max: number): STM.STM<Array<A>> {
        return STM.gen(this, function* () {
            ...
            return [
                ...first,
                ...(yield* this.second.takeUpTo(max - first.length)),
            ];
        });
    }
    ...
    isShutdown: STM.STM<boolean> = STM.gen(this, function* () {
        return (
            (yield* this.first.isShutdown) && (yield* this.second.isShutdown)
        );
    });
...
/** @internal */
class TSubscriptionRefImpl<in out A>
    implements TSubscriptionRef.TSubscriptionRef<A>
{
    ...
    get changes(): STM.STM<TQueue.TDequeue<A>> {
        return STM.gen(this, function* () {
            ...
            return new TDequeueMerge(
                first,
                yield* TPubSub.subscribe(this.pubsub),
            );
        });
    }
