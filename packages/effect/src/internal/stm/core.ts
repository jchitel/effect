...
/** @internal */
interface STMOnRetry
    extends Op<
        OpCodes.OP_ON_RETRY,
        {
            readonly effect_instruction_i1: STM.STM<unknown, unknown, unknown>;
            readonly effect_instruction_i2: () => STM.STM<
                unknown,
                unknown,
                unknown
            >;
...
export const unsafeAtomically = <A, E, R>(
    ...
    withFiberRuntime((state) => {
        ...
        const commitResult = tryCommitSync(
            fiberId,
            self,
            env,
            scheduler,
...
const tryCommitSync = <A, E, R>(
    ...
    if (
        analysis === Journal.JournalAnalysisReadWrite &&
        TExit.isSuccess(tExit)
    ) {
        Journal.commitJournal(journal);
    } else if (analysis === Journal.JournalAnalysisInvalid) {
        throw new Error(
                        ...
                        case "Commit": {
                            switch (current.effect_instruction_i0) {
                                ...
                                case OpCodes.OP_ON_RETRY: {
                                    ...
                                    curr =
                                        current.effect_instruction_i1 as Primitive;
                                    break;
                                }
