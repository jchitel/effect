    ...
    const swap = (
        ...
        pipe(
            ...
            core.flatMap((tmp) =>
                pipe(
                    buffer,
                    tArray.updateSTM(index1, () =>
                        pipe(buffer, tArray.get(index2)),
                    ),
                    core.zipRight(
                        pipe(
                            buffer,
...
class TRandomImpl implements TRandom.TRandom {
    ...
    constructor(readonly state: TRef.TRef<Random.PCGRandomState>) {
        ...
        this.nextBoolean = core.flatMap(this.next, (n) =>
            core.succeed(n > 0.5),
        );
    ...
    nextRange(min: number, max: number): STM.STM<number> {
        return core.flatMap(this.next, (n) =>
            core.succeed((max - min) * n + min),
        );
    }
