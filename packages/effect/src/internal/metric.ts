...
> => {
    ...
    > => {
        ...
        hook = globalMetricRegistry.get(
            metricKey.taggedWithLabels(key, extraTags),
        );
...
export const set = dual<
    {
        (
            value: number,
        ): (self: Metric.Metric.Gauge<number>) => Effect.Effect<void>;
        (
            value: bigint,
        ): (self: Metric.Metric.Gauge<bigint>) => Effect.Effect<void>;
    },
    {
...
>(
    ...
    <A, E extends In2, R, Type, In, Out, In2>(
        ...
        const updater = (error: E): Effect.Effect<void> =>
            update(metric, f(error));
...
>(
    ...
    <A extends In2, E, R, Type, In, Out, In2>(
        ...
        const updater = (value: A): Effect.Effect<void> =>
            update(metric, f(value));
