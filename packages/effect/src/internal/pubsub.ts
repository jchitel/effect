...
class BoundedPubSubArb<in out A> implements AtomicPubSub<A> {
    ...
    subscribe(): Subscription<A> {
        ...
        return new BoundedPubSubArbSubscription(
            this,
            this.publisherIndex,
            false,
        );
...
class BoundedPubSubPow2<in out A> implements AtomicPubSub<A> {
    ...
    subscribe(): Subscription<A> {
        this.subscriberCount += 1;
        return new BoundedPubSubPow2Subscription(
            this,
            this.publisherIndex,
            false,
        );
    ...
    implements Queue.Dequeue<A>
{
    ...
    get size(): Effect.Effect<number> {
        return core.suspend(() =>
            MutableRef.get(this.shutdownFlag)
                ? core.interrupt
                : core.succeed(
                      this.subscription.size() + this.replayWindow.remaining,
                  ),
        );
    }
        ...
        return core.uninterruptible(
            core.withFiberRuntime<void>((state) => {
                ...
                return pipe(
                    ...
                    core.whenEffect(
                        core.deferredSucceed(this.shutdownHook, void 0),
                    ),
                    core.asVoid,
                );
            }),
    ...
    get takeAll(): Effect.Effect<Chunk.Chunk<A>> {
        return core.suspend(() => {
            ...
            if (this.replayWindow.remaining > 0) {
                return core.succeed(
                    Chunk.appendAll(this.replayWindow.takeAll(), as),
                );
            }
    ...
    takeBetween(min: number, max: number): Effect.Effect<Chunk.Chunk<A>> {
        return core.suspend(() =>
            takeRemainderLoop(this, min, max, Chunk.empty()),
        );
    }
}
    ...
    handleSurplus(
        ...
        return core.withFiberRuntime((state) => {
            ...
            return pipe(
                ...
                core.onInterrupt(() =>
                    core.sync(() => this.unsafeRemove(deferred)),
                ),
            );
        });
    }
