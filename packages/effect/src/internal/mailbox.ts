    ...
    implements Api.Mailbox<A, E>
{
    ...
    shutdown: Effect<boolean> = core.sync(() => {
        ...
        this.finalize(
            this.state._tag === "Open" ? core.exitVoid : this.state.exit,
        );
        if (offers.size > 0) {
                    ...
                    entry.resume(
                        core.exitSucceed(
                            Chunk.unsafeFromArray(
                                entry.remaining.slice(entry.offset),
                            ),
                        ),
                    );
                }
    ...
    private offerRemainingArray(remaining: Array<A>) {
        return core.asyncInterrupt<Chunk.Chunk<A>>((resume) => {
            if (this.state._tag !== "Open") {
                return resume(
                    core.exitSucceed(Chunk.unsafeFromArray(remaining)),
                );
            }
