        ...
        return core.suspend(() => {
            ...
            const pTakers =
                this.queue.length() === 0
                    ...
                    : Arr.empty;
            const [forTakers, remaining] = pipe(
                values,
                Arr.splitAt(pTakers.length),
            );
    ...
    get takeAll(): Effect.Effect<Chunk.Chunk<A>> {
        return core.suspend(() => {
            return MutableRef.get(this.shutdownFlag)
                ? core.interrupt
                : core.sync(() => {
                      const values = this.queue.pollUpTo(
...
/** @internal */
export class BackingQueueFromMutableQueue<in out A>
