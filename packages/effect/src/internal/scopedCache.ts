    ...
    Exit.matchEffect(self.exit, {
        ...
        onSuccess: ([value]) =>
            fiberRuntime.acquireRelease(
                core.as(
                    core.sync(() =>
                        MutableRef.incrementAndGet(self.ownerCount),
                    ),
                    value,
                ),
                () => releaseOwner(self),
    ...
    Exit.matchEffect(self.exit, {
        ...
        onSuccess: ([, finalizer]) =>
            core.flatMap(
                ...
                (numOwner) =>
                    effect.when(finalizer(Exit.void), () => numOwner === 0),
            ),
    });
...
> implements ScopedCache.ScopedCache<Key, Value, Error>
{
    ...
    constructor(
        ...
        readonly scopedLookup: ScopedCache.Lookup<
            Key,
            Value,
            Error,
            Environment
    ...
    getOption(
        key: Key,
    ): Effect.Effect<Option.Option<Value>, Error, Scope.Scope> {
        return core.suspend(() =>
    ...
    getOptionComplete(
        ...
        return core.suspend(() =>
            Option.match(MutableHashMap.get(this.cacheState.map, key), {
                ...
                onSome: (value) =>
                    core.flatten(
                        this.resolveMapValue(value, true),
                    ) as Effect.Effect<
                        Option.Option<Value>,
                        never,
                        Scope.Scope
    ...
    entryStats(key: Key): Effect.Effect<Option.Option<Cache.EntryStats>> {
        return core.sync(() => {
            ...
            switch (value._tag) {
                ...
                case "Refreshing": {
                    return Option.some(
                        cache_.makeEntryStats(
                            value.complete.entryStats.loadedMillis,
                        ),
                    );
                }
            }
    ...
    get invalidateAll(): Effect.Effect<void> {
        return fiberRuntime.forEachConcurrentDiscard(
            HashSet.fromIterable(
                Array.from(this.cacheState.map).map(([key]) => key),
            ),
    ...
    lookupValueOf(
        ...
        return pipe(
            ...
            core.flatMap(([exit, release]) => {
                ...
                const expiredAt =
                    now + Duration.toMillis(this.timeToLive(exit));
                switch (exit._tag) {
                    case "Success": {
                        const exitWithFinalizer: Exit.Exit<
                            [Value, Scope.Scope.Finalizer]
                    ...
                    case "Failure": {
                        const completedResult = complete<Key, Value, Error>(
                            cache_.makeMapKey(key),
                            exit as Exit.Exit<
                                readonly [Value, Scope.Scope.Finalizer],
                                Error
                            >,
                            MutableRef.make(0),
                            cache_.makeEntryStats(now),
