        ...
        (ds) =>
            core.fiberIdWith((id) => {
                ...
                return core.fiberRefGetWith(
                    ...
                    (cacheEnabled) => {
                        if (cacheEnabled) {
                            const cached: Effect.Effect<any, any> =
                                core.fiberRefGetWith(currentCache, (cache) =>
                                    core.flatMap(
                                        cache.getEither(proxy),
                                        (orNew) => {
                                            switch (orNew._tag) {
                                                case "Left": {
                                                    if (
                                                        orNew.left.listeners
                                                            .interrupted
                                                    ) {
                                                        return core.flatMap(
