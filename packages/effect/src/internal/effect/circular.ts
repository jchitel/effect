...
export const cached: {
    ...
    <A, E, R>(
        ...
        core.map(
            cachedInvalidateWithTTL(self, timeToLive),
            (tuple) => tuple[0],
        ),
);
        ...
        return core.flatMap(core.context<R>(), (env) =>
            core.map(
                ...
                (cache) =>
                    [
                        core.provideContext(
                            getCachedValue(self, duration, cache),
                            env,
                        ),
...
> => {
    ...
    return pipe(
        ...
        core.map((deferred) =>
            Option.some([start + timeToLiveMillis, deferred]),
        ),
    );
};
            ...
            core.flatMap((time) =>
                updateSomeAndGetEffectSynchronized(cache, (option) => {
                    switch (option._tag) {
                        case "None": {
                            return Option.some(
                                computeCachedValue(self, timeToLive, time),
                            );
                        }
                        case "Some": {
            ...
            (ref) => (a: A) =>
                pipe(
                    ref.modifyEffect((map) => {
                        const result = pipe(
                            map,
                            MutableHashMap.get(new Key(a, eq)),
                        );
    ...
    modifyEffect<B, E, R>(
        ...
        return this.withLock(
            pipe(
                ...
                core.flatMap(([b, a]) =>
                    core.as(internalRef.set(this.ref, a), b),
                ),
            ),
        );
    }
...
export const bindAll: {
    ...
    ] extends [Effect.Effect<infer Success, infer Error, infer Context>]
        ? Effect.Effect<
              {
