            ...
            if (leftovers.length === 0) {
                return collectAllWhileReader(
                    ...
                    pipe(
                        ...
                        Chunk.appendAll(Chunk.unsafeFromArray(collected)),
                    ),
                );
            }
            return pipe(
                    ...
                    ? pipe(
                          core.fromEffect(
                              Ref.set(
                                  leftoversRef,
                                  Chunk.flatten(
                                      leftovers as Chunk.Chunk<Chunk.Chunk<In>>,
                                  ),
                              ),
                          ),
                          core.flatMap(() =>
...
export const filterInput: {
    <In, In1 extends In, In2 extends In1>(
        ...
        self: Sink.Sink<A, In, L, E, R>,
    ) => Sink.Sink<A, In2, L, E, R>;
    <In, In1 extends In>(
        f: Predicate<In1>,
    ): <A, L, E, R>(
        self: Sink.Sink<A, In, L, E, R>,
    ) => Sink.Sink<A, In1, L, E, R>;
...
export const fold = <S, In>(
    ...
    suspend(() => new SinkImpl(foldReader(s, contFn, f)));
                        ...
                        return core.flatMap(
                            ...
                            ([newLeftovers, z1]) =>
                                pipe(
                                    ...
                                    channel.zipRight(
                                        channel.writeChunk(newLeftovers),
                                    ),
                                    channel.as(z1),
                                ),
                        );
    ...
    return pipe(
        ...
        Effect.flatMap((s1) =>
            contFn(s1)
                ...
                : Effect.succeed([
                      s1,
                      Option.some(pipe(chunk, Chunk.drop(index + 1))),
                  ]),
        ),
    );
};
...
> =>
    core.readWith({
        onInput: (input: Chunk.Chunk<In>) => {
            const [nextS, nextCost, nextDirty, leftovers] =
...
export const forEach = <In, X, E, R>(
    ...
    > = core.readWithCause({
        onInput: (input: Chunk.Chunk<In>) =>
            pipe(
                core.fromEffect(
                    Effect.forEach(input, (v) => f(v), { discard: true }),
                ),
                ...
                onFailure: ([either, leftovers]) =>
                    Either.match(either, {
                        ...
                        onRight: (z) =>
                            pipe(
                                core.write(leftovers),
                                channel.zipRight(core.succeedNow(z)),
                            ),
                    }),
        ...
        function race(scope: Scope.Scope) {
            return Effect.gen(function* () {
                ...
                const writer = channel.fromQueue(subscription1).pipe(
                    ...
                    channel.zipLeft(
                        core.fromEffect(Queue.shutdown(subscription1)),
                    ),
                    channel.mergeWith({
                        other: channel
    ...
    pipe(
        ...
        flatMap((acc) => {
            ...
            return new SinkImpl(
                pipe(
                    core.write(leftover),
                    channel.zipRight(core.succeedNow(taken)),
                ),
            );
        }),
    );
...
export const unwrapScoped = <A, In, L, E, R>(
    ...
    new SinkImpl(
        channel.unwrapScoped(
            effect.pipe(Effect.map((sink) => toChannel(sink))),
        ),
    );
        ...
        options?.concurrent
            ? raceWith(self, {
                  ...
                  onSelfDone: Exit.match({
                      onFailure: (cause) =>
                          mergeDecision.Done(Effect.failCause(cause)),
                      onSuccess: (leftZ) =>
                          mergeDecision.Await<R | R2, E2, A2, E | E2, A3>(
                              Exit.match({
                                  onFailure: Effect.failCause,
                                  onSuccess: (rightZ) =>
                                      Effect.succeed(f(leftZ, rightZ)),
                              }),
                          ),
                  }),
