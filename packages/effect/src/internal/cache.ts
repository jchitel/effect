...
class CacheImpl<in out Key, in out Value, in out Error>
    implements Cache.Cache<Key, Value, Error>
{
        ...
        return core.sync(() => {
            ...
            if (Option.isSome(option)) {
                switch (option.value._tag) {
                    ...
                    case "Refreshing": {
                        const loaded =
                            option.value.complete.entryStats.loadedMillis;
                        return Option.some(makeEntryStats(loaded));
                    }
                }
            }
    ...
    getEither(key: Key): Effect.Effect<Either.Either<Value, Value>, Error> {
        return core.suspend(
            (): Effect.Effect<Either.Either<Value, Value>, Error> => {
                ...
                let deferred: Deferred.Deferred<Value, Error> | undefined =
                    undefined;
    ...
    refresh(key: Key): Effect.Effect<void, Error> {
        return effect.clockWith((clock) =>
            core.suspend(() => {
                const k = key;
                const deferred: Deferred.Deferred<Value, Error> =
                ...
                if (value === undefined) {
                    ...
                    switch (value._tag) {
                        case "Complete": {
                            if (
                                this.hasExpired(clock, value.timeToLiveMillis)
                            ) {
        ...
        return effect.clockWith((clock) =>
            core.sync(() => {
                ...
                const mapValue = complete(
                    ...
                    now +
                        Duration.toMillis(
                            Duration.decode(this.timeToLive(lookupResult)),
                        ),
                );
                MutableHashMap.set(
                    this.cacheState.map,
    ...
    get values(): Effect.Effect<Array<Value>> {
        return core.sync(() => {
            ...
            for (const entry of this.cacheState.map) {
                if (
                    entry[1]._tag === "Complete" &&
                    entry[1].exit._tag === "Success"
                ) {
        ...
        return effect.clockWith((clock) => {
            switch (value._tag) {
                ...
                case "Refreshing": {
                    ...
                    this.trackHit();
                    if (
                        this.hasExpired(clock, value.complete.timeToLiveMillis)
                    ) {
                        if (ignorePending) {
                            return core.succeed(Option.none<Value>());
                        }
                ...
                return pipe(
                    ...
                    core.flatMap((exit) => {
                        ...
                        const value = complete(
                            ...
                            now +
                                Duration.toMillis(
                                    Duration.decode(this.timeToLive(exit)),
                                ),
                        );
