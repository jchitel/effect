                          ...
                          Array.from(
                              Chunk.map(
                                  RA.reduce(
                                      requests,
                                      Chunk.empty<
                                          Chunk.Chunk<Request.Entry<A>>
                                      >(),
                                      (acc, chunk) =>
                                          Chunk.appendAll(
                                              acc,
                                              Chunk.chunksOf(
                                                  Chunk.unsafeFromArray(chunk),
                                                  n,
                                              ),
                                          ),
                                  ),
...
export const fromEffect = <R, A extends Request.Request<any, any>>(
    ...
    makeBatched((requests: RA.NonEmptyArray<A>) =>
        Effect.forEach(
            requests,
            (a) =>
                Effect.flatMap(Effect.exit(f(a)), (e) => complete(a, e as any)),
        ...
        Fns extends {
            readonly [Tag in A["_tag"]]: [
                ...
                ? Req extends Request.Request<infer ReqA, infer ReqE>
                    ? (
                          requests: Array<Req>,
                      ) => Effect.Effect<Iterable<ReqA>, ReqE, any>
                    : never
                : never;
        },
    >(
