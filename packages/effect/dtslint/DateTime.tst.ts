...
describe("DateTime", () => {
    it("min", () => {
        expect(DateTime.min(utc, zoned)).type.toBe<
        ...
        expect(
            dateTime.pipe(DateTime.min(zoned)),
        ).type.toBe<DateTime.DateTime>();
        expect(dateTime.pipe(DateTime.min(utc))).type.toBe<DateTime.DateTime>();
        expect(
            dateTime.pipe(DateTime.min(dateTime)),
        ).type.toBe<DateTime.DateTime>();
    });

    it("max", () => {
        expect(DateTime.max(utc, zoned)).type.toBe<
            DateTime.Utc | DateTime.Zoned
        >();
        ...
        expect(
            dateTime.pipe(DateTime.max(zoned)),
        ).type.toBe<DateTime.DateTime>();
        expect(dateTime.pipe(DateTime.max(utc))).type.toBe<DateTime.DateTime>();
        expect(
            dateTime.pipe(DateTime.max(dateTime)),
        ).type.toBe<DateTime.DateTime>();
    });
});
