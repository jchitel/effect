...
describe("Effect", () => {
    describe("forEach", () => {
        ...
        it("non empty array", () => {
            expect(
                ...
                Effect.Effect<[string, ...Array<string>], "err-1", "dep-1">
            >();
            expect(
                pipe(
    ...
    describe("all", () => {
        it("tuple", () => {
            ...
            expect(Effect.all([string, number], { mode: "either" })).type.toBe<
                Effect.Effect<
                    [
                        Either.Either<string, "err-1">,
                        Either.Either<number, "err-2">,
                    ],
                    never,
                    "dep-1" | "dep-2"
                >
        ...
        it("array", () => {
            ...
            expect(Effect.all(stringArray, { mode: "either" })).type.toBe<
                Effect.Effect<
                    Array<Either.Either<string, "err-3">>,
                    never,
                    "dep-3"
                >
            >();
    ...
    it("tap", () => {
        ...
        when(Effect.succeed("a" as const).pipe).isCalledWith(
            expect(Effect.tap).type.not.toBeCallableWith("a", {
                onlyEffect: true,
            }),
        );
    ...
    it("tapErrorCause", () => {
        expect(
            Effect.fail("a" as const).pipe(
                Effect.tapErrorCause(tacitStringCause),
            ),
        ).type.toBe<Effect.Effect<never, "a">>();
    });
    ...
    it("validate", () => {
        ...
        ).type.toBe<Effect.Effect<[number, string]>>();
    });
    ...
    it("dropWhile", () => {
        ...
        expect(
            pipe(
                numbersArray,
                Effect.dropWhile(predicateNumbersOrStringsEffect),
            ),
        ).type.toBe<Effect.Effect<Array<number>>>();
    });
    ...
    it("dropUntil", () => {
        ...
        expect(
            pipe(
                numbersArray,
                Effect.dropUntil(predicateNumbersOrStringsEffect),
            ),
        ).type.toBe<Effect.Effect<Array<number>>>();
    });
    ...
    it("findFirst", () => {
        ...
        expect(
            Effect.findFirst(numberArray, (_n: unknown) =>
                Effect.succeed(true),
            ),
        ).type.toBe<Effect.Effect<Option.Option<number>>>();
        expect(
    ...
    it("takeUntil", () => {
        ...
        expect(
            Effect.takeUntil(numberArray, (_n: unknown) =>
                Effect.succeed(true),
            ),
        ).type.toBe<Effect.Effect<Array<number>>>();
        expect(
    ...
    it("takeWhile", () => {
        ...
        expect(
            Effect.takeWhile(numberArray, (_n: unknown) =>
                Effect.succeed(true),
            ),
        ).type.toBe<Effect.Effect<Array<number>>>();
        expect(
    ...
    it("catchSome", () => {
        ...
        expect(
            Effect.catchSome(string, (_e: string) =>
    ...
    it("retryOrElse", () => {
        ...
        expect(
            string.pipe(
                Effect.retryOrElse(Schedule.forever, (_e: string) =>
                    Effect.succeed(0),
                ),
            ),
        ).type.toBe<Effect.Effect<string | number, never, "dep-1">>();
    ...
    it("Tag.Proxy", () => {
        expect(
            hole<Types.Simplify<Effect.Tag.Proxy<"R", {}>>>(),
        ).type.toBe<{}>();
        expect(
        ...
        expect(
            hole<
                Types.Simplify<Effect.Tag.Proxy<"R", { a: () => Promise<1> }>>
            >(),
