...
describe("ScopedCache", () => {
                ...
                yield* Effect.scoped(
                    Effect.gen(function* () {
                        ...
                        yield* Effect.forEach(
                            ...
                            (n) =>
                                Effect.scoped(
                                    Effect.zipRight(cache.get(n), Effect.void),
                                ),
                            { concurrency: "unbounded", discard: true },
                        );
    ...
    it.effect(
        ...
        () =>
            Effect.gen(function* () {
                const observablesResource =
                    yield* ObservableResource.makeVoid();
                const scopedCache = ScopedCache.make({
                    capacity: 4,
                    timeToLive: Duration.infinity,
            ...
            Effect.gen(function* () {
                ...
                yield* Effect.scoped(
                    Effect.gen(function* () {
                        ...
                        yield* Effect.either(
                            Effect.scoped(resourceScopedProxy),
                        );
        ...
        const arb = fc
            ...
            .chain((cacheSize) =>
                fc
                    ...
                    .map(
                        (numCreatedKey) => [cacheSize, numCreatedKey] as const,
                    ),
            );
        return fc.assert(
            fc.asyncProperty(arb, ([cacheSize, numCreatedKey]) => {
                const program = Effect.gen(function* () {
                    const watchableLookup = yield* WatchableLookup.make<
                        number,
                        void
                    >(() => void 0);
                    const scopedCache = ScopedCache.make({
                    ...
                    yield* Effect.scoped(
                        Effect.gen(function* () {
                            ...
                            yield* Effect.forEach(
                                ...
                                (key) =>
                                    Effect.scoped(
                                        Effect.asVoid(cache.get(key)),
                                    ),
                                { discard: true },
                            );
                            const createdResources =
                                yield* watchableLookup.createdResources();
                            const cleanedAssertions =
                                numCreatedKey - cacheSize - 1;
                            ...
                            yield* Effect.all(oldestResourceCleaned, {
                                discard: true,
                            });
                            const newestResourceNotCleanedYet = pipe(
                                Array.range(
                                    numCreatedKey - cacheSize,
                                    numCreatedKey - 1,
                                ),
                                Array.flatMap((key) =>
                                    Chunk.toReadonlyArray(
                            ...
                            yield* Effect.all(newestResourceNotCleanedYet, {
                                discard: true,
                            });
                        }),
                    );
                ...
                yield* Effect.scoped(
                    Effect.gen(function* () {
                        ...
                        const scoped = Effect.scoped(
                            Effect.asVoid(cache.get(void 0)),
                        );
                        yield* scoped;
                        yield* TestClock.adjust(Duration.seconds(5));
                        yield* scoped;
    ...
    it.effect(
        "getOption - should return None if resource is not in cache",
        () =>
            Effect.scoped(
                Effect.gen(function* () {
    ...
    it.effect(
        ...
        () =>
            Effect.gen(function* () {
                const error = new Cause.RuntimeException(
                ...
                const result = yield* Effect.scoped(
                    Effect.gen(function* () {
                        ...
                        yield* cache.refresh(key);
                        const failure2 = yield* Effect.either(
                            cache.refresh(key),
                        );
                        yield* cache.refresh(key);
                ...
                yield* Effect.scoped(
                    Effect.gen(function* () {
                        ...
                        const refreshFiber = yield* Effect.fork(
                            cache.refresh(void 0),
                        );
                        yield* pipe(
                            watchableLookup.getCalledTimes(void 0),
                            Effect.repeat(
                                pipe(
                                    Schedule.recurWhile<number>(
                                        (calledTimes) => calledTimes < 2,
                                    ),
