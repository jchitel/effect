...
describe("Match", () => {
    ...
    it("schema exhaustive-literal", () => {
        const match = pipe(
            M.type<
                { _tag: "A"; a: number | string } | { _tag: "B"; b: number }
            >(),
    ...
    it("unify", () => {
        const match = pipe(
            ...
            M.tag(
                "A",
                () => Either.right("a") as Either.Either<string, number>,
            ),
            M.tag(
                "B",
                () => Either.right(123) as Either.Either<number, string>,
            ),
            M.exhaustive,
        );
    ...
    it("whenAnd nested", () => {
        ...
        strictEqual(
            match({ status: 100, user: { name: "Tim" } }),
            "number, Tim",
        );
