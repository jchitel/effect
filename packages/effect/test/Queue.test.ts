...
describe("Queue", () => {
    it.effect(
        "bounded - offerAll returns true when there is enough space",
        () =>
            Effect.gen(function* () {
    ...
    it.effect("back pressured offers are retrieved", () =>
        Effect.gen(function* () {
            ...
            yield* pipe(
                Queue.take(queue),
                Effect.flatMap((n) =>
                    Ref.update(ref, (values) => [...values, n]),
                ),
                Effect.repeatN(9),
            );
            const result = yield* Ref.get(ref);
    ...
    it.effect("shutdown with take fiber", () =>
        Effect.gen(function* () {
            ...
            const result = yield* Effect.either(
                Effect.sandbox(Fiber.join(fiber)),
            );
    ...
    it.effect(
        "elements can be enqueued syncroniously when there is space",
        () =>
            Effect.gen(function* () {
                const queue = yield* Queue.unbounded<number>();
    ...
    it.effect(
        "takeBetween returns immediately if there is enough elements",
        () =>
            Effect.gen(function* () {
                const queue = yield* Queue.bounded<number>(100);
                yield* Queue.offer(queue, 1);
    ...
    it.effect("should handle taking up to Number.POSITIVE_INFINITY items", () =>
        Effect.gen(function* () {
            const queue = yield* Queue.bounded<number>(100);
            yield* Queue.offer(queue, 1);
            const result = yield* Queue.takeUpTo(
                queue,
