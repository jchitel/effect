...
export const assertions = Effect.gen(function* () {
    ...
    const out = {
        ...
        testRoundtripConsistency<A, I>(
            ...
            const params = Predicate.isObject(
                config.testRoundtripConsistency?.params,
            )
                ? {
                      ...config.testRoundtripConsistency?.params,
                      ...options?.params,
            ...
            FastCheck.assert(
                FastCheck.property(arb, (a) => {
                    const roundtrip = encode(a).pipe(
                        Effect.mapError(
                            (issue) => ["encoding", issue] as const,
                        ),
                        Effect.flatMap((i) =>
                            decode(i).pipe(
                                Effect.mapError(
                                    (issue) => ["decoding", issue] as const,
                                ),
                            ),
                        ),
                        Effect.either,
                    ...
                    if (Either.isLeft(roundtrip)) {
                        if (
                            ...
                            return options.ignoreEncodingErrors(
                                roundtrip.left[1],
                            );
                        }
                        return false;
        ...
        effect: {
            ...
            async fail<A>(
                ...
                const result = await Effect.runPromise(
                    Effect.either(effectWithMessage),
                );
        ...
        either: {
            ...
            async fail<R>(
                ...
                const result = await Effect.runPromise(
                    Effect.either(eitherWithMessage),
                );
                return out.either.left(result, message);
            },
        },
