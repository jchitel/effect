...
schema (Declaration): <declaration schema>`),
                ...
                throws(
                    () =>
                        Arbitrary.makeLazy(
                            S.Tuple(S.declare(Predicate.isUnknown)),
                        ),
                    new Error(`Missing annotation
at path: [0]
details: Generating an Arbitrary for this schema requires an "arbitrary" annotation
schema (Declaration): <declaration schema>`),
                ...
                throws(
                    () =>
                        Arbitrary.makeLazy(
                            S.Struct({ a: S.declare(Predicate.isUnknown) }),
                        ),
                    new Error(`Missing annotation
at path: ["a"]
details: Generating an Arbitrary for this schema requires an "arbitrary" annotation
                ...
                throws(
                    () =>
                        Arbitrary.makeLazy(
                            S.Record({
                                key: S.String,
                                value: S.declare(Predicate.isUnknown),
                            }),
                        ),
                    new Error(`Missing annotation
details: Generating an Arbitrary for this schema requires an "arbitrary" annotation
...
details: Generating an Arbitrary for this schema requires at least one enum`),
        ...
        describe("Unrefined Primitives", () => {
            ...
            describe("TemplateLiteral", () => {
                ...
                it("https://www.typescriptlang.org/docs/handbook/2/template-literal-types.html", async () => {
                    const EmailLocaleIDs = S.Literal(
                        "welcome_email",
                        "email_heading",
                    );
                    const FooterLocaleIDs = S.Literal(
                        "footer_title",
                        "footer_sendoff",
                    );
                    const schema = S.TemplateLiteral(
            ...
            describe("Struct", () => {
                ...
                it("optional property signature with undefined", () => {
                    const schema = S.Struct({
                        a: S.optionalWith(S.Union(S.Number, S.Undefined), {
                            exact: true,
                        }),
                    });
                    Util.assertions.arbitrary.validateGeneratedValues(schema);
        ...
        describe("Data Types", () => {
            ...
            describe("Suspend", () => {
                ...
                it("EitherFromSelf", () => {
                    const Rec = S.suspend((): any => schema);
                    const schema: any = S.EitherFromSelf({
                        left: S.String,
                        right: Rec,
                    });
                    Util.assertions.arbitrary.validateGeneratedValues(schema);
                ...
                it("MapFromSelf", () => {
                    const Rec = S.suspend((): any => schema);
                    const schema: any = S.MapFromSelf({
                        key: S.String,
                        value: Rec,
                    });
                    Util.assertions.arbitrary.validateGeneratedValues(schema);
                ...
                it("HashMapFromSelf", () => {
                    const Rec = S.suspend((): any => schema);
                    const schema: any = S.HashMapFromSelf({
                        key: S.String,
                        value: Rec,
                    });
                    Util.assertions.arbitrary.validateGeneratedValues(schema);
        ...
        describe("Refinement", () => {
            ...
            describe("array filters", () => {
                it("Array", () => {
                    ...
                    assertConstraints(schema, [
                        Arbitrary.makeArrayConstraints({}),
                    ]);
                    Util.assertions.arbitrary.validateGeneratedValues(schema);
                });
            ...
            describe("string filters", () => {
                it("String", () => {
                    ...
                    assertConstraints(schema, [
                        Arbitrary.makeStringConstraints({}),
                    ]);
                    Util.assertions.arbitrary.validateGeneratedValues(schema);
                });
                ...
                it("pattern + pattern", () => {
                    ...
                    assertConstraints(schema, [
                        Arbitrary.makeStringConstraints({
                            pattern: regexp1.source,
                        }),
                        Arbitrary.makeStringConstraints({
                            pattern: regexp2.source,
                        }),
                    ]);
                    Util.assertions.arbitrary.validateGeneratedValues(schema);
                });
            ...
            describe("bigint filters", () => {
                ...
                it("between", () => {
                    ...
                    assertConstraints(schema, [
                        Arbitrary.makeBigIntConstraints({
                            min: BigInt(2),
                            max: BigInt(5),
                        }),
                    ]);
        ...
        describe("Annotations", () => {
            const assertAnnotation = <A, I>(source: S.Schema<A, I>) => {
                const schema = source.annotations({
                    arbitrary: () => (fc) =>
                        fc.constant("custom arbitrary") as any,
                });
