...
describe("extend", () => {
    describe("String", () => {
        ...
        it("String Branded Refinement & String Branded Refinement", () => {
            ...
            deepStrictEqual(schema.ast.annotations[AST.BrandAnnotationId], [
                ":end",
            ]);
            assertTrue(AST.isRefinement(schema.ast));
            const from = schema.ast.from;
            deepStrictEqual(from.annotations[AST.BrandAnnotationId], [
                "start:",
            ]);
    ...
    describe("Number", () => {
        ...
        it("Number Branded Refinement & Number Branded Refinement", () => {
            const gt0 = Schema.Number.pipe(
                Schema.greaterThan(0),
                Schema.brand("> 0"),
            );
            const lt2 = Schema.Number.pipe(
                Schema.lessThan(2),
                Schema.brand("< 2"),
            );
    ...
    describe("Refinements", () => {
        ...
        it("Struct & Refinement (two filters)", async () => {
            ...
            const B = Schema.Struct({ b: Schema.Number }).pipe(
                ...
                Schema.filter((input) => input.b < 10, {
                    message: () => "filter2",
                }),
            );
            assertExtend(A, B, [
