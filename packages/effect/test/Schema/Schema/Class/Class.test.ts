...
└─ Encoded side transformation failure
   └─ A (Encoded side)
    ...
    it("encoding", async () => {
        class A extends S.Class<A>("A")({ a: S.NonEmptyString }) {}
        await Util.assertions.encoding.succeed(A, new A({ a: "a" }), {
            a: "a",
        });
...
details: Duplicate key "a"`),
    ...
    it("with a field with a context !== never", async () => {
        ...
        const person = S.decodeUnknown(PersonContext)({
            id: 1,
            name: "John",
        }).pipe(Effect.provideService(Name, "John"), Effect.runSync);
    ...
    it("should accept a refinement of a Struct as argument", async () => {
        ...
        class A extends S.Class<A>("A")(
            S.Struct(fields).pipe(
                S.filter(({ a, b }) =>
                    a === b ? undefined : "a should be equal to b",
                ),
            ),
        ) {}
        Util.expectFields(A.fields, fields);
   ...
   └─ a should be equal to b`,
    ...
    describe("encode", () => {
        ...
        it("struct a class with a getter", async () => {
            ...
            await Util.assertions.encoding.succeed(A, { n: 1 } as any, {
                n: "1",
            });
        });
