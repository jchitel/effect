...
const expectSyncTree = <A, I>(
    ...
    const actual = S.decodeUnknownEither(schema)(
        ...
        Either.mapLeft((e) =>
            ParseResult.TreeFormatter.formatIssueSync(e.issue),
        ),
    );
...
const expectSyncIssues = <A, I>(
    ...
    const actual = S.decodeUnknownEither(schema)(input, options).pipe(
        Either.mapLeft((e) =>
            ParseResult.ArrayFormatter.formatIssueSync(e.issue),
        ),
    );
    assertLeft(actual, expected);
};
...
const expectAsyncTree = async <A, I>(
    ...
    const actualEffect = ParseResult.TreeFormatter.formatIssue(
        result.left.issue,
    );
...
const expectAsyncIssues = async <A, I>(
    ...
    assertTrue(Either.isLeft(result));
    const actualEffect = ParseResult.ArrayFormatter.formatIssue(
        result.left.issue,
    );
    assertTrue(Effect.isEffect(actualEffect));
    throws(() => Effect.runSync(actualEffect));
         ...
         └─ Expected a non empty string, actual ""`,
            ...
            it("custom message (kind=Transformation, override=false)", () => {
                const schema = S.transformOrFail(S.String, S.String, {
                    ...
                    decode: (s, _, ast) =>
                        ParseResult.fail(
                            new ParseResult.Type(ast, s, "message field"),
                        ),
                    encode: ParseResult.succeed,
                }).annotations({ message: () => "custom message" });
                const input = "";
