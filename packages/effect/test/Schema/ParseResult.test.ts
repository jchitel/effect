...
└─ ["a"]
   └─ is missing`,
    ...
    it("eitherOrUndefined", () => {
        ...
        strictEqual(
            ParseResult.eitherOrUndefined(Effect.succeed(1)),
            undefined,
        );
        strictEqual(
            ParseResult.eitherOrUndefined(Effect.fail("err")),
            undefined,
        );
    });
    ...
    it("map", () => {
        ...
        assertSuccess(
            Effect.runSyncExit(
                ParseResult.map(Effect.succeed(1), (n) => n + 1),
            ),
            2,
        );
        assertFailure(
    ...
    it("orElse", () => {
        ...
        deepStrictEqual(
            ParseResult.orElse(Either.left(typeParseError1), () =>
                Either.right(2),
            ),
            asEffect(Either.right(2)),
        );
        ...
        assertSuccess(
            Effect.runSyncExit(
                ParseResult.orElse(Effect.fail(typeParseError1), () =>
                    Either.right(2),
                ),
            ),
            2,
...
describe("ParseIssue.actual", () => {
    it("transform decode", () => {
        const result = S.decodeEither(
            S.transformOrFail(S.NumberFromString, S.Boolean, {
                strict: true,
                decode: (n, _, ast) =>
                    ParseResult.fail(new ParseResult.Type(ast, n)),
                encode: (b, _, ast) =>
                    ParseResult.fail(new ParseResult.Type(ast, b)),
            }),
        )("1");
    ...
    it("mergeInternalOptions", () => {
        ...
        deepStrictEqual(
            ParseResult.mergeInternalOptions({}, { exact: false }),
            { exact: false },
        );
        deepStrictEqual(
    ...
    it("asserts", () => {
        ...
        throws(
            ...
            new ParseResult.ParseError({
                issue: new ParseResult.Type(schema.ast, 1),
            }),
        );
    });
    ...
    describe("getLiterals", () => {
        ...
        it("Tuple", () => {
            deepStrictEqual(
                ParseResult.getLiterals(
                    S.Tuple(S.Literal("a"), S.String).ast,
                    true,
                ),
                [[0, new AST.Literal("a")]],
            );
    ...
    describe("getSearchTree", () => {
        ...
        it("struct + primitive", () => {
            ...
            deepStrictEqual(
                ParseResult.getSearchTree([a.ast, S.Number.ast], true),
                {
                    keys: {
                        _tag: {
                            buckets: {
        ...
        it("should handle multiple tags", () => {
            ...
            deepStrictEqual(
                ParseResult.getSearchTree([a.ast, b.ast, c.ast], true),
                {
                    keys: {
                        category: {
                            buckets: {
