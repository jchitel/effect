...
const makePollingGauge = (name: string, increment: number) => {
    ...
    const metric = MetricPolling.make(
        ...
        Metric.value(gauge).pipe(
            Effect.map((gauge) => gauge.value + increment),
        ),
    );
...
describe("Metric", () => {
        ...
        it.effect("count + taggedWith", () =>
            Effect.gen(function* () {
                ...
                const result = yield* pipe(
                    ...
                    Effect.zipRight(
                        pipe(base, Metric.tagged("dyn", "!"), Metric.value),
                    ),
                );
                deepStrictEqual(result, MetricState.counter(2));
            }),
    ...
    describe("Frequency", () => {
        ...
        it.effect("occurences + taggedWith", () =>
            Effect.gen(function* () {
                ...
                const { result1, result2, result3 } = yield* pipe(
                    ...
                    Effect.zipRight(
                        Effect.all({
                            ...
                            result2: pipe(
                                base,
                                Metric.tagged("dyn", "hello"),
                                Metric.value,
                            ),
                            result3: pipe(
                                base,
                                Metric.tagged("dyn", "world"),
                                Metric.value,
                            ),
                        }),
                    ),
                ...
                deepStrictEqual(
                    result2.occurrences,
                    new Map([["hello", 2] as const]),
                );
                deepStrictEqual(
                    result3.occurrences,
                    new Map([["world", 1] as const]),
                );
            }),
        );
    });
        ...
        it.effect("custom set as aspect", () =>
            Effect.gen(function* () {
                ...
                const result = yield* pipe(
                    ...
                    Effect.zipRight(
                        pipe(Effect.succeed(3), Effect.withMetric(gauge)),
                    ),
        ...
        it.effect("gauge + taggedWith", () =>
            Effect.gen(function* () {
                ...
                const result = yield* pipe(
                    ...
                    Effect.withMetric(gauge),
                    Effect.zipRight(
                        pipe(Effect.succeed("!"), Effect.withMetric(gauge)),
                    ),
                    Effect.zipRight(
                        pipe(Effect.succeed("!"), Effect.withMetric(gauge)),
                    ),
        ...
        it.effect("custom observe as aspect", () =>
            Effect.gen(function* () {
                ...
                const result = yield* pipe(
                    ...
                    Effect.zipRight(
                        pipe(Effect.succeed(3), Effect.withMetric(summary)),
                    ),
        ...
        it.scoped(
            ...
            () =>
                Effect.gen(function* () {
                    ...
                    const fiber = yield* pipe(
                        metric,
                        MetricPolling.launch(Schedule.once),
                    );
        ...
        it.scoped(
            ...
            () =>
                Effect.gen(function* () {
                    ...
                    const [gauge1, metric1] = makePollingGauge(
                        name1,
                        gaugeIncrement1,
                    );
                    const [gauge2, metric2] = makePollingGauge(
                        name2,
                        gaugeIncrement2,
                    );
    ...
    it.effect("with a description", () =>
        Effect.gen(function* () {
            ...
            const counter2 = Metric.counter(name, {
                description: "description1",
            });
            const counter3 = Metric.counter(name, {
