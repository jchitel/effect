...
describe("TMap", () => {
        ...
        fc.assert(
            fc.asyncProperty(mapEntriesArb, async (entries) => {
                ...
                strictEqual(
                    pipe(
                        result,
                        Array.differenceWith(equivalentElements())(keys),
                    ).length,
                    0,
                );
                strictEqual(
                    pipe(
                        keys,
                        Array.differenceWith(equivalentElements())(result),
                    ).length,
                    0,
                );
            }),
    ...
    it.effect("reduceSTM - non-empty map", () =>
        Effect.gen(function* () {
            const transaction = pipe(
                TMap.make(["a", 1], ["b", 2], ["c", 3]),
                STM.flatMap(
                    TMap.reduceSTM(0, (acc, value) => STM.succeed(acc + value)),
                ),
            );
            const result = yield* STM.commit(transaction);
            strictEqual(result, 6);
        ...
        Effect.gen(function* () {
            ...
            const transaction = pipe(
                ...
                STM.tap((map) =>
                    STM.all(
                        entries.map((entry) =>
                            pipe(map, TMap.set(entry[0], entry[1])),
                        ),
                    ),
                ),
                STM.flatMap(TMap.toArray),
    ...
    it.effect("setIfAbsent", () =>
        Effect.gen(function* () {
            ...
            strictEqual(
                pipe(
                    result,
                    Array.differenceWith(equivalentElements())(expected),
                ).length,
                0,
            );
            strictEqual(
    ...
    it.effect("transform - and shrink", () =>
        Effect.gen(function* () {
            const transaction = pipe(
                TMap.make(["a", 1], ["aa", 2], ["aaa", 3]),
                STM.tap((map) =>
                    TMap.transform(map, (_, value) => ["key", value * 2]),
                ),
                STM.flatMap(TMap.toArray),
            );
            const result = yield* STM.commit(transaction);
    ...
    it.effect("transformSTM - and shrink", () =>
        Effect.gen(function* () {
            const transaction = pipe(
                TMap.make(["a", 1], ["aa", 2], ["aaa", 3]),
                STM.tap((map) =>
                    TMap.transformSTM(map, (_, value) =>
                        STM.succeed(["key", value * 2]),
                    ),
                ),
                STM.flatMap(TMap.toArray),
            );
    ...
    it.effect("transformValues", () =>
        Effect.gen(function* () {
            const transaction = pipe(
                TMap.make(["a", 1], ["aa", 2], ["aaa", 3]),
                STM.tap((map) =>
                    TMap.transformValues(map, (value) => value * 2),
                ),
                STM.flatMap(TMap.toArray),
            );
            const result = yield* STM.commit(transaction);
    ...
    it.effect("transformValuesSTM", () =>
        Effect.gen(function* () {
            const transaction = pipe(
                TMap.make(["a", 1], ["aa", 2], ["aaa", 3]),
                STM.tap((map) =>
                    TMap.transformValuesSTM(map, (value) =>
                        STM.succeed(value * 2),
                    ),
                ),
                STM.flatMap(TMap.toArray),
        ...
        fc.assert(
            fc.asyncProperty(mapEntriesArb, async (entries) => {
                ...
                strictEqual(
                    pipe(
                        result,
                        Array.differenceWith(equivalentElements())(values),
                    ).length,
                    0,
                );
                strictEqual(
    ...
    it.effect("avoid issues due to race conditions (ZIO Issue #4648)", () =>
        Effect.gen(function* () {
            ...
            const map = yield* TMap.fromIterable(
                Array.map(keys, (n, i) => [n, i]),
            );
