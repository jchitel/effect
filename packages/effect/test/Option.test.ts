...
describe("Option", () => {
  ...
  "value": {
    ...
    it("toJSON", () => {
        deepStrictEqual(Option.none().toJSON(), {
            _id: "Option",
            _tag: "None",
        });
    ...
    it("Hash", () => {
        ...
        strictEqual(
            Hash.hash(Option.some(1)) === Hash.hash(Option.some(2)),
            false,
        );
    ...
    it("partitionMap", () => {
        const f = (n: number) =>
            gt2(n) ? Either.right(n + 1) : Either.left(n - 1);
    ...
    it("reduceCompact", () => {
        ...
        strictEqual(
            sumCompact([Option.some(2), Option.none(), Option.some(3)]),
            5,
        );
    });
    ...
    it("all/ tuple", () => {
        assertType<Option.Option<[number, string]>>(
            Option.all([Option.some(1), Option.some("hello")]),
        );
    ...
    it("all/ iterable", () => {
        assertType<Option.Option<Array<number>>>(
            Option.all([Option.some(1), Option.some(2)]),
        );
        assertType<Option.Option<Array<number>>>(
            Option.all(new Set([Option.some(1), Option.some(2)])),
        );
    ...
    it("all/ struct", () => {
        assertType<Option.Option<{ a: number; b: string }>>(
