...
describe("Stream", () => {
    ...
    it.effect("mapEffectPar - ordering when parallelism is 1", () =>
        Effect.gen(function* () {
            ...
            yield* pipe(
                Stream.range(0, 8),
                Stream.mapEffect((n) => Queue.offer(queue, n), {
                    concurrency: 1,
                }),
                Stream.runDrain,
            );
    ...
    it.effect("mapEffectPar - awaits child fibers properly", () =>
        Effect.gen(function* () {
            const result = yield* pipe(
                ...
                Stream.mapEffect(
                    () => pipe(Effect.succeed(1), Effect.repeatN(200)),
                    { concurrency: 8 },
                ),
                Stream.runDrain,
                Effect.exit,
        ...
        () =>
            Effect.gen(function* () {
                const result = yield* pipe(
                    ...
                    Stream.mapEffect(
                        (n) =>
                            n < 10 ? Effect.succeed(n) : Effect.fail("boom"),
                        { concurrency: 20 },
                    ),
                    Stream.mapEffect((n) => Effect.succeed(n), {
                        concurrency: 20,
                    }),
                    Stream.runCollect,
                    Effect.either,
    ...
    it.effect(
        "mapEffectPar - propagates the error of the original stream",
        () =>
            Effect.gen(function* () {
                const fiber = yield* pipe(
                    Stream.range(1, 10),
                    Stream.concat(
                        Stream.fail(new Cause.RuntimeException("boom")),
                    ),
                ...
                deepStrictEqual(
                    exit,
                    Exit.fail(new Cause.RuntimeException("boom")),
                );
            }),
    );
    ...
    it.effect("mapEffect with key", () =>
        Effect.gen(function* () {
            ...
            deepStrictEqual(
                Chunk.toReadonlyArray(exit.value),
                [10, 20, 30, 40],
            );
        }),
    );
