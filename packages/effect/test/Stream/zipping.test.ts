...
export const splitChunks = <A>(
    ...
    const split = (
        ...
        fc
            .integer({ min: 0, max: Math.max(chunks.length - 1, 0) })
            .chain((i) => {
...
describe("Stream", () => {
    it("zipAllSortedByKeyWith", () => {
        ...
        const chunkArb = fc
            ...
            .map((entries) =>
                pipe(
                    Chunk.fromIterable(new Map(entries)),
                    Chunk.sort(OrderByKey),
                ),
            );
        const chunksArb = chunkArb.chain((chunk) =>
        ...
        return fc.assert(
            fc.asyncProperty(chunksArb, chunksArb, async (as, bs) => {
                ...
                const expected = pipe(
                    Chunk.flatten(as),
                    Chunk.reduce(
                        new Map(Array.from(Chunk.flatten(bs))),
                        (map, [k, v]) =>
                            pipe(
                    ...
                    const { result1, result2 } = await Effect.runPromise(
                        Effect.all({
                            ...
                            result2: pipe(
                                ...
                                Stream.concat(
                                    pipe(stream, Stream.map(Option.some)),
                                ),
                                Stream.zip(stream),
                                Stream.runCollect,
                            ),
