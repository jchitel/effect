        ...
        Effect.gen(function* () {
            const result = yield* pipe(
                Stream.contextWithEffect(
                    (context: Context.Context<StringService>) =>
                        Effect.succeed(
                            pipe(context, Context.get(StringService)),
                        ),
                ),
                Stream.provideContext(
                    pipe(
    ...
    it.effect("serviceWith", () =>
        Effect.gen(function* () {
            const result = yield* pipe(
                ...
                Stream.provideLayer(
                    Layer.succeed(StringService, { string: "test" }),
                ),
                Stream.runCollect,
            );
            deepStrictEqual(Chunk.toReadonlyArray(result), ["test"]);
        ...
        Effect.gen(function* () {
            ...
            const effect = Effect.acquireRelease(
                pipe(
                    StringService,
                    Effect.tap((s) =>
                        Effect.sync(() => messages.push(s.string)),
                    ),
                ),
                () =>
                    pipe(
            ...
            const stream = pipe(
                ...
                Stream.concat(
                    pipe(Stream.scoped(effect), Stream.provideSomeLayer(L2)),
                ),
