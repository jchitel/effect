...
describe("Stream", () => {
    it.effect("buffer - maintains elements and ordering", () =>
        Effect.gen(function* () {
            ...
            deepStrictEqual(
                Array.from(result),
                Array.from(Chunk.flatten(chunks)),
            );
        }),
    );
    ...
    it.effect(
        "bufferChunksDropping - fast producer progress independently",
        () =>
            Effect.gen(function* () {
                const ref = yield* Ref.make(Chunk.empty<number>());
    ...
    it.effect(
        "bufferChunksSliding - fast producer progress independently",
        () =>
            Effect.gen(function* () {
                const ref = yield* Ref.make(Chunk.empty<number>());
                const latch1 = yield* Deferred.make<void>();
    ...
    it.effect("bufferSliding - propagates defects", () =>
        Effect.gen(function* () {
            ...
            deepStrictEqual(
                result,
                Exit.die(new Cause.RuntimeException("boom")),
            );
        }),
    );
