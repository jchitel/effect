...
describe("Stream", () => {
    ...
    it.effect("async - backpressure", () =>
        Effect.gen(function* () {
            ...
            const stream = Stream.async<number, Option.Option<never>>(
                (emit) => {
                    Promise.all(
    ...
    it.effect("asyncScoped - backpressure", () =>
        Effect.gen(function* () {
            const refCount = yield* Ref.make(0);
            const refDone = yield* Ref.make(false);
            const stream = Stream.asyncScoped<number, Option.Option<never>>(
                (cb) => {
                    Promise.all(
                        // 1st consumed by sink, 2-6 – in queue, 7th – back pressured
                        [1, 2, 3, 4, 5, 6, 7].map((n) =>
    ...
    it.effect("asyncPush - handles errors", () =>
        Effect.gen(function* () {
            const error = new Cause.RuntimeException("boom");
            const result = yield* Stream.asyncPush<
                number,
