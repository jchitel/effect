            ...
            const result = yield* pipe(
                Stream.paginate(s, ([n, nums]) =>
                    nums.length === 0
                        ...
                        : ([
                              n,
                              Option.some([nums[0], nums.slice(1)] as const),
                          ] as const),
                ),
                Stream.runCollect,
            );
            ...
            const result = yield* pipe(
                Stream.paginateEffect(
                    ...
                    ([n, nums]): Effect.Effect<
                        readonly [
                            number,
                            Option.Option<readonly [number, Array<number>]>,
                        ]
                    > =>
                        nums.length === 0
                            ...
                            : Effect.succeed([
                                  n,
                                  Option.some([nums[0], nums.slice(1)]),
                              ]),
                ),
                Stream.runCollect,
            );
                ...
                Stream.paginateChunkEffect(s, ([chunk, nums]) =>
                    nums.length === 0
                        ? Effect.succeed([
                              chunk,
                              Option.none<
                                  readonly [Chunk.Chunk<number>, Array<number>]
                              >(),
                          ] as const)
                        : Effect.succeed([
                              chunk,
