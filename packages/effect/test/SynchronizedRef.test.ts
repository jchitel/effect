...
describe("SynchronizedRef", () => {
    ...
    it.effect("getAndUpdateEffect - with failure", () =>
        Effect.gen(function* () {
            const ref = yield* SynchronizedRef.make(current);
            const result = yield* pipe(
                SynchronizedRef.getAndUpdateEffect(ref, (_) =>
                    Effect.fail(failure),
                ),
                Effect.exit,
            );
    ...
    it.effect(
        "getAndUpdateSomeEffect - interrupt parent fiber and update",
        () =>
            Effect.gen(function* () {
                const deferred =
                    yield* Deferred.make<
                        SynchronizedRef.SynchronizedRef<State>
                    >();
                ...
                const result = yield* SynchronizedRef.updateAndGetEffect(
                    ref,
                    (_) => Effect.succeed(Closed),
                );
