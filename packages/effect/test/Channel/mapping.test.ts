...
describe("Channel", () => {
    ...
    it.effect(
        "concatMap - upstream acquireReleaseOut + downstream failure",
        () =>
            Effect.gen(function* () {
                ...
                const effect = pipe(
                    Channel.acquireReleaseOut(event("Acquired"), () =>
                        event("Released"),
                    ),
                    Channel.concatMap(() => Channel.fail("error")),
                    Channel.runDrain,
        ...
        () =>
            Effect.gen(function* () {
                const result = yield* pipe(
                    Channel.write(void 0),
                    Channel.concatMap(() =>
                        Channel.write(Channel.fail("error")),
                    ),
                    Channel.concatMap(
        ...
        Effect.gen(function* () {
            const result = yield* pipe(
                ...
                Channel.concatMapWithCustom(
                    (x) =>
                        Channel.writeAll(
                            Option.some([x, 1]) as Option.Option<
                                readonly [number, number]
                            >,
                            Option.none() as Option.Option<
                                readonly [number, number]
                            >,
                            Option.some([x, 2]) as Option.Option<
                                readonly [number, number]
                            >,
                            Option.none() as Option.Option<
                                readonly [number, number]
                            >,
                            Option.some([x, 3]) as Option.Option<
                                readonly [number, number]
                            >,
                            Option.none() as Option.Option<
                                readonly [number, number]
                            >,
                            Option.some([x, 4]) as Option.Option<
                                readonly [number, number]
                            >,
                        ),
                    constVoid,
