...
export const mapper = <A, B>(
    ...
    return Channel.readWith({
        onInput: (a: A) =>
            Channel.flatMap(Channel.write(f(a)), () => mapper(f)),
...
export const refWriter = <A>(
    ...
    return Channel.readWith({
        onInput: (a: A) =>
            Channel.flatMap(
                Channel.fromEffect(
                    Effect.asVoid(Ref.update(ref, Array.prepend(a))),
                ),
                () => refWriter(ref),
            ),
        onFailure: () => Channel.void,
    ...
    return pipe(
        ...
        Channel.flatMap(
            Option.match({
                onNone: () => Channel.void,
                onSome: (i) =>
                    Channel.flatMap(Channel.write(i), () => refReader(ref)),
            }),
        ),
    );
};
...
describe("Channel", () => {
    ...
    it.effect("simple concurrent reads", () =>
        Effect.gen(function* () {
            const capacity = 128;
            const elements = yield* Effect.replicateEffect(
                Random.nextInt,
                capacity,
            );
