...
describe("Chunk", () => {
  ...
  "values": [
    ...
    it("remove", () => {
        ...
        assertEquals(
            pipe(Chunk.make(1, 2, 3), Chunk.remove(0)),
            Chunk.make(2, 3),
        );
    });
    ...
    describe("toReadonlyArray", () => {
        ...
        describe("Given a large Chunk", () => {
            ...
            for (let i = 0; i < len; i++)
                chunk = Chunk.appendAll(Chunk.of(i), chunk);
            ...
            it("should throw", () => {
                fc.assert(
                    fc.property(fc.array(fc.anything()), (array) => {
                        ...
                        throws(() =>
                            pipe(chunk, Chunk.unsafeGet(array.length)),
                        );
                    }),
                );
            });
                    ...
                    fc.property(
                        ...
                        (arr1, arr2) => {
                            ...
                            throws(() =>
                                pipe(
                                    chunk,
                                    Chunk.unsafeGet(arr1.length + arr2.length),
                                ),
                            );
                        },
                    ),
    ...
    describe("take", () => {
        ...
        describe("Given a concatenated Chunk and an amount > 1", () => {
            const chunk = pipe(
                Chunk.of(1),
                Chunk.appendAll(Chunk.make(2, 3, 4)),
            );
            const amount = 2;
    ...
    describe("drop", () => {
        ...
        it("should handle concatenated chunks", () => {
            ...
            deepStrictEqual(
                pipe(self, Chunk.drop(2), Chunk.toReadonlyArray),
                [3, 4],
            );
        });
    });
    ...
    it("zip", () => {
        ...
        deepStrictEqual(
            Chunk.toArray(Chunk.zip(Chunk.make(1), Chunk.make(2))),
            [[1, 2]],
        );
    });
    ...
    describe("Given two non-materialized chunks of different sizes", () => {
        it("should zip the chunks together and drop the leftover", () => {
            ...
            const zipped = pipe(
                left,
                Chunk.zipWith(pipe(right, Chunk.take(left.length)), (a, b) => [
                    a,
                    b,
                ]),
            );
    ...
    it("separate", () => {
        assertTuple(Chunk.separate(Chunk.empty()), [
            Chunk.empty(),
            Chunk.empty(),
        ]);
        assertTuple(
    ...
    it("tail", () => {
        ...
        // TODO: use assertSome?
        assertEquals(
            Chunk.tail(Chunk.make(1, 2, 3)),
            Option.some(Chunk.make(2, 3)),
        );
    });
    ...
    it("compact", () => {
        ...
        assertEquals(
            Chunk.compact(
                Chunk.make(Option.some(1), Option.some(2), Option.some(3)),
            ),
            Chunk.make(1, 2, 3),
        );
        assertEquals(
            Chunk.compact(
                Chunk.make(Option.some(1), Option.none(), Option.some(3)),
            ),
            Chunk.make(1, 3),
        );
    });
    ...
    it("unzip", () => {
        ...
        assertTuple(
            Chunk.unzip(Chunk.make(["a", 1] as const, ["b", 2] as const)),
