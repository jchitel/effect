...
function diffLaws<Value, Patch>(
    ...
    describe("differ laws", () => {
        it("combining patches is associative", () => {
            ...
            const right = differ.combine(
                patch1,
                differ.combine(patch2, patch3),
            );
            assertTrue(
                equal(differ.patch(left, value1), differ.patch(right, value1)),
            );
        });
...
describe("Differ", () => {
    ...
    describe("readonlyArray", () => {
        diffLaws(
            Differ.readonlyArray<number, (n: number) => number>(
                Differ.update(),
            ),
            randomReadonlyArray,
