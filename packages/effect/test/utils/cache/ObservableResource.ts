    ...
    assertNotAcquired(): Effect.Effect<void> {
        return Effect.map(this.getState, ([numAcquisition, numCleaned]) => {
            ...
            strictEqual(
                numCleaned,
                0,
                "Resource cleaned when it should not have",
            );
        });
    }
    ...
    assertAcquiredOnceAndCleaned(): Effect.Effect<void> {
        return Effect.map(this.getState, ([numAcquisition, numCleaned]) => {
            ...
            strictEqual(
                numCleaned,
                1,
                "Resource not cleaned when it should have",
            );
        });
    }
...
export const makeEffect = <V, E>(
    ...
    pipe(
        ...
        Effect.map(
            ([resourceAcquisitionCount, resourceAcquisitionReleasing]) => {
