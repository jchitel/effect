...
describe("Schedule", () => {
                ...
                const schedule = Schedule.stop.pipe(
                    Schedule.union(
                        Schedule.spaced("2 seconds").pipe(
                            Schedule.intersect(Schedule.stop),
                        ),
                    ),
    ...
    it.effect("Schedule.fixed should compute delays correctly", () =>
        Effect.gen(function* () {
            ...
            const result = yield* pipe(
                ...
                Effect.map((output) =>
                    output[0].pipe(Chunk.map((tuple) => tuple[0])),
                ),
            );
        ...
        Effect.gen(function* () {
            ...
            const actual = Chunk.toReadonlyArray(
                scanLeft(
                    delays,
                    now,
                    (now, delay) => now + Duration.toMillis(delay),
                ),
            ).slice(1);
            assertTrue(
        ...
        it.effect("without timing out", () =>
            Effect.gen(function* () {
                ...
                const result = yield* pipe(
                    ...
                    Effect.flatMap((now) =>
                        schedule.pipe(
                            Schedule.run(now, Array.makeBy(5, constVoid)),
                        ),
                    ),
                );
            ...
            () =>
                Effect.gen(function* () {
                    ...
                    const result = yield* Clock.currentTimeMillis.pipe(
                        Effect.flatMap((now) =>
                            schedule.pipe(
                                Schedule.run(now, Array.range(1, 10)),
                            ),
                        ),
                    );
                    deepStrictEqual(Chunk.toReadonlyArray(result), [
    ...
    describe("repeat on success according to a provided strategy", () => {
        ...
        it.effect(
            "for 'recurWhile(cond)' repeats while the cond still holds",
            () =>
                Effect.gen(function* () {
                    const result = yield* repeat(
                        Schedule.recurWhile((n) => n < 10),
                    );
                    strictEqual(result, 10);
                }),
        );
                ...
                const program = effect.pipe(
                    Effect.retryOrElse(strategy, (s, n) =>
                        TestClock.currentTimeMillis.pipe(
                            Effect.map(
                                (now) => [Duration.millis(now), s, n] as const,
                            ),
                        ),
                    ),
                );
    ...
    describe("cron-like scheduling - repeats at point of time (minute of hour, day of week, ...)", () => {
        it.effect("recur every second minute using cron", () =>
            Effect.gen(function* () {
                ...
                yield* TestClock.setTime(
                    new Date(2024, 0, 1, 0, 0, 35).getTime(),
                );
        ...
        it.effect("recur at Tuesday of each week", () =>
            Effect.gen(function* () {
                ...
                const wednesday = new Date(tuesday).setDate(
                    tuesday.getDate() + 1,
                );
                const input = Chunk.make(
                    tuesdayHour,
        ...
        it.effect("recur in the 2nd day of each month", () =>
            Effect.gen(function* () {
                ...
                const expectedFirstInTime = new Date(
                    new Date(originOffset).setDate(2),
                );
                const expectedSecondInTime = new Date(
                    expectedFirstInTime,
                ).setMonth(expectedFirstInTime.getMonth() + 1);
        ...
        it.effect("recur only in months containing valid number of days", () =>
            Effect.gen(function* () {
                ...
                const expected = Chunk.make(
                    new Date(originOffset).setMonth(2, 30),
                );
                deepStrictEqual(result, expected);
            }),
        );
...
const runCollectLoop = <Env, In, Out>(
    ...
    return driver.next(head).pipe(
        Effect.matchEffect({
            ...
            onSuccess: (b) =>
                runCollectLoop(driver, tail, acc.pipe(Chunk.append(b))),
        }),
    );
};
