...
describe("ConfigProvider", () => {
    ...
    it.effect("top-level missing list", () =>
        Effect.gen(function* () {
            ...
            const result = yield* Effect.exit(
                provider(map).load(hostPortsConfig),
            );
    ...
    it.effect("collection of atoms", () =>
        Effect.gen(function* () {
            const map = new Map([
                [
                    "targets",
                    "https://effect.website,https://github.com/Effect-TS",
                ],
            ]);
                ...
                assertTrue(
                    Exit.isFailure(result) &&
                        ...
                        ConfigError.isMissingData(
                            result.effect_instruction_i0.error,
                        ) &&
                        ...
                        Equal.equals(
                            Chunk.unsafeFromArray(
                                result.effect_instruction_i0.error.path,
                            ),
    ...
    it.effect("values are not split unless a sequence is expected", () =>
        Effect.gen(function* () {
            ...
            const result = yield* configProvider.load(
    ...
    it.effect(
        "orElse - with indexed sequences and each provider unnested",
        () =>
            Effect.gen(function* () {
                const configProvider = ConfigProvider.fromMap(
                    new Map([
    ...
    it.effect("within", () =>
        Effect.gen(function* () {
            ...
            const config = Config.string("key1").pipe(
                ...
                Config.zip(
                    Config.string("key2").pipe(Config.nested("nesting2")),
                ),
            );
            const result = yield* configProvider.load(config);
