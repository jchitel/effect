...
describe("Layer", () => {
    it.effect("layers can be acquired in parallel", () =>
        Effect.gen(function* () {
            ...
            const layer1 = Layer.effectContext<never, never, never>(
                Effect.never,
            );
    ...
    it.effect("can map a layer to an unrelated type", () =>
        Effect.gen(function* () {
            ...
            const layer1 = Layer.succeed(ServiceATag, {
                name: "name",
                value: 1,
            });
            ...
            const live = layer2.pipe(
                Layer.provide(
                    Layer.map(layer1, (context) =>
                        Context.make(
                            StringTag,
                            context.pipe(Context.get(ServiceATag)).name,
                        ),
                    ),
                ),
            );
    ...
    it.scoped("fiberRef changes are memoized", () =>
        Effect.gen(function* () {
            ...
            const layer1 = Layer.scopedDiscard(
                Effect.locallyScoped(fiberRef, true),
            );
    ...
    it.effect("to provides a partial environment to another layer", () =>
        Effect.gen(function* () {
            ...
            const fooBuilder = Layer.context<string | Ref.Ref<number>>().pipe(
                Layer.map((context) => {
                    ...
                    return Context.make(FooTag, {
                        ref,
                        string: s,
                        get: Ref.get(ref).pipe(
                            Effect.map((i) => [i, s] as const),
                        ),
                    });
                }),
            );
            ...
            const needsString = fooBuilder.pipe(
                Layer.provide(provideNumberRef),
            );
    ...
    it.effect("andTo provides a partial environment to another layer", () =>
        Effect.gen(function* () {
            ...
            const provideString = Layer.succeed(StringTag, "hi");
            const needsString = fooBuilder.pipe(
                Layer.provideMerge(provideNumberRef),
            );
    ...
    it.effect("passthrough passes the inputs through to the next layer", () =>
        Effect.gen(function* () {
            ...
            const NumberTag =
                Context.GenericTag<NumberService>("NumberService");
            interface ToStringService {
                readonly value: string;
            }
            const ToStringTag =
                Context.GenericTag<ToStringService>("ToStringService");
            const layer = Layer.function(
                NumberTag,
                ToStringTag,
                (numberService) => ({
    ...
    it.effect("project", () =>
        Effect.gen(function* () {
            ...
            const PersonTag =
                Context.GenericTag<PersonService>("PersonService");
            const AgeTag = Context.GenericTag<AgeService>("AgeService");
            const personLayer = Layer.succeed(PersonTag, {
                name: "User",
                age: 42,
            });
...
export const makeLayer1 = (
    ...
    return Layer.scoped(
        ...
        Effect.acquireRelease(
            ref.pipe(
                Ref.update(Chunk.append(acquire1)),
                Effect.as(new Service1()),
            ),
...
export const makeLayer2 = (
    ...
    return Layer.scoped(
        ...
        Effect.acquireRelease(
            ref.pipe(
                Ref.update(Chunk.append(acquire2)),
                Effect.as(new Service2()),
            ),
...
export const makeLayer3 = (
    ...
    return Layer.scoped(
        ...
        Effect.acquireRelease(
            ref.pipe(
                Ref.update(Chunk.append(acquire3)),
                Effect.as(new Service3()),
            ),
