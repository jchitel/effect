        ...
        Effect.gen(function* () {
            const result = yield* pipe(
                ...
                Stream.run(
                    pipe(
                        Sink.head(),
                        Sink.zipLeft(Sink.succeed("hello"), {
                            concurrent: true,
                        }),
                    ),
                ),
        ...
        Effect.gen(function* () {
            const result = yield* pipe(
                ...
                Stream.run(
                    pipe(
                        Sink.head(),
                        Sink.zipRight(Sink.succeed("hello"), {
                            concurrent: true,
                        }),
                    ),
                ),
    ...
    it.effect("zipWithPar - coherence", () =>
        Effect.gen(function* () {
            ...
            const chunk = pipe(
                ...
                Chunk.appendAll(
                    success1 ? Chunk.of(20) : Chunk.empty<number>(),
                ),
                Chunk.appendAll(
                    success2 ? Chunk.of(40) : Chunk.empty<number>(),
                ),
            );
