...
describe("Effect", () => {
    ...
    it.effect(
        "acquireUseRelease acquire returns immediately on interrupt",
        () =>
            Effect.gen(function* () {
                const deferred1 = yield* Deferred.make<void>();
    ...
    it.effect("interrupted cause persists after catching", () =>
        Effect.gen(function* () {
            const process = (
                ...
                return pipe(
                    list,
                    Chunk.map(Exit.mapErrorCause((cause) => cause)),
                );
            };
                    ...
                    Effect.flatMap((either) =>
                        Ref.set(
                            ...
                            Option.some(
                                pipe(
                                    either,
                                    Either.mapLeft(Cause.isInterrupted),
                                ),
                            ),
                        ),
                    ),
    ...
    it.effect("run of interruptible", () =>
        Effect.gen(function* () {
            const recovered = yield* Ref.make<Option.Option<boolean>>(
                Option.none(),
            );
            const fiber = yield* withLatch((release) =>
                pipe(
                    ...
                    Effect.flatMap((exit) =>
                        Ref.set(
                            recovered,
                            Option.some(Exit.isInterrupted(exit)),
                        ),
                    ),
                    Effect.uninterruptible,
                    Effect.fork,
