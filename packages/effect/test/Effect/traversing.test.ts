...
describe("Effect", () => {
    ...
    it.effect(
        "loop/discard - loops with the specified effectual function",
        () =>
            Effect.gen(function* () {
                const ref = yield* Ref.make(Array.empty<number>());
                yield* Effect.loop(0, {
                ...
                const result = yield* pipe(
                    ...
                    Effect.forEach((s) =>
                        pipe(
                            ...
                            Effect.zipRight(
                                Effect.sync(() => Number.parseInt(s)),
                            ),
                        ),
                    ),
                );
                const effects = yield* pipe(
                    Ref.get(ref),
                    Effect.map(Chunk.reverse),
                );
                deepStrictEqual(Chunk.toReadonlyArray(effects), [
                    "1",
                    "2",
                    "3",
                ]);
    ...
    it.effect("forEach - fails if one of the effects fails", () =>
        Effect.gen(function* () {
            ...
            deepStrictEqual(
                result,
                Exit.die(new Cause.IllegalArgumentException()),
            );
        }),
    );
    ...
    it.effect("forEach/discard - can be run twice", () =>
        Effect.gen(function* () {
            ...
            const effect = pipe(
                [1, 2, 3, 4, 5],
                Effect.forEach((n) => Ref.update(ref, (_) => _ + n), {
                    discard: true,
                }),
            );
            yield* effect;
        ...
        Effect.gen(function* () {
            ...
            assertTrue(
                Exit.isFailure(result) &&
                    Cause.isDie(result.effect_instruction_i0),
            );
        }),
    );
    ...
    it.effect("forEach/concurrency - parallelism - works on large lists", () =>
        Effect.gen(function* () {
            ...
            const result = yield* pipe(
                array,
                Effect.forEach((n) => Effect.succeed(n), {
    ...
    it.effect("forEach/concurrency+discard - accumulates errors", () =>
        Effect.gen(function* () {
            ...
            const result = yield* pipe(
                ...
                Effect.forEach(
                    (n) =>
                        pipe(task(started, trigger, n), Effect.uninterruptible),
                    {
                        concurrency: "unbounded",
                        discard: true,
                    },
    ...
    it.effect(
        "reduceEffect/concurrency - return error if zero is an error",
        () =>
            Effect.gen(function* () {
                const result = yield* pipe(
