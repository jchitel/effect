...
describe("Effect", () => {
            ...
            const fiber = yield* pipe(
                ...
                Effect.ensuring(
                    Effect.async<void, never, never>(() => {
                        ...
                        Runtime.runCallback(runtime)(
                            Deferred.succeed(step, undefined),
                        );
                    }),
                ),
    ...
    it.live(
        "async should not resume fiber twice after synchronous result",
        () =>
            Effect.gen(function* () {
                const step = yield* Deferred.make<void>();
                const unexpectedPlace = yield* Ref.make(Chunk.empty<number>());
                ...
                const fiber = yield* pipe(
                    ...
                    Effect.flatMap(() =>
                        Effect.async<void, never, never>(() => {
                            // The callback is never called so this never completes
                            Runtime.runCallback(runtime)(
                                Deferred.succeed(step, void 0),
                            );
                        }),
                    ),
                    Effect.ensuring(
                        Ref.update(unexpectedPlace, Chunk.prepend(2)),
                    ),
