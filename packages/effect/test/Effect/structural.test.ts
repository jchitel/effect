...
describe("Effect", () => {
    describe("all", () => {
        ...
        it.effect("should work with an array argument", () =>
            Effect.gen(function* () {
                const y = Effect.all(
                    [0, 1, 2].map((n) => Effect.succeed(n + 1)),
                );
                const x = yield* y;
        ...
        it.effect("failure should work with one array argument", () =>
            Effect.gen(function* () {
                const res = yield* Effect.flip(
                    Effect.all([Effect.fail(0), Effect.succeed(1)], {
                        mode: "validate",
                    }),
                );
                deepStrictEqual(res, [Option.some(0), Option.none()]);
                satisfies<true>(
                    assertType<[Option.Option<number>, Option.Option<never>]>()(
                        res,
                    ),
                );
            }),
    ...
    describe("all/ either mode", () => {
        it.effect("should work with one array argument", () =>
            Effect.gen(function* () {
                ...
                satisfies<true>(
                    assertType<
                        [Either.Either<number>, Either.Either<number>]
                    >()(res),
                );
            }),
        );
        it.effect("failure should work with one array argument", () =>
            Effect.gen(function* () {
                const res = yield* Effect.all(
                    [Effect.fail(0), Effect.succeed(1)],
                    { mode: "either" },
                );
                deepStrictEqual(res, [Either.left(0), Either.right(1)]);
                satisfies<true>(
                    assertType<
                        [Either.Either<never, number>, Either.Either<number>]
                    >()(res),
                );
            }),
        );
        ...
        it.effect("should work with one iterable argument", () =>
            Effect.gen(function* () {
                ...
                satisfies<true>(
                    assertType<Array<Either.Either<number>>>()(result),
                );
            }),
        );
    });
