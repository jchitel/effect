...
export interface GetParentChildren
    extends Request.Request<ReadonlyArray<Child>> {
        ...
        Effect.flatMap(Requests, (r) => {
            ...
            return counted(
                Effect.all([
                    Effect.forEach(
                        requests.filter(
                            (_): _ is GetParentChildren =>
                                _._tag === "GetParentChildren",
                        ),
                        (request) =>
                            Request.succeed(request, children.get(request.id)!),
                    ),
                    Effect.forEach(
                        requests.filter(
                            (_): _ is GetChildExtra =>
                                _._tag === "GetChildExtra",
                        ),
                        (request) =>
                    ...
                    Effect.forEach(
                        requests.filter(
                            (_): _ is GetAllParents =>
                                _._tag === "GetAllParents",
                        ),
