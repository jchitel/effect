...
describe("Effect", () => {
    ...
    it.effect("catchAllDefect - leaves errors", () =>
        Effect.gen(function* () {
            const error = new Cause.IllegalArgumentException(
                "division by zero",
            );
            const result = yield* pipe(
                Effect.fail(error),
    ...
    it.effect(
        "catch - does not recover from one of several tagged errors",
        () =>
            Effect.gen(function* () {
                interface ErrorA {
                    readonly _tag: "ErrorA";
    ...
    it.effect(
        ...
        () =>
            Effect.gen(function* () {
                ...
                deepStrictEqual(result, Exit.fail({ _tag: "ErrorB" as const }));
                satisfies<true>(
                    assertType<Exit.Exit<ErrorA, ErrorB>>()(result),
                );
            }),
    );
        ...
        Effect.gen(function* () {
            const result = yield* pipe(
                ...
                Effect.match({
                    ...
                    onSuccess: () =>
                        Option.none() as Option.Option<Cause.Cause<never>>,
                }),
            );
    ...
    it.effect("ignore - not catch throwable", () =>
        Effect.gen(function* () {
            const result = yield* Effect.exit(
                Effect.ignore(Effect.die(ExampleError)),
            );
                    ...
                    if (Cause.isDie(cause)) {
                        ...
                        if (Chunk.isNonEmpty(defects)) {
                            ...
                            return Effect.succeed(
                                (head as Cause.RuntimeException).message ===
                                    "2",
                            );
                        }
                    }
    ...
    it.effect(
        "orElseSucceed - otherwise succeeds with the specified value",
        () =>
            Effect.gen(function* () {
                const result = yield* pipe(
                    Effect.fail(false),
