...
export const getAllUserNamesN = (concurrency: Concurrency) =>
    getAllUserIds.pipe(
        ...
        Effect.onInterrupt(() =>
            FiberRef.getWith(interrupts, (i) =>
                Effect.sync(() => i.interrupts++),
            ),
        ),
    );
...
describe("Effect", () => {
    it.effect(
        ...
        () =>
            Effect.gen(function* () {
                class RequestUserById extends Request.TaggedClass(
                    "RequestUserById",
                )<
                    number,
                    never,
                ...
                const resolver = Resolver.makeBatched((i) => {
                    ...
                    return Effect.forEach(
                        i,
                        Request.complete(Exit.succeed(1)),
                        { concurrency: "unbounded" },
                    );
                });
                yield* Effect.request(
                    new RequestUserById({ id: "1" }),
                    resolver,
                ).pipe(Effect.withRequestCaching(true), Effect.repeatN(3));
                strictEqual(count, 1);
            }),
    );
        ...
        Effect.locally(interrupts, { interrupts: 0 })(
            provideEnv(
                Effect.gen(function* () {
                    ...
                    const cache = yield* FiberRef.get(
                    ...
                    deepStrictEqual(yield* FiberRef.get(interrupts), {
                        interrupts: 1,
                    });
                }),
            ),
