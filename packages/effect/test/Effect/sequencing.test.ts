...
describe("Effect", () => {
    it.effect("andThen", () =>
        Effect.gen(function* () {
            ...
            const a1 = Effect.succeed(0).pipe(
                Effect.andThen(Effect.succeed(1)),
            );
            const a2 = Effect.andThen(Effect.succeed(0), (n) =>
                Effect.succeed(n + 1),
            );
            ...
            const a8 = Effect.andThen(Effect.succeed(0), () =>
                Promise.resolve("ok"),
            );
    ...
    it.effect("tap", () =>
        Effect.gen(function* () {
            ...
            const a3 = Effect.tap(Effect.succeed(0), (n) =>
                Effect.succeed(n + 1),
            );
            const a4 = Effect.tap(
                Effect.succeed(0),
            ...
            const a11 = Effect.tap(Effect.succeed(0), () =>
                Promise.resolve("ok"),
            );
                ...
                Effect.gen(function* () {
                    ...
                    assertTrue(
                        Exit.isFailure(result) &&
                            Option.isSome(
                                Cause.dieOption(result.effect_instruction_i0),
                            ),
                    );
                    assertTrue(effect);
                }),
    ...
    it.effect(
        "unlessEffect - executes condition effect and correct branch",
        () =>
            Effect.gen(function* () {
                ...
                yield* pipe(
                    Ref.set(effectRef, 1),
                    Effect.unlessEffect(conditionTrue),
                );
    ...
    it.effect("whenEffect - executes condition effect and correct branch", () =>
        Effect.gen(function* () {
            ...
            yield* pipe(
                Ref.set(effectRef, 1),
                Effect.whenEffect(conditionFalse),
            );
            const v1 = yield* Ref.get(effectRef);
            const c1 = yield* Ref.get(conditionRef);
            yield* pipe(
    ...
    it.effect(
        ...
        () =>
            Effect.gen(function* () {
                ...
                const right = pipe(
                    Deferred.succeed(latch3, void 0),
                    Effect.as(42),
                );
                yield* pipe(
