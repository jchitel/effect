...
describe.concurrent("Micro", () => {
    ...
    describe("fromEither", () => {
        ...
        it("from a left", () =>
            Either.left("error").pipe(
                ...
                Micro.flip,
                Micro.tap((error) =>
                    Micro.sync(() => strictEqual(error, "error")),
                ),
                Micro.runPromise,
            ));
    });
    ...
    describe("forEach", () => {
        it("sequential", () =>
            Micro.gen(function* () {
                const results = yield* Micro.forEach([1, 2, 3], (_) =>
                    Micro.succeed(_),
                );
        ...
        it("length = 0", () =>
            Micro.gen(function* () {
                const results = yield* Micro.forEach([], (_) =>
                    Micro.succeed(_),
                );
    ...
    describe("filter", () => {
        ...
        it.live("iterable", () =>
            Micro.gen(function* () {
                const results = yield* Micro.filter(
                    new Set([1, 2, 3, 4, 5]),
                    (_) => Micro.succeed(_ % 2 === 1),
                );
                deepStrictEqual(results, [1, 3, 5]);
            }),
        );
    ...
    describe("interruption", () => {
        ...
        it.effect("AbortSignal is aborted", () =>
            Micro.gen(function* () {
                ...
                const fiber = yield* Micro.async<void, never, never>(
                    (_cb, signal_) => {
                        signal = signal_;
                    },
                ).pipe(Micro.fork);
    ...
    describe("finalization", () => {
        ...
        it.effect("finalizer errors reported", () =>
            Micro.gen(function* () {
                ...
                assertFalse(
                    reported !== undefined && Micro.exitIsSuccess(reported),
                );
            }),
        );
    ...
    describe("zipWith", () => {
        it.effect("concurrent: false", () => {
            ...
            return Micro.gen(function* () {
                const result = yield* Micro.zipWith(
                    task1,
                    task2,
                    (a, b) => a + b,
                );
    ...
    describe("catchAllCause", () => {
        it.effect("first argument as success", () =>
            Micro.gen(function* () {
                const result = yield* Micro.catchAllCause(
                    Micro.succeed(1),
                    () => Micro.fail("e2" as const),
                );
                deepStrictEqual(result, 1);
