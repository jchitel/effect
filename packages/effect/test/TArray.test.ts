...
describe("TArray", () => {
    ...
    it.effect("collectFirst - succeeds for empty array", () =>
        Effect.gen(function* () {
            const array = yield* makeTArray<Option.Option<number>>(
                0,
                Option.none(),
            );
        ...
        Effect.gen(function* () {
            ...
            assertTrue(
                (Option.isSome(result) &&
                    result.value === String(largePrime)) ||
    ...
    it.effect(
        "findFirstIndexFrom - correct index if in array, with offset",
        () =>
            Effect.gen(function* () {
                const array = yield* makeRepeats(3, 3);
                const result = yield* pipe(
                    array,
                    TArray.findFirstIndexFrom(2, 2),
                );
                assertSome(result, 4);
            }),
    );
    ...
    it.effect("findFirstIndexWhere - none for empty array", () =>
        Effect.gen(function* () {
            ...
            const result = yield* pipe(
                array,
                TArray.findFirstIndexWhere(constTrue),
            );
            assertNone(result);
        }),
    );
