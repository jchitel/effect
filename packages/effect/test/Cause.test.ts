...
describe("Cause", () => {
    ...
    describe("UnknownException", () => {
        it("exposes its `error` property", () => {
            strictEqual(
                new Cause.UnknownException("my message").error,
                "my message",
            );
        ...
        it("exposes its `cause` property", () => {
            strictEqual(
                new Cause.UnknownException("my message").cause,
                "my message",
            );
        ...
        it("accepts a custom override message", () => {
            strictEqual(
                new Cause.UnknownException(new Error("my error"), "my message")
                    .message,
                "my message",
            );
        });
    ...
    it("[internal] prettyErrorMessage converts errors into readable JSON-like strings", () => {
        ...
        strictEqual(
            internal.prettyErrorMessage(new Error1()),
            `{"_tag":"WithTag"}`,
        );
        class Error2 {
        ...
        strictEqual(
            internal.prettyErrorMessage(new Error5()),
            `Error: my string`,
        );
    });
    ...
    describe("Cause prototype", () => {
        describe("toJSON / [NodeInspectSymbol]", () => {
            const expectJSON = (
                ...
                deepStrictEqual(
                    cause[Inspectable.NodeInspectSymbol](),
                    expected,
                );
            };
            ...
            it("Sequential", () => {
                expectJSON(
                    Cause.sequential(
                        Cause.fail("failure 1"),
                        Cause.fail("failure 2"),
                    ),
                    {
                        _id: "Cause",
                        _tag: "Sequential",
    ...
    describe("Getters", () => {
        it("isEmpty", () => {
            ...
            assertTrue(
                Cause.isEmpty(
                    Cause.parallel(empty, Cause.sequential(empty, empty)),
                ),
            );
            assertTrue(
                Cause.isEmpty(
            ...
            assertFalse(
                Cause.isEmpty(
                    Cause.parallel(empty, Cause.sequential(empty, failure)),
                ),
            );
            assertFalse(
                Cause.isEmpty(
                    Cause.sequential(empty, Cause.parallel(empty, failure)),
                ),
            );
        });
        ...
        it("isDie", () => {
            ...
            assertTrue(
                Cause.isDie(
                    Cause.parallel(empty, Cause.sequential(empty, defect)),
                ),
            );
            assertTrue(
                Cause.isDie(
                    Cause.sequential(empty, Cause.parallel(empty, defect)),
                ),
            );
        ...
        it("isInterrupted", () => {
            ...
            assertTrue(
                Cause.isInterrupted(Cause.sequential(empty, interruption)),
            );
            assertTrue(
                Cause.isInterrupted(Cause.parallel(empty, interruption)),
            );
            assertTrue(
                Cause.isInterrupted(
                    Cause.parallel(
                        empty,
                        Cause.sequential(empty, interruption),
                    ),
                ),
            );
            assertTrue(
                Cause.isInterrupted(
                    Cause.sequential(
                        empty,
                        Cause.parallel(empty, interruption),
                    ),
                ),
            );

            assertTrue(
                Cause.isInterrupted(Cause.sequential(failure, interruption)),
            );
            assertTrue(
                Cause.isInterrupted(Cause.parallel(failure, interruption)),
            );
            assertTrue(
                Cause.isInterrupted(
                    Cause.parallel(
        ...
        it("isInterruptedOnly", () => {
            ...
            assertTrue(
                Cause.isInterruptedOnly(Cause.sequential(empty, interruption)),
            );
            assertTrue(
                Cause.isInterruptedOnly(Cause.parallel(empty, interruption)),
            );
            assertTrue(
            ...
            assertFalse(
                Cause.isInterruptedOnly(
                    Cause.sequential(failure, interruption),
                ),
            );
            assertFalse(
        ...
        it("interruptors", () => {
            ...
            expectInterruptors(
                Cause.sequential(
                    ...
                    Cause.parallel(
                        empty,
                        Cause.interrupt(FiberId.runtime(2, 0)),
                    ),
                ),
        ...
        it("keepDefects", () => {
            ...
            assertSome(
                Cause.keepDefects(Cause.sequential(defect, failure)),
                defect,
            );
            assertNone(Cause.keepDefects(Cause.parallel(empty, empty)));
            assertSome(
                Cause.keepDefects(Cause.parallel(defect, failure)),
                defect,
            );
            assertSome(
                Cause.keepDefects(
        ...
        it("linearize", () => {
            ...
            expectLinearize(
                Cause.sequential(
                    failure,
                    Cause.sequential(interruption, defect),
                ),
                [
                    Cause.sequential(
                        failure,
        ...
        it("stripSomeDefects", () => {
            ...
            const stripNumberFormatException = Cause.stripSomeDefects(
                (defect) =>
            ...
            assertNone(
                stripNumberFormatException(Cause.sequential(cause1, cause1)),
            );
            assertSome(
            ...
            assertNone(
                stripNumberFormatException(Cause.parallel(cause1, cause1)),
            );
            assertSome(
            ...
            it("is associative", () => {
                fc.assert(
                    fc.property(
                        ...
                        errorCauseFunctions,
                        (cause, f, g) => {
                            const left = cause.pipe(
                                Cause.flatMap(f),
                                Cause.flatMap(g),
                            );
                            const right = cause.pipe(
                                Cause.flatMap((error) =>
                                    f(error).pipe(Cause.flatMap(g)),
                                ),
                            );
                            assertTrue(Equal.equals(left, right));
                        },
                ...
                describe("Fail", () => {
                    it("no cause", () => {
                        const pretty = Cause.pretty(
                            Cause.fail(new Error("my message")),
