...
describe("STM", () => {
    ...
    it.effect("collectAll - ordering", () =>
        Effect.gen(function* () {
            const transaction = pipe(
                ...
                STM.flatMap((queue) =>
                    STM.all(
                        Array.from({ length: 3 }, () => TQueue.take(queue)),
                    ),
                ),
            );
            const result = yield* STM.commit(transaction);
    ...
    it.effect(
        "filter - filters a collection using an effectual predicate",
        () =>
            Effect.gen(function* () {
    ...
    it.effect(
        "filterOrDieMessage - dies with message when predicate fails",
        () =>
            Effect.gen(function* () {
                const transaction = pipe(
                    STM.succeed(1),
                ...
                deepStrictEqual(
                    result,
                    Exit.die(new Cause.RuntimeException("Ouch")),
                );
            }),
    );
    ...
    it.effect(
        "orElseFail - fails with the specified error once left fails",
        () =>
            Effect.gen(function* () {
                const transaction = pipe(
                    STM.fail(true),
    ...
    it.effect("unsome - converts None in E to None in A", () =>
        Effect.gen(function* () {
            const transaction = STM.unsome(
                STM.fromEither(Either.left(Option.none())),
            );
            const result = yield* STM.commit(transaction);
            assertNone(result);
        }),
        ...
        Effect.gen(function* () {
            ...
            const transaction = STM.gen(function* () {
                ...
                yield* pipe(
                    input,
                    STM.partition((n) =>
                        pipe(ref, TRef.update(Chunk.append(n))),
                    ),
                );
    ...
    it.effect("reject - doesnt collect value", () =>
        Effect.gen(function* () {
            const transaction = pipe(
                STM.succeed(0),
                STM.reject((n) =>
                    n !== 0 ? Option.some("Ouch") : Option.none(),
                ),
            );
            const result = yield* STM.commit(transaction);
    ...
    it.effect("repeatWhile - runs effect while it satisfies predicate", () =>
        Effect.gen(function* () {
            const transaction = pipe(
                ...
                STM.tap((queue) =>
                    pipe(queue, TQueue.offerAll([0, 0, 0, 1, 2])),
                ),
                STM.flatMap((queue) =>
                    pipe(
                        TQueue.take(queue),
        ...
        Effect.gen(function* () {
            const transaction = STM.gen(function* () {
                ...
                const result = yield* pipe(
                    ...
                    STM.summarized(
                        increment,
                        (before, after) => [before, after] as const,
                    ),
                );
            ...
            Effect.gen(function* () {
                const transaction = STM.gen(function* () {
                    ...
                    const result = yield* pipe(
                        ...
                        STM.tapBoth({
                            onFailure: (e: string) =>
                                pipe(tapError, TDeferred.succeed(e)),
                            onSuccess: (n) =>
                                pipe(tapSuccess, TDeferred.succeed(n)),
                        }),
                    );
            ...
            Effect.gen(function* () {
                const transaction = STM.gen(function* () {
                    ...
                    const result = yield* pipe(
                        ...
                        STM.tapError((e) =>
                            pipe(errorRef, TDeferred.succeed(e)),
                        ),
                        STM.either,
                    );
    ...
    it.effect("stack-safety - long provideEnvironment chains", () =>
        Effect.gen(function* () {
            const result = yield* chain(10_000)(
                STM.provideContext(Context.empty()),
            );
            strictEqual(result, 0);
        }),
    );
    ...
    it.effect("permutations - permutes two variables in 100 fibers", () =>
        Effect.gen(function* () {
            ...
            const fiber = yield* pipe(
                Array.from({ length: 100 }, () =>
                    STM.commit(permutation(ref1, ref2)),
                ),
                Effect.forkAll(),
            );
            yield* Fiber.join(fiber);
