...
describe("RateLimiter", () => {
    ...
    describe.concurrent("token-bucket", () => {
        ...
        it.scoped(
            "uses the token-bucket algorithm for token replenishment",
            () =>
                Effect.scoped(
                    Effect.gen(function* () {
                        ...
                        yield* Effect.forEach(Array.range(1, 10), () =>
                            limit(Effect.void),
                        );
...
const RateLimiterTestSuite = (algorithm: "fixed-window" | "token-bucket") => {
    ...
    it.scoped(`${algorithm} - fail with the result of a failed call`, () =>
        Effect.gen(function* () {
            ...
            const result = yield* limit(
                Effect.either(Effect.fail(Option.none())),
            );
        ...
        () =>
            Effect.gen(function* () {
                ...
                const fib = yield* pipe(
                    ...
                    Effect.onInterrupt(() =>
                        Deferred.succeed(interrupted, void 0),
                    ),
                    limit,
                    Effect.fork,
                );
