...
describe("Command", () => {
        ...
        runPromise(
            Effect.gen(function* (_) {
                ...
                const result = yield* _(
                    Stream.runCollect(Command.streamLines(command)),
                );
            ...
            Effect.gen(function* (_) {
                ...
                const command = pipe(
                    ...
                    Command.workingDirectory(
                        path.join(...TEST_BASH_SCRIPTS_PATH),
                    ),
                );
    ...
    it("should delegate workingDirectory to all commands", () => {
        ...
        const directories = Command.flatten(command).map(
            (command) => command.cwd,
        );
    ...
    it("should delegate stderr to the right-most command", () => {
        ...
        const stderr = Command.flatten(command).map(
    ...
    it("should delegate stdout to the right-most command", () => {
        ...
        const stdout = Command.flatten(command).map(
