    ...
    it("readFile", () =>
        runPromise(
            Effect.gen(function* (_) {
                ...
                const data = yield* _(
                    fs.readFile(`${__dirname}/fixtures/text.txt`),
                );
            ...
            Effect.gen(function* (_) {
                ...
                yield* _(
                    Effect.gen(function* (_) {
                        let text: string;
                        const file = yield* _(
                            fs.open(`${__dirname}/fixtures/text.txt`),
                        );
            ...
            Effect.gen(function* (_) {
                ...
                yield* _(
                    Effect.gen(function* (_) {
                        ...
                        yield* _(
                            file.write(new TextEncoder().encode("lorem ipsum")),
                        );
                        yield* _(file.write(new TextEncoder().encode(" ")));
                        yield* _(
                            file.write(
                                new TextEncoder().encode("dolor sit amet"),
                            ),
                        );
            ...
            Effect.gen(function* (_) {
                ...
                yield* _(
                    Effect.gen(function* (_) {
                        ...
                        const cursor = yield* _(
                            file.seek(Fs.Size(0), "current"),
                        );
                            ...
                            const fiber = yield* fs
                                ...
                                .pipe(
                                    Stream.tap((event) =>
                                        Effect.sync(() => {
                                            if (
                                                event.path.includes("root.txt")
                                            ) {
                                                rootEventCount++;
                                            }
                                            if (
                                                event.path.includes("sub.txt")
                                            ) {
                                                subEventCount++;
                                            }
                                        }),
