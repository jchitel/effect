...
>(
    ...
    Channel.suspend(() => {
        ...
        const exit = MutableRef.make<Exit.Exit<void, IE | E> | undefined>(
            undefined,
        );
        return Channel.embedInput(
            ...
            writeInput<IE, I>(
                ...
                (cause) =>
                    Effect.sync(() =>
                        MutableRef.set(exit, Exit.failCause(cause)),
                    ),
                options,
            ),
        );
...
class StreamAdapter<E, R> extends Readable {
    ...
    _destroy(
        ...
        Effect.runFork(Fiber.interrupt(this.fiber)).addObserver((exit) => {
            callback(
                exit._tag === "Failure"
                    ? (Cause.squash(exit.cause) as any)
                    : error,
            );
        });
    }
