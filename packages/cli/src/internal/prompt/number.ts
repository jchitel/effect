...
import { Array as Arr, Effect, Number as EffectNumber, Option, Schema } from "effect";
...
function handleClear(options: IntegerOptions) {
    return (state: State, _: Prompt.Prompt.Action<State, number>) => {
        return Effect.gen(function* () {
            ...
            const clearOutput = InternalAnsiUtils.eraseText(
                options.message,
                columns,
            );
...
export const integer = (
    ...
    const opts: IntegerOptions = {
        ...
        validate: (n) => {
            if (n < opts.min) {
                return Effect.fail(
                    `${n} must be greater than or equal to ${opts.min}`,
                );
            }
            if (n > opts.max) {
                return Effect.fail(
                    `${n} must be less than or equal to ${opts.max}`,
                );
            }
            ...
            case "return": {
                return Effect.matchEffect(parseFloat(state.value), {
                        ...
                        Effect.succeed(
                            Action.NextFrame({
                                state: {
                                    ...state,
                                    error: Option.some(
                                        "Must provide a floating point value",
                                    ),
                                },
                            }),
                        ),
                    onSuccess: (n) =>
                        Effect.flatMap(
                            Effect.sync(() =>
                                EffectNumber.round(n, options.precision),
                            ),
                            (rounded) =>
