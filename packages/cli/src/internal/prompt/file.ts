...
import { Array as Arr, Data, Effect, Option, identity } from "effect";
...
interface FileOptions
    extends Required<Omit<Prompt.Prompt.FileOptions, "startingPath">> {
...
function handleClear(options: FileOptions) {
    return (state: State, _: Prompt.Prompt.Action<State, string>) => {
        return Effect.gen(function* () {
            ...
            const text = "\n".repeat(
                Math.min(state.files.length, options.maxPerPage),
            );
