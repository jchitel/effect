...
import { Array as Arr, Effect, Number, Option } from "effect";
        ...
        onSome: (error) =>
            Arr.match(error.split(NEWLINE_REGEX), {
                ...
                onNonEmpty: (errorLines) => {
                    ...
                    const prefix = Doc.cat(
                        Doc.annotate(pointer, Ansi.red),
                        Doc.space,
                    );
                    const lines = Arr.map(errorLines, (str) =>
                        annotateLine(str),
                    );
...
function renderChoices<A>(
    ...
    const inverseSelectionText =
        options?.inverseSelection ?? "Inverse Selection";
    ...
    for (
        let index = toDisplay.startIndex;
        index < toDisplay.endIndex;
        index++
    ) {
...
export function handleClear<A>(options: SelectOptions<A>) {
    return Effect.gen(function* () {
        ...
        const text =
            "\n".repeat(
                Math.min(options.choices.length + 2, options.maxPerPage),
            ) +
            options.message +
            1;
                    ...
                    return Effect.succeed(
                        Action.NextFrame({
                            state: {
                                ...state,
                                error: Option.some(
                                    `At least ${options.min} are required`,
                                ),
                            },
                        }),
                    );
...
export const multiSelect = <const A>(
    ...
    return InternalPrompt.custom(
        { index: 0, selectedIndices: new Set<number>(), error: Option.none() },
        {
