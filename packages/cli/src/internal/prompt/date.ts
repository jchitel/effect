...
import { Match, Array as Arr, Effect, Option } from "effect";
...
export const date = (
    ...
    const initialCursorPosition = dateParts.findIndex(
        (part) => !part.isToken(),
    );
...
class Milliseconds extends DatePart {
    ...
    setValue(value: string): void {
        this.date.setMilliseconds(
            Number.parseInt(value.slice(-this.token.length)),
        );
    }
