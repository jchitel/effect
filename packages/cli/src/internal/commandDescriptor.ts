        ...
        case "Subcommands": {
            const getUsage = (
                ...
                switch (command._tag) {
                    ...
                    case "GetUserInput": {
                        ...
                        const description = InternalHelpDoc.getSpan(
                            command.description,
                        );
            ...
            return InternalHelpDoc.sequence(
                ...
                InternalHelpDoc.sequence(
                    ...
                    printSubcommands(
                        Arr.flatMap(self.children, (child) =>
                            getUsage(child, Arr.empty()),
                        ),
                    ),
                ),
            );
        ...
        Arr.matchLeft(args, {
            ...
            onNonEmpty: (head, tail) => {
                ...
                return Effect.succeed(tail).pipe(
                    Effect.when(
                        () => normalizedArgv0 === normalizedCommandName,
                    ),
                    Effect.flatten,
                ...
                Arr.matchLeft(args, {
                    ...
                    onNonEmpty: (argv0) => {
                        ...
                        const normalizedCommandName =
                            InternalCliConfig.normalizeCase(config, self.name);
            ...
            > =>
                parseCommandLine(self, args).pipe(
                    Effect.flatMap((commandOptionsAndArgs) => {
                        const [optionsAndArgs, forcedCommandArgs] =
                            splitForcedArgs(commandOptionsAndArgs);
                        return InternalOptions.processCommandLine(
                            self.options,
                            optionsAndArgs,
                            config,
                        ...
                        switch (directive._tag) {
                            case "BuiltIn": {
                                if (
                                    ...
                                    return Arr.isNonEmptyReadonlyArray(
                                        childArgs,
                                    )
                                        ? Effect.orElse(
                            ...
                            case "UserDefined": {
                                const args = Arr.appendAll(
                                    directive.leftover,
                                    childArgs,
                                );
                                if (Arr.isNonEmptyReadonlyArray(args)) {
                                    return parseChildren.pipe(
                                        Effect.mapBoth({
                                            onFailure: (err) => {
                                                if (
                                                    InternalValidationError.isCommandMismatch(
                                                    ...
                                                    return InternalValidationError.commandMismatch(
                                                        error,
                                                    );
                                                }
                                                return err;
    ...
    > => {
        switch (self._tag) {
                ...
                return Effect.gen(function* () {
                    const logCurrentCommand = Ref.get(commandLineRef).pipe(
                        Effect.flatMap((commandLine) => {
                            ...
                            return Console.log(
                                InternalHelpDoc.toAnsiText(currentCommand),
                            );
                        }),
                    );
                    if (isStandard(self)) {
                        ...
                        const commandName = InternalSpan.highlight(
                            self.name,
                            Color.magenta,
                        );
                        ...
                        if (
                            ...
                            yield* Console.log(
                                InternalHelpDoc.toAnsiText(message),
                            );
                        ...
                        if (
                            ...
                            const options = yield* InternalArgs.wizard(
                                self.args,
                                config,
                            );
                            yield* Ref.updateAndGet(
            ...
            case "Subcommands": {
                ...
                const message =
                    InternalHelpDoc.toAnsiText(description).trimEnd();
    ...
    return Ref.make(prefix).pipe(
        Effect.flatMap((commandLineRef) =>
            loop(self, commandLineRef).pipe(
                Effect.zipRight(
                    Ref.get(commandLineRef) as Effect.Effect<Array<string>>,
                ),
            ),
        ),
    );
...
const getShortDescription = (self: Instruction): string => {
    switch (self._tag) {
        case "Standard": {
            return InternalSpan.getText(
                InternalHelpDoc.getSpan(self.description),
            );
        }
        case "GetUserInput": {
            return InternalSpan.getText(
...
const traverseCommand = <S>(
    ...
    SynchronizedRef.make(initialState).pipe(
        Effect.flatMap((ref) => {
            ...
            return Effect.suspend(() =>
                loop(self, Arr.empty(), Arr.empty(), 0),
            ).pipe(Effect.zipRight(SynchronizedRef.get(ref)));
        }),
    );
    ...
    switch (self._tag) {
        ...
        case "GetUserInput": {
            ...
            const argCompletions = pipe(
                InternalArgs.getZshCompletions(
                    args as InternalArgs.Instruction,
                ),
                Arr.map((completion) => `'${completion}' \\`),
            );
