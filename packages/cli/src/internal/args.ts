...
import { Array as Arr, Inspectable, ParseResult, Ref } from "effect";
...
/** @internal */
export interface WithFallbackConfig
    extends Op<
        "WithFallbackConfig",
        {
            readonly args: Args.Args<unknown>;
...
export const fileText = (
    ...
    mapEffect(file({ ...config, exists: "yes" }), (path) =>
        Effect.mapError(InternalFiles.readString(path), (e) =>
            InternalHelpDoc.p(e),
        ),
    );
...
const getMinSizeInternal = (self: Instruction): number => {
    switch (self._tag) {
        ...
        case "Variadic": {
            ...
            return Math.floor(
                Option.getOrElse(self.min, () => 0) * argsMinSize,
            );
        }
    }
};
    ...
    switch (self._tag) {
        ...
        case "Variadic": {
            return InternalUsage.repeated(
                getUsageInternal(self.args as Instruction),
            );
        }
        case "WithDefault":
        case "WithFallbackConfig": {
        ...
        case "Single": {
            return Effect.suspend(() => {
                return Arr.matchLeft(args, {
                    onEmpty: () => {
                        const choices = InternalPrimitive.getChoices(
                            self.primitiveType,
                        );
                        if (
                            Option.isSome(self.pseudoName) &&
                            Option.isSome(choices)
                        ) {
                            return Effect.fail(
            ...
            return validateInternal(
                ...
                Effect.flatMap(([leftover, a]) =>
                    Effect.matchEffect(self.f(a), {
                        onFailure: (doc) =>
                            Effect.fail(
                                InternalValidationError.invalidArgument(doc),
                            ),
            ...
            return validateInternal(
                ...
                Effect.flatMap(([args, a]) =>
                    validateInternal(
                        ...
                        args,
                        config,
                    ).pipe(Effect.map(([args, b]) => [args, [a, b]])),
                ),
            );
        }
        case "Variadic": {
            const min1 = Option.getOrElse(self.min, () => 0);
            const max1 = Option.getOrElse(
                self.max,
        ...
        case "Single": {
            ...
            return InternalPrimitive.wizard(self.primitiveType, help).pipe(
                ...
                Effect.flatMap((input) => {
                    ...
                    return validateInternal(self, args, config).pipe(
                        Effect.as(args),
                    );
                }),
            );
                        ...
                        : Ref.make(Arr.empty<string>()).pipe(
                              Effect.flatMap((ref) =>
                                  wizardInternal(
                                      ...
                                      Effect.tap((args) =>
                                          validateInternal(self, args, config),
                                      ),
                                  ),
                              ),
                          ),
