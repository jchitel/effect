...
import type { Cause, Mailbox } from 'effect';
import { Effect, Effectable, Pipeable } from "effect";
...
export interface Loop
    extends Op<
        "Loop",
        {
            ...
            readonly process: Prompt.Prompt.Handlers<
                unknown,
                unknown
            >["process"];
...
export const custom = <State, Output>(
    initialState:
        | State
        | Effect.Effect<State, never, Prompt.Prompt.Environment>,
...
const runLoop = Effect.fnUntraced(
    ...
    (effect, _, terminal) =>
        Effect.ensuring(
            ...
            Effect.orDie(
                terminal.display(
                    Doc.render(Doc.cursorShow, { style: "pretty" }),
                ),
            ),
        ),
);
