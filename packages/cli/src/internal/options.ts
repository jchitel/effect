    ...
    switch (self._tag) {
            ...
            return InternalHelpDoc.descriptionList(
                Arr.of([
                    ...
                    InternalHelpDoc.sequence(
                        InternalHelpDoc.p(
                            InternalPrimitive.getHelp(self.primitiveType),
                        ),
                        self.description,
                    ),
                ]),
            ...
            return InternalHelpDoc.mapDescriptionList(
                ...
                (span, oldBlock) => {
                    ...
                    const description = InternalHelpDoc.enumeration([
                        InternalHelpDoc.p(
                            `May be specified a single time:  '${single}'`,
                        ),
                        InternalHelpDoc.p(
                            `May be specified multiple times: '${multiple}'`,
                        ),
                    ]);
                    const newBlock = pipe(
                        oldBlock,
        ...
        case "WithDefault": {
            return InternalHelpDoc.mapDescriptionList(
                ...
                (span, block) => {
                    const optionalDescription = Option.isOption(self.fallback)
                        ? Option.match(self.fallback, {
                              ...
                              onSome: (fallbackValue) => {
                                  const inspectableValue = Predicate.isObject(
                                      fallbackValue,
                                  )
                                      ? fallbackValue
                    ...
                    return [
                        span,
                        InternalHelpDoc.sequence(block, optionalDescription),
                    ];
                },
            );
        }
        case "WithFallback": {
            ...
            return InternalHelpDoc.mapDescriptionList(
                ...
                (span, block) => [
                    span,
                    InternalHelpDoc.sequence(block, helpDoc),
                ],
            );
        }
    }
    ...
    switch (self._tag) {
        ...
        case "KeyValueMap": {
            ...
            > => {
                ...
                return Effect.fail(
                    InternalValidationError.invalidArgument(error),
                );
            };
                ...
                Effect.matchEffect({
                    onFailure: (e) =>
                        InternalValidationError.isMultipleValuesDetected(e)
                            ? Effect.forEach(e.values, (kv) =>
                                  extractKeyValue(kv),
                              ).pipe(Effect.map(HashMap.fromIterable))
                            : Effect.fail(e),
                    onSuccess: (kv) =>
                        extractKeyValue(kv as string).pipe(
                            Effect.map(HashMap.make),
                        ),
                }),
            );
        }
        case "Map": {
            return parseInternal(
                self.options as Instruction,
                args,
                config,
            ).pipe(Effect.flatMap((a) => self.f(a)));
        }
        case "Both": {
            return parseInternal(self.left as Instruction, args, config).pipe(
                Effect.catchAll((err1) =>
                    parseInternal(self.right as Instruction, args, config).pipe(
                        Effect.matchEffect({
                            onFailure: (err2) => {
                                const error = InternalHelpDoc.sequence(
                                    err1.error,
                                    err2.error,
                                );
                                return Effect.fail(
                ...
                Effect.zip(
                    parseInternal(self.right as Instruction, args, config),
                ),
            );
        }
        case "OrElse": {
                ...
                Effect.matchEffect({
                    onFailure: (error) =>
                        InternalValidationError.isMultipleValuesDetected(error)
                            ...
                            : Effect.fail(error),
                    onSuccess: (value) =>
                        validateMinMax(Arr.of(value as string)),
                }),
            );
        }
        case "WithDefault": {
                    ...
                    self.effect.pipe(
                        Effect.catchAll((e2) => {
                            ...
                            if (
                                ...
                                const error =
                                    InternalValidationError.invalidValue(help);
        ...
        case "Single": {
            ...
            return InternalPrimitive.wizard(self.primitiveType, help).pipe(
                Effect.flatMap((input) => {
                    ...
                    return parseCommandLine(self, args, config).pipe(
                        Effect.as(args),
                    );
                }),
                ...
                Effect.flatMap((args) =>
                    Arr.matchLeft(args, {
                        onEmpty: () => {
                            ...
                            return Effect.fail(
                                InternalValidationError.missingFlag(error),
                            );
                        },
                        onNonEmpty: (head, tail) => {
                            ...
                            if (Arr.contains(normalizedNames, normalizedHead)) {
                                if (
                                    InternalPrimitive.isBool(self.primitiveType)
                                ) {
                                    return Arr.matchLeft(tail, {
                                        ...
                                        onNonEmpty: (value, leftover) => {
                                            ...
                                            if (
                                                InternalPrimitive.isFalseValue(
                                                    value,
                                                )
                                            ) {
                                                const parsed = Option.some({
    ...
    switch (self._tag) {
        ...
        case "Single": {
            ...
            return pipe(
                ...
                Arr.sort(order),
                Arr.flatMap(([isShort, name]) =>
                    Arr.make(isShort ? "-s" : "-l", name),
                ),
                Arr.appendAll(
