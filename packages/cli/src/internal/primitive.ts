...
const validateInternal = (
    ...
    switch (self._tag) {
        ...
        case "Integer": {
            const intFromString = Schema.compose(
...
const validatePathType = (
    ...
    switch (pathType) {
        case "file": {
            ...
            return Effect.fail(
                `Expected path '${path}' to be a regular file`,
            ).pipe(Effect.unlessEffect(checkIsFile), Effect.asVoid);
        }
        case "directory": {
            ...
            ).pipe(Effect.unlessEffect(checkIsDirectory), Effect.asVoid);
        }
        case "either": {
            return Effect.void;
        }
    ...
    switch (self._tag) {
        ...
        case "Path": {
            switch (self.pathType) {
                case "file": {
                    return self.pathExists === "yes" ||
                        self.pathExists === "either"
                        ? '$(compgen -f "${cur}")'
                        : "";
                }
                case "directory": {
