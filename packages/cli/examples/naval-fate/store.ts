...
export const make = Effect.gen(function* () {
    const shipsStore = yield* Effect.map(KeyValueStore.KeyValueStore, (store) =>
        store.forSchema(
            Schema.ReadonlyMap({ key: Schema.String, value: Ship }),
        ),
    );
    ...
    const getShips = shipsStore
        ...
        .pipe(
            Effect.map(
                Option.getOrElse<ReadonlyMap<string, Ship>>(() => new Map()),
            ),
            Effect.orDie,
        );
        ...
        Effect.gen(function* () {
            ...
            if (Option.isNone(foundShip)) {
                return yield* Effect.fail(
                    new ShipNotFoundError({ name, x, y }),
                );
            }
            ...
            if (Option.isSome(shipAtCoords)) {
                return yield* Effect.fail(
                    new CoordinatesOccupiedError({
                        name: shipAtCoords.value.name,
                        x,
                        y,
                    }),
                );
