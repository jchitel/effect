...
const runEffect = <E, A>(
    ...
    Effect.provide(self, NodeContext.layer).pipe(Effect.runPromise);
...
describe("Options", () => {
    it("should validate without ambiguity", () =>
        Effect.gen(function* (_) {
            ...
            const expected1 = [
                Array.empty(),
                Array.make("--lastName", "--firstName"),
            ];
            const expected2 = [
                Array.empty(),
                Array.make("--firstName", "--lastName"),
            ];
    ...
    it("should return a HelpDoc if an option is not an exact match and it's a short option", () =>
        Effect.gen(function* (_) {
            ...
            const result = yield* _(
                Effect.flip(process(age, args, CliConfig.defaultConfig)),
            );
    ...
    it("should return a HelpDoc if there is a collision between arguments", () =>
        Effect.gen(function* (_) {
            ...
            const result = yield* _(
                Effect.flip(process(options, args, CliConfig.defaultConfig)),
            );
            expect(result).toEqual(
    ...
    it("validates a boolean option without a value", () =>
        Effect.gen(function* (_) {
            ...
            const result = yield* _(
                process(verbose, args, CliConfig.defaultConfig),
            );
    ...
    it("validates a boolean option with a followup option", () =>
        Effect.gen(function* (_) {
            ...
            const result1 = yield* _(
                process(options, args1, CliConfig.defaultConfig),
            );
            const result2 = yield* _(
                process(options, args2, CliConfig.defaultConfig),
            );
            const result3 = yield* _(
                process(options, args3, CliConfig.defaultConfig),
            );
    ...
    it("validates a boolean option with negation", () =>
        Effect.gen(function* (_) {
            ...
            const result2 = yield* _(
                process(option, ["--verbose"], CliConfig.defaultConfig),
            );
            const result3 = yield* _(
                process(option, ["-v"], CliConfig.defaultConfig),
            );
            const result4 = yield* _(
                process(option, ["--silent"], CliConfig.defaultConfig),
            );
            const result5 = yield* _(
                process(option, ["-s"], CliConfig.defaultConfig),
            );
    ...
    it("validates a text option with an alias", () =>
        Effect.gen(function* (_) {
            const result = yield* _(
                process(firstName, ["-f", "John"], CliConfig.defaultConfig),
            );
            expect(result).toEqual([[], "John"]);
        }).pipe(runEffect));
    ...
    it("validates an integer option", () =>
        Effect.gen(function* (_) {
            const result = yield* _(
                process(age, ["--age", "100"], CliConfig.defaultConfig),
            );
    ...
    it("validates with case-sensitive configuration", () =>
        Effect.gen(function* (_) {
            ...
            const result3 = yield* _(
                Effect.flip(process(option, args3, config)),
            );
            const result4 = yield* _(
                Effect.flip(process(option, args4, config)),
            );
    ...
    it("validates an unsupplied optional option", () =>
        Effect.gen(function* (_) {
            const result = yield* _(
                process(ageOptional, [], CliConfig.defaultConfig),
            );
    ...
    it("validates an unsupplied optional option with remainder", () =>
        Effect.gen(function* (_) {
            ...
            const result = yield* _(
                process(ageOptional, args, CliConfig.defaultConfig),
            );
    ...
    it("validates using all and returns the specified structure", () =>
        Effect.gen(function* (_) {
            ...
            const args = Array.make("--firstName", "John", "--lastName", "Doe");
            const result1 = yield* _(
                process(option1, args, CliConfig.defaultConfig),
            );
            const result2 = yield* _(
                process(option2, args, CliConfig.defaultConfig),
            );
    ...
    it("validate provides a suggestion if a provided option is close to a specified option", () =>
        Effect.gen(function* (_) {
            ...
            const result = yield* _(
                Effect.flip(process(firstName, args, CliConfig.defaultConfig)),
            );
            expect(result).toEqual(
                ValidationError.correctedFlag(
                    HelpDoc.p(
    ...
    it("orElse - no options provided", () =>
        Effect.gen(function* (_) {
            ...
            const result = yield* _(
                Effect.flip(process(option, [], CliConfig.defaultConfig)),
            );
    ...
    it("keyValueMap - validates a missing option", () =>
        Effect.gen(function* (_) {
            const result = yield* _(
                Effect.flip(process(defs, [], CliConfig.defaultConfig)),
            );
            expect(result).toEqual(
                ValidationError.missingValue(
    ...
    it("keyValueMap - validates repeated values", () =>
        Effect.gen(function* (_) {
            ...
            const result = yield* _(
                process(defs, args, CliConfig.defaultConfig),
            );
            expect(result).toEqual([
                ["--verbose"],
    ...
    it("repeated", () =>
        Effect.gen(function* (_) {
            ...
            const result3 = yield* _(
                Effect.flip(process(option, args3, CliConfig.defaultConfig)),
            );
            const result4 = yield* _(
                Effect.flip(process(option, args4, CliConfig.defaultConfig)),
            );
            const result5 = yield* _(
                process(option, args5, CliConfig.defaultConfig),
            );
            const result6 = yield* _(
                process(option, args6, CliConfig.defaultConfig),
            );
            expect(result1).toEqual([Array.empty(), []]);
            expect(result2).toEqual([Array.empty(), [1, 2, 3]]);
            expect(result3).toEqual(
                ValidationError.invalidValue(
                    HelpDoc.p("'v2' is not a integer"),
                ),
            );
            expect(result4).toEqual(
                ValidationError.invalidValue(
    ...
    it("atLeast", () =>
        Effect.gen(function* (_) {
            ...
            const result2 = yield* _(
                Effect.flip(process(option, args2, CliConfig.defaultConfig)),
            );
    ...
    it("between", () =>
        Effect.gen(function* (_) {
            ...
            const result1 = yield* _(
                Effect.flip(process(option, args1, CliConfig.defaultConfig)),
            );
            const result2 = yield* _(
                process(option, args2, CliConfig.defaultConfig),
            );
    ...
    it("validates with a Schema", () =>
        Effect.gen(function* (_) {
            ...
            assert.deepStrictEqual(result, [
                [],
                BigDecimal.unsafeFromString("100.50").pipe(
