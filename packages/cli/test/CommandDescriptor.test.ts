...
describe("Command", () => {
    ...
    describe("Subcommands without Options or Arguments", () => {
        ...
        it("should match the top-level command if no subcommands are specified", () =>
            Effect.gen(function* (_) {
                ...
                expect(result).toEqual(
                    CommandDirective.userDefined([], expected),
                );
            }).pipe(runEffect));
        ...
        it("matches the first subcommand with a surplus option", () =>
            Effect.gen(function* (_) {
                ...
                expect(result).toEqual(
                    CommandDirective.userDefined(Array.of("-v"), expected),
                );
            }).pipe(runEffect));
    ...
    describe("Nested Subcommands", () => {
        ...
        it("should properly parse deeply nested subcommands with options and arguments", () =>
            Effect.gen(function* (_) {
                ...
                const result = yield* _(
                    Descriptor.parse(command, args, CliConfig.defaultConfig),
                );
                const expected = {
                    name: "command",
                    options: void 0,
    ...
    describe("Help Documentation", () => {
        it("should allow adding help documentation to a command", () =>
            Effect.gen(function* (_) {
                ...
                const result = yield* _(
                    Descriptor.parse(cmd, args, CliConfig.defaultConfig),
                );
                const expectedValue = {
                    name: "tldr",
                    options: void 0,
        ...
        it("should correctly display help documentation for a command", () => {
            ...
            const result = Doc.render(
                Doc.unAnnotate(
                    HelpDoc.toAnsiDoc(Descriptor.getHelp(parent, config)),
                ),
                { style: "pretty" },
            );
            expect(result).toBe(
    ...
    describe("End of Command Options Symbol", () => {
        ...
        it("should properly handle the end of command options symbol", () =>
            Effect.gen(function* (_) {
                ...
                const result1 = yield* _(
                    Descriptor.parse(command, args1, CliConfig.defaultConfig),
                );
                const result2 = yield* _(
                    Descriptor.parse(command, args2, CliConfig.defaultConfig),
                );
                const result3 = yield* _(
                    Descriptor.parse(command, args3, CliConfig.defaultConfig),
                );
                const expected1 = {
                    name: "cmd",
    ...
    describe("Completions", () => {
        ...
        it("should create completions for the bash shell", () =>
            Effect.gen(function* (_) {
                const result = yield* _(
                    Descriptor.getBashCompletions(command, "forge"),
                );
                yield* _(
                    Effect.promise(() =>
                        expect(result).toMatchFileSnapshot(
                            "./snapshots/bash-completions",
                        ),
                    ),
                );
            }).pipe(runEffect));
        ...
        it("should create completions for the zsh shell", () =>
            Effect.gen(function* (_) {
                const result = yield* _(
                    Descriptor.getZshCompletions(command, "forge"),
                );
                yield* _(
                    Effect.promise(() =>
                        expect(result).toMatchFileSnapshot(
                            "./snapshots/zsh-completions",
                        ),
                    ),
                );
            }).pipe(runEffect));
        ...
        it("should create completions for the fish shell", () =>
            Effect.gen(function* (_) {
                const result = yield* _(
                    Descriptor.getFishCompletions(command, "forge"),
                );
                yield* _(
                    Effect.promise(() =>
                        expect(result).toMatchFileSnapshot(
                            "./snapshots/fish-completions",
                        ),
                    ),
                );
            }).pipe(runEffect));
