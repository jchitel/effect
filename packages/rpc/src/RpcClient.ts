...
>(
        ...
        Effect.withFiberRuntime<any, any, any>((parentFiber) => {
            ...
            const send = middleware({
                ...
                headers: Headers.merge(
                    parentFiber.getFiberRef(currentHeaders),
                    headers,
                ),
            });
            if (discard) {
    ...
    if (options.flatten) {
        ...
        for (const rpc of group.requests.values()) {
            ...
            const target =
                prefix !== undefined ? (client as any)[prefix] : client;
            const tag =
                prefix !== undefined ? rpc._tag.slice(dot + 1) : rpc._tag;
        ...
        onFromClient({ message }) {
            switch (message._tag) {
                case "Request": {
                    ...
                    const schemas = RpcSchema.getStreamSchemas(
                    ...
                    return Schema.encode(rpc.payloadSchema)(
                        message.payload,
                    ).pipe(
                ...
                return client.post("", { body }).pipe(
                    Effect.flatMap((r) =>
                        Stream.runForEachChunk(r.stream, (chunk) => {
                            const responses = Chunk.toReadonlyArray(
                                chunk,
                            ).flatMap(
                                parser.decode,
