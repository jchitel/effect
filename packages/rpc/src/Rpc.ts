...
export type SuccessExitEncoded<R> =
    SuccessEncoded<R> extends infer T
        ? T extends Stream<infer _A, infer _E, infer _Env>
            ? void
            : T
        : never;
...
export type SuccessChunkEncoded<R> =
    SuccessEncoded<R> extends Stream<infer _A, infer _E, infer _Env>
        ? _A
        : never;
 ...
 * @category models
 */
export interface From<S extends AnyTaggedRequestSchema>
    extends Rpc<S["_tag"], S, S["success"], S["failure"]> {}
...
export const make = <
    ...
    Payload extends
        | Schema.Schema.Any
        | Schema.Struct.Fields = typeof Schema.Void,
...
>(
    ...
    options?: {
        ...
        readonly primaryKey?: [Payload] extends [Schema.Struct.Fields]
            ? (
                  payload: Schema.Simplify<
                      Schema.Struct.Type<NoInfer<Payload>>
                  >,
              ) => string
            : never;
    },
...
> => {
    ...
    if (options?.primaryKey) {
        payloadSchema = class Payload extends (
            Schema.Class<Payload>(`@effect/rpc/Rpc/${tag}`)(
 ...
 * @category Fork
 */
export const fork = <A>(value: A): Fork<A> => ({
