...
> = Effect.fnUntraced(function* <Rpcs extends Rpc.Any>(
    ...
    const handleRequest = (
        ...
        const entry = context.unsafeMap.get(rpc?.key) as Rpc.Handler<
            Rpcs["_tag"]
        >;
        if (!rpc || !entry) {
        ...
        let responded = false;
        let effect = Effect.uninterruptible(
    ...
    const streamEffect = (
        ...
        stream:
            ...
            | Effect.Effect<
                  Mailbox.ReadonlyMailbox<any, any>,
                  any,
                  Scope.Scope
              >,
    ) => {
    ...
    const getSchemas = (rpc: Rpc.AnyWithProps) => {
        ...
        if (!schemas) {
            const entry = context.unsafeMap.get(rpc.key) as Rpc.Handler<
                Rpcs["_tag"]
            >;
            const streamSchemas = RpcSchema.getStreamSchemas(
...
> = Effect.gen(function* () {
    ...
    const httpApp: HttpApp.Default<never, Scope.Scope> = Effect.gen(
        function* () {
...
export const makeProtocolWebsocket: <I = HttpRouter.Default>(options: {
    readonly path: HttpRouter.PathInput;
    readonly routerTag?: Context.Tag<
        I,
        ...
        function* () {
                ...
                write: (response) => {
                    try {
                        ...
                        return !includesFraming
                            ...
                            : offer(
                                  parser.encode(ResponseDefectEncoded(cause))!,
                              );
                    }
                },
                end: mailbox.end,
            ...
            if (!includesFraming) {
                ...
                return HttpServerResponse.text(
                    parser.encode(responses) as string,
            ...
            return HttpServerResponse.stream(
                Stream.ensuringWith(
                    Mailbox.toStream(
                        mailbox as Mailbox.ReadonlyMailbox<Uint8Array>,
                    ),
                    (exit) => {
                        clients.delete(id);
                        disconnects.unsafeOffer(id);
