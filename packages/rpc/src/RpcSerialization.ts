...
export const jsonRpc = (options?: {
    ...
    RpcSerialization.of({
        ...
        unsafeMake: () => {
            ...
            return {
                decode: (bytes) => {
                    const decoded: JsonRpcMessage | Array<JsonRpcMessage> =
                        JSON.parse(
                            typeof bytes === "string"
                                ? bytes
                ...
                encode: (response) => {
                    if (Array.isArray(response)) {
                        ...
                        return JSON.stringify(
                            response.map(encodeJsonRpcMessage),
                        );
                    }
            ...
            return {
                decode: (bytes) => {
                    ...
                    for (let i = 0; i < frames.length; i++) {
                        ...
                        messages.push(
                            ...(decodeJsonRpcRaw(frame as any, batches) as any),
                        );
                    }
                    return messages;
                },
