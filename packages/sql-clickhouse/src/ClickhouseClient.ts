        ...
        class ConnectionImpl implements Connection {
            ...
            private runRaw(
                ...
                >((fiber) => {
                    ...
                    >((resume) => {
                        const queryId =
                            fiber.getFiberRef(currentQueryId) ??
                            Crypto.randomUUID();
                        const settings =
                            fiber.getFiberRef(currentClickhouseSettings) ?? {};
                    ...
                    >((fiber) =>
                        Effect.async((resume) => {
                            const queryId =
                                fiber.getFiberRef(currentQueryId) ??
                                Crypto.randomUUID();
                            const settings = fiber.getFiberRef(
                                currentClickhouseSettings,
                            );
                            const controller = new AbortController();
                            client
                                .insert({
...
interface ClickhouseParam
    extends Statement.Custom<"ClickhouseParam", string, Statement.Primitive> {}
