    ...
    Effect.gen(function* () {
        ...
        return UserRpcs.of({
            ...
            StreamUsers: Effect.fnUntraced(function* (req) {
                ...
                yield* mailbox
                    .offer(new User({ id: req.id, name: "John" }))
                    .pipe(
                        Effect.tap(() => {
                            emits++;
                        }),
            ...
            GetTimingMiddlewareMetrics: () =>
                Effect.all({
                    defect: Metric.value(rpcDefects).pipe(
                        Effect.map((_) => _.count),
                    ),
                    success: Metric.value(rpcSuccesses).pipe(
                        Effect.map((_) => _.count),
                    ),
                    count: Metric.value(rpcCount).pipe(
                        Effect.map((_) => _.count),
                    ),
                }),
        });
