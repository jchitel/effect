...
describe("HttpServer", () => {
        ...
        Effect.gen(function* () {
            yield* HttpRouter.empty.pipe(
                HttpRouter.post(
                    ...
                    Effect.gen(function* () {
                        const result =
                            yield* HttpServerRequest.schemaBodyFormJson(
                                Schema.Struct({
                                    test: Schema.String,
                                }),
                            )("json");
    ...
    it.scoped("tracing", () =>
        Effect.gen(function* () {
            ...
            expect((body as any).parent.value.spanId).toEqual(
                requestSpan.spanId,
            );
        }).pipe(Effect.provide(NodeHttpServer.layerTest)),
    );
    ...
    it.scoped("multiplex", () =>
        Effect.gen(function* () {
            yield* HttpMultiplex.empty.pipe(
                HttpMultiplex.hostExact(
                    "a.example.com",
                    HttpServerResponse.text("A"),
                ),
                HttpMultiplex.hostStartsWith(
                    "b.",
                    HttpServerResponse.text("B"),
                ),
                HttpMultiplex.hostRegex(
                    /^c\.example/,
                    HttpServerResponse.text("C"),
                ),
                HttpServer.serveEffect(),
            );
            const client = yield* HttpClient.HttpClient;
            ...
            Effect.gen(function* () {
                class CustomError extends Schema.TaggedError<CustomError>()(
                    ...
                    [HttpServerRespondable.symbol]() {
                        return HttpServerResponse.schemaJson(CustomError)(
                            this,
                            { status: 599 },
                        );
                    }
                ...
                const err =
                    yield* HttpClientResponse.schemaBodyJson(CustomError)(res);
        ...
        it.scoped("respondable schema", () =>
            Effect.gen(function* () {
                ...
                yield* HttpRouter.empty.pipe(
                    HttpRouter.get(
                        "/user",
                        Effect.succeed(new User({ name: "test" })),
                    ),
                    HttpServer.serveEffect(),
                );
                const client = yield* HttpClient.HttpClient;
    ...
    it.scoped("concat", () =>
        Effect.gen(function* () {
            yield* HttpRouter.concat(routerA, routerB).pipe(
                HttpServer.serveEffect(),
            );
