...
describe("HttpApi", () => {
    describe("payload", () => {
        ...
        it.live("multipart stream", () =>
            Effect.gen(function* () {
                ...
                const result = yield* client.users.uploadStream({
                    payload: data,
                });
                assert.deepStrictEqual(result, {
                    contentType: "text/plain",
    ...
    describe("security", () => {
        ...
        it.effect("apiKey header security", () =>
            Effect.gen(function* () {
                const decode = HttpApiBuilder.securityDecode(
                    securityHeader,
                ).pipe(
                    Effect.provideService(
    ...
    it.effect("multiple payload types", () =>
        Effect.gen(function* () {
            ...
            assert.deepStrictEqual(
                group,
                new Group({ id: 1, name: "Some group" }),
            );
            assert.strictEqual(response.status, 200);
    ...
    it.effect("error from plain text", () => {
        ...
        const Api = HttpApi.make("api").add(
            HttpApiGroup.make("group").add(
                HttpApiEndpoint.get("error")`/error`.addError(
...
class GroupsApi extends HttpApiGroup.make("groups")
    .add(
        HttpApiEndpoint.get(
            "findById",
        )`/${HttpApiSchema.param("id", Schema.NumberFromString)}`.addSuccess(
            Group,
        ),
    )
    .add(
...
class UsersApi extends HttpApiGroup.make("users")
    ...
    .add(
        HttpApiEndpoint.post(
            "upload",
        )`/upload/${Schema.optional(Schema.String)}`
            .setPayload(
                HttpApiSchema.Multipart(
                    Schema.Struct({
    ...
    handlers
        ...
        .handleRaw(
            ...
            Effect.fn(function* ({ path, request }) {
                const body = (yield* Effect.orDie(request.json)) as {
                    name: string;
                };
                return HttpServerResponse.unsafeJson({
                    id: path.id,
