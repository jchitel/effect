...
describe.concurrent("SemiProduct", () => {
    it("productMany should be equivalent to `ap`", () => {
        const curry =
            (f: Function, n: number, acc: ReadonlyArray<unknown>) =>
            (x: unknown) => {
    ...
    describe.concurrent("productManyComposition", () => {
        ...
        it("Option", () => {
            ...
            U.deepStrictEqual(
                productMany(O.some(O.none()), []),
                O.some(O.none()),
            );
            U.deepStrictEqual(
                productMany(O.some(O.some(1)), []),
                O.some(O.some([1])),
            );
            U.deepStrictEqual(productMany(O.none(), [O.none()]), O.none());
            U.deepStrictEqual(
                productMany(O.some(O.none()), [O.none()]),
                O.none(),
            );
            U.deepStrictEqual(
    ...
    describe.concurrent("appendElement", () => {
        it("Covariant (Option)", () => {
            ...
            U.deepStrictEqual(
                pipe(O.some([1, 2]), appendElement(O.none())),
                O.none(),
            );
        ...
        it("Contravariant (Predicate)", () => {
            const appendElement = _.appendElement(
                PredicateInstances.SemiProduct,
            );
            const p = pipe(
    ...
    describe.concurrent("nonEmptyTuple", () => {
        ...
        it("Contravariant (Predicate)", () => {
            const nonEmptyTuple = _.nonEmptyTuple(
                PredicateInstances.SemiProduct,
            );
