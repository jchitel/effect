...
export const makeExternalSpan = (options: {
    ...
    if (typeof options.traceState === "string") {
        context = Option.match(createTraceState(options.traceState), {
            onNone: () => context,
            onSome: (traceState) =>
                Context.add(context, traceStateTag, traceState),
        });
...
/** @internal */
export const currentOtelSpan = Effect.flatMap(Effect.currentSpan, (span) => {
    ...
    Effect.flatMap(
        ...
        ([resource, provider]) =>
            Effect.sync(() =>
                provider.getTracer(
                    resource.attributes[
                        OtelSemConv.ATTR_SERVICE_NAME
                    ] as string,
                    resource.attributes[
                        OtelSemConv.ATTR_SERVICE_VERSION
                    ] as string,
                ),
            ),
    ),
